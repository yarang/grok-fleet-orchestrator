...
pub enum SelectionError {
    ...
    /// 않아 `Online` 표시를 신뢰할 수 없다. `AllOffline`과 구분하는 이유는
    /// 운영자가 볼 원인이 다르기 때문이다: 저쪽은 "워커를 켜라"이고, 이쪽은
    /// "확인했는데 답하지 않았다"이다.
    ///
    /// **2026-09-06 이전에는 이 문장이 "이를 확인할 probe는 아직 없다(로드맵
    /// `#67` 의존)"였고 뒤 절도 "probe가 구현될 때까지 이 워커는 배정 대상이
    /// 아니다"였다.** probe가 들어온 뒤에도 남아 있어, 이 파일 최상단이 반대를
    /// 적는 자기모순이 됐다 — 이 독스트링을 뒤에서 읽는 사람을 없는 의존으로
    /// 다시 보냈다.
