...
//!
//! **여기의 판정은 여전히 원자적이지 않다 — 그래도 상한은 집행된다.**
//! 아래 필터는 읽은 시점의 카운트로 판정하므로 동시에 들어온 두 요청이
//! 같은 Worker를 고를 수 있다. 그것을 막는 것은 이 모듈이 아니라 Store다:
//! [`fleet_store::Store::create_agent`]와
//! [`fleet_store::Store::assign_agent_worker`]가 `workers` 행을 `FOR UPDATE`로
//! 잠근 아래에서 다시 세고 조건부로 쓴다(`#67` 구현 게이트 ①-A-2). 여기서
//! 거르는 이유는 선점 실패를 **줄이기** 위해서이지 막기 위해서가 아니다.
//!
//! 그래서 남는 것은 다른 종류의 한계다. `choose_worker`는 승자를 **하나만**
//! 돌려주므로, 두 요청이 같은 후보를 골랐다가 한쪽이 선점에 실패하면 그
//! 요청은 다른 Worker가 비어 있어도 미배정으로 끝난다. 불변식은 깨지지
//! 않지만 배정이 놓친다 — 후보를 순위 목록으로 돌려 흘러내리게 하는 것이
//! 그 답이고, 지금은 `POST /api/agents/{id}/place`가 회복 경로라 미룬다.
//!
