    ...
    pub async fn apply_one_to(state: &FleetState, event: &FleetEvent) -> bool {
        match event {
            FleetEvent::WorkerCircuitChanged {
                ...
                let cb = state.breakers.get(*worker_id, *to);
                match to {
                    CircuitState::Open => {
...
mod tests {
    ...
    impl fleet_store::Store for NoopStore {
        ...
        async fn delete_task(
            &self,
            _: TaskId,
        ) -> Result<fleet_core::TaskDeleteOutcome, fleet_store::StoreError> {
            unimplemented!()
        }
        async fn bump_worker_incarnation(
            &self,
            _: fleet_core::WorkerId,
        ) -> Result<Option<chrono::DateTime<chrono::Utc>>, fleet_store::StoreError> {
            unimplemented!()
        }
