...
pub enum TransitionOutcome {
    ...
    Fenced,
    /// 호출자는 현재 제어 기관이 맞지만, 보고된 결과가 **다른 세대가 디스패치한**
    /// 작업의 것이라 아무것도 쓰지 않았다 (로드맵 #67 1단계, 불변식 ②).
    ///
    /// `Fenced`와 나누는 이유는 `Fenced`를 `Rejected`와 나눈 이유와 같다 —
    /// 후속 동작이 다르다. `Fenced`는 "나는 더 이상 제어 기관이 아니다"이므로
    /// 이후의 모든 쓰기도 실패하지만, 이 값은 "나는 제어 기관이 맞고 이
    /// **결과 하나**가 내 세대의 것이 아니다"이므로 다음 쓰기는 정상 동작한다.
    /// 둘을 뭉개면 리스를 정상 보유한 인스턴스가 낡은 결과 하나 때문에 제어권을
    /// 잃었다고 판단하게 된다.
    ///
    /// `Rejected`와도 나눠야 한다. 이 경우 위상은 기대와 **일치**했으므로
    /// `Rejected { current: Dispatched }`는 "다른 writer가 먼저 옮겼다"는
    /// 거짓을 말하게 된다. 운영자를 존재하지 않는 경합 조사로 보내는 값이다.
    ///
    /// `dispatched_under`는 `Rejected { current }`와 달리 제어 흐름에 써도
    /// 안전하다 — `tasks.dispatch_control_epoch`는 Dispatched 전이에서 한 번
    /// 쓰이고 그 뒤로 바뀌지 않으므로, 재조회해도 같은 값이다.
    StaleDispatchEpoch { dispatched_under: i64 },
}

/// 상태 전이가 담은 정보가 **어디서 왔는가** (로드맵 #67 1단계).
///
/// `compare_and_set_task_status`가 dispatch 세대 술어를 걸지 말지를 가른다.
/// 저장소는 이것을 목표 상태로부터 유도할 수 없다 — `Failed`는 워커가 보고한
/// 실패이기도 하고, reconciler가 고아를 회수하며 내리는 새 결정이기도 하다.
/// 목표 상태로 유도하려 들면 후자가 술어에 걸려 **epoch 5에 디스패치된 고아를
/// epoch 7 보유자가 영원히 회수하지 못하는** 라이브락이 된다. 그래서 호출자가
/// 명시한다.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum TransitionOrigin {
    /// 현재 리스 보유자가 **지금** 내리는 결정 — dispatch, 취소, 고아 회수,
    /// dispatch 실패 확정. 이 전이가 담은 정보는 전부 현재 세대의 것이므로
    /// 작업을 어느 세대가 디스패치했는지는 묻지 않는다.
    ControlDecision,
    /// 디스패치된 작업의 **결과 보고** — 워커가 보낸 완료/실패. 이 정보는 이
    /// 작업을 디스패치한 세대에 속하므로, 그 세대가 현재 세대와 다르면
    /// 적용해선 안 된다.
    WorkerOutcome,
}
