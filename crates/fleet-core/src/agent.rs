...
/// Worker가 제어면을 잃어 **스스로 멈춘** Agent 프로세스 하나에 대한 보고
/// (로드맵 `#67` 게이트 ⑥).
///
/// [`AgentOrphan`]과 형제 필드로 따로 싣는다. 두 사건은 프로세스를 죽인다는
/// 결과만 같고 나머지가 전부 다르다:
///
/// - orphan은 정의상 **이 Worker에 배정되지 않은** 프로세스다. 그래서 그
///   보고는 `agents` 행에 닿을 자리가 아예 없다(`036`이 금지한다).
/// - 이쪽은 **배정이 유효한 채로** 멈춘 것이다. 그 사실은 다음 beat의 관측
///   목록이 이미 말한다 — 멈춘 Agent가 목록에서 빠지므로
///   `apply_agent_observations`가 그 관측을 지운다. 즉 상태는 저절로 옳아지고,
///   이 타입이 나르는 것은 **왜** 그렇게 됐는지다.
///
/// 그 "왜"가 없으면 운영자에게는 멀쩡하던 Agent가 이유 없이 멈춘 것으로
/// 보인다. `unplaced` orphan이 "게이트 ②의 술어가 뚫렸다"를 뜻하는 것과 달리
/// 이 사건의 처방은 **네트워크**이며, 처방이 다르므로 값도 따로 둔다.
///
/// **`AgentOrphanReason`의 세 번째 값으로 넣지 않는다.** 그 타입이 붙어 있는
/// [`AgentOrphan`]의 정의가 "배정되지 않은 프로세스"이고, 배정된 것을 그
/// 목록에 실으면 타입의 이름과 독스트링이 곧바로 거짓이 된다.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct AgentFenced {
    pub agent_id: AgentId,
    /// 마지막으로 성공한 heartbeat 이후 흐른 시간(초).
    ///
    /// 운영자가 이 값 하나로 "설정된 유예를 실제로 넘겼는가"를 확인할 수 있게
    /// 싣는다. Worker의 시계로 잰 **경과 시간**이라 두 호스트의 시계가 어긋나도
    /// 뜻이 변하지 않는다 — 절대 시각을 실으면 그렇지 않다.
    pub unreachable_secs: u64,
}
...
impl Agent {
    ...
    /// 배정을 지운다 — 저장소가 슬롯 선점에 실패했을 때 로컬 구조체를
    /// 저장된 사실에 되맞추는 자리다 (로드맵 `#67` 구현 게이트 ①-A-2).
    ///
    /// **두 필드를 함께 지운다.** `030`의 `agents_placement_complete`가
    /// "둘 다 있거나 둘 다 없거나"를 요구하므로, 하나만 지운 구조체는
    /// 저장하면 거절될 상태를 메모리에서 표현한 것이 된다.
    ///
    /// 이것은 운영자의 배정 **해제** 표면이 아니다 — 그런 표면은 없고,
    /// 해제가 필요한 두 경우(Worker 등록 해제, Agent 회수)는 각각 FK의
    /// `ON DELETE SET NULL`과 원장의 `stopped` 제외가 처리한다.
