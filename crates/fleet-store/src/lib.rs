...
pub enum SlotClaim {
    ...
    Fenced,
    /// **다른** Worker가 이 Agent의 프로세스를 돌리고 있다고 보고했다
    /// (`observed_status = 'running'`). 옮기지 않았다 (로드맵 `#67` 게이트 ②).
    ///
    /// 이 거절이 필요한 이유는 관측이 **분할 중에도 단조롭기** 때문이다.
    /// 명령은 heartbeat 응답으로만 가므로 제어평면과 끊긴 Worker에는 새
    /// 프로세스가 생길 수 없다. 그래서 "돌고 있다고 보고했다"는 사실은
    /// 연락이 끊긴 뒤에도 유효하고, 그 상태에서 옮기면 같은 Agent가 두
    /// 곳에서 도는 것을 막을 방법이 없다.
    ///
    /// [`Fenced`](Self::Fenced)와 같은 이유로 별도 값이다 — 술어에 걸린
    /// UPDATE는 0행을 돌려주므로 뭉개면 [`NoSuchAgent`](Self::NoSuchAgent)가
    /// 되고, 운영자는 "돌고 있어서 못 옮겼다"는 사실 대신 존재하지 않는
    /// Agent를 찾으라는 안내를 받는다. 후속 동작도 다르다 — 회수해서 프로세스를
    /// 죽이고 관측이 비면 그때 옮길 수 있다.
    ///
    /// 같은 Worker로의 재배정은 이 값을 내지 않는다. 프로세스가 움직이지
    /// 않으므로 중복이 생길 여지가 없고, 이는 [`CapReached`](Self::CapReached)가
    /// 자기 자신을 세지 않는 것과 같은 근거다.
    ObservedRunning,
}
...
pub trait Store: Send + Sync {
    ...
    /// (HA 리스를 쓰지 않는 배포).
    ///
    /// **다른 Worker가 돌리고 있다고 보고한 Agent는 옮기지 않는다**
    /// (로드맵 `#67` 게이트 ②, [`SlotClaim::ObservedRunning`]). 술어는
    /// `fence`와 마찬가지로 **같은 UPDATE 문 안에** 있어야 한다 — 먼저 읽고
    /// 분기하면 그 사이에 도착한 beat이 `running`을 적어도 이미 떠난 UPDATE는
    /// 그대로 도착하고, 그것이 정확히 막으려는 중복이다.
    ///
    /// ## 살아 있는 Agent를 옮기는 순서
    ///
    /// 회수(`set_agent_desired_status` → Worker가 프로세스를 죽임 → 다음 beat이
    /// 관측을 비움) → 이 메서드 → 재기동. 그래서 이 메서드는 `stopped` Agent를
    /// **거절하지 않는다**. 호출부에 있던 "회수된 Agent는 배정하지 않는다"
    /// 400 가드는 게이트 ②와 함께 걷어냈다: 그 가드의 근거("아무도 읽지 않는
    /// 값을 쓰는 것이 된다")는 `list_agent_commands`가 미확인인 동안 `stopped`
    /// 행도 싣는다는 사실과, 이 순서가 유일한 안전한 이동 경로라는 사실 둘 다에
    /// 의해 무너진다.
    ///
    /// ## 남는 창
    ///
    /// 관측은 heartbeat 주기만큼 뒤처진다. Worker가 프로세스를 띄운 뒤 다음
    /// 보고까지의 한 주기 동안 관측은 아직 NULL이고, 그 사이의 재배정은 통과한다.
    /// 이 창은 관측을 신선도로 판정해서는 닫히지 않는다 — 신선하지 않은 관측을
    /// 무시하면 분할된 Worker의 `running`이 곧바로 무시 대상이 되어 게이트 ②가
    /// 통째로 무력해진다. 창을 닫으려면 Worker 쪽 프로세스 인벤토리(게이트 ③)가
    /// 필요하다.
