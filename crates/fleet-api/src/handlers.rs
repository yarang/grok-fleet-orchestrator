...
/// 한 beat이 감사에 남기는 고아 종료 사건의 상한 (로드맵 `#70` 게이트 3).
///
...
pub async fn join_worker(
    ...
    let worker_config_toml = render_worker_config_toml(WorkerConfigTomlParams {
        ...
        liveness_mode: req.liveness_mode,
        orchestrator_url: state.public_base_url.as_deref(),
    });
...
struct WorkerConfigTomlParams<'a> {
    ...
    /// orchestrator의 공개 base URL (`AppState::public_base_url`,
    /// `FLEET_BASE_URL` env). `None`이면 플레이스홀더를 그대로 남긴다.
    orchestrator_url: Option<&'a str>,
}
...
fn render_worker_config_toml(params: WorkerConfigTomlParams<'_>) -> String {
    ...
    // bind_addr는 이 워커가 로컬에서 ACP 서버를 리슨할 주소다 — agent_endpoint는
    // orchestrator가 워커에 도달하는 공개 주소(Cloudflare Tunnel 등 리버스 경로 뒤에
    // 있는 경우가 많음)라 서로 다른 값이며, agent_endpoint에서 파생시키면 잘못된
    // host(예: 터널 도메인)를 bind_addr로 써서 리슨에 실패한다. 기존 배포
    // (worker-ec1/ec2)와 동일한 고정 기본값을 쓴다.
    const DEFAULT_BIND_ADDR: &str = "0.0.0.0:2419";
    ...
    match orchestrator_url {
        Some(url) if !url.trim().is_empty() => {
            out.push_str(&format!("orchestrator_url = \"{url}\"\n"));
        }
        _ => out.push_str("orchestrator_url = \"<set-to-your-orchestrator-url>\"\n"),
    }
    out.push_str(&format!(
    ...
    out.push_str(&format!("bind_addr = \"{DEFAULT_BIND_ADDR}\"\n"));
