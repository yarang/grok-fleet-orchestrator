...
## 서로를 선행으로 지목한 두 항목은 아무도 시작하지 않는다 (#70)

`#70`에 착수하려고 게이트 현황표를 읽었다. 표는 `## 구현 상태 (2026-08-28)`이라는 제목을 달고
일곱 게이트의 막힘을 적고 있었는데, 그 사이에 `#67`이 4a부터 게이트 ④까지 거의 다 닫혔다.
차단 사유가 만료됐을 가능성을 코드로 확인했고, 결과는 행마다 갈렸다.

**표의 신선도가 행마다 달랐고, 제목 하나가 그것을 대표하고 있었다.** `git blame`으로 보면 표는
`f054868`(2026-08-29)에 통째로 쓰였는데 제목은 `2026-08-28`이고, 게이트 3 행만 `38a3633`
(2026-09-01)에서 고쳐졌는데 제목은 그대로였다. 즉 그 날짜는 **어느 행에도 맞지 않는다**. 행 하나를
고치면서 제목을 올리면 손대지 않은 여섯 행이 "오늘 확인됨"으로 위장되고, 올리지 않으면 고친 행이
과거로 위장된다. 이번에는 일곱 행을 전부 재확인하고 제목을 올리는 쪽으로 풀었다 — 표의 날짜가
의미를 가지려면 그것이 **모든 행에 대한 확인 날짜**여야 한다.

재확인 결과 세 행(1·4·7)은 그대로 참이었다. `EffectLedger`/`PartiallyApplied`도 `CancelUnconfirmed`도
여전히 grep 0건이다. 나머지 넷은 서로 다른 방식으로 틀려 있었다.

**게이트 2는 차단이 약해진 게 아니라 사유가 틀려 있었다.** 표는 "control epoch·fencing token을 갖는
Reconciler가 없다"고 적었는데, `crates/fleet-scheduler/src/reconcile.rs:175`에 `struct Reconciler`가
있고 `lease_allows_control()`로 lease를 잃은 인스턴스의 sweep 전체를 건너뛰며 그 시험
(`reconcile_once_skips_the_whole_sweep_when_control_plane_lease_is_fenced`)까지 있다. 선행으로 적힌
`#63`도 1~4단계로 완료됐다. 참인 차단은 **inventory-first라는 순서**다 — 이 Reconciler는 `#62`의
stale sweeper라 저장소가 기억하는 작업만 훑고, failover 뒤 워커에게 "무엇을 들고 있는가"를 먼저
묻지 않는다. 그 물음의 답은 게이트 3이 기다리는 것과 같은 것이므로, 선행은 `#63`이 아니라 게이트 3이다.

**게이트 6은 취소된 이름을 차단 사유로 들고 있었다.** `lease_generation`·`fencing_token`은
`worker_execution_lease`에 딸린 이름인데 그 테이블은 2026-09-01에 만들지 않기로 확정됐다.
`da523a6`가 로드맵 세 행에서 같은 이름을 지웠지만 이 표의 행 6은 놓쳤다. 게다가 "audit.rs는
actor·outcome 계열만 갖는다"도 이제 거짓이다 — `project_id`는 `037`로 1급 컬럼이 되어 `AuditFilter`
술어와 인덱스를 갖고(`#95` 1단계), Agent 배정·회수의 `generation`은 `detail`에 실린다(`#67` 게이트 ④).
남은 진짜 차단은 effect 경로 하나다.

**게이트 5에서 가장 큰 것이 나왔다: 귀속이 순환하고 있었다.** 이 표와 `selector.rs`·`health.rs`
주석은 ACP probe를 "`#67` 의존"이라 적고, `#67` 로드맵 행과 `placement.rs`는 같은 probe를
"`#70`가 푼다"고 적는다. 두 항목이 서로를 선행으로 지목하면 **어느 쪽 세션이 와도 상대에게
미루므로 그 작업은 영원히 시작되지 않는다.** 이것은 표기 문제가 아니라 데드락이고, 로드맵을
읽고 다음 작업을 고르는 방식으로 일하는 한 조용히 유지된다.

소유를 `#70` 게이트 5로 확정했다. 근거는 셋이다. (1) 게이트 5의 문장 자체가 "probe 성공 전
dispatch되지 않는 시험"이라 probe를 자기 완료 조건으로 명시한다. (2) probe는 Agent 프로비저닝이
아니라 Worker liveness다. (3) 수단인 `WorkerTransport::ping(worker_id) -> Duration`은 `#67`이 아니라
`fleet-transport`에 이미 있다. 세 번째가 차단의 크기도 바꾼다 — 표는 "ACP probe가 없어 미구현"이라
적었지만 없는 것은 probe라는 **수단**이 아니라 selector가 dispatch 직전에 그것을 부르고 결과를
후보 판정에 되먹이는 **배선**이다.

**게이트 3은 차단이 유지되지만 사유가 좁아졌다.** `#67` 4c-B가 관측 채널(`agents.observed_status`,
migration 032)을 넣었으므로 "비교할 process inventory가 없다"는 서술은 이제 너무 넓다. 정확히는
`crates/fleet-worker/src/agent_process.rs`의 재조정 루프가 관측을 `Vec::with_capacity(commands.len())`에
담고 `commands.iter().filter(desired_status == Running)`만 순회한다 — 오케스트레이터가 **이미
배치했다고 믿는** Agent를 확인·부인할 뿐, 명령한 적 없는 프로세스를 실어 나를 **형식 자체가 없다**.
같은 루프 2단계는 목록에서 사라진 프로세스를 보고 없이 종료하고, `procs`는 in-memory라 워커가
SIGKILL되면 워커 자신도 자기 자식을 잃는다. 즉 orphan은 지금 wire format에서 **표현 불가능**하다.

이 정밀화가 실용적인 이유는 게이트 2와의 연결이 드러나기 때문이다. 두 게이트가 같은 것 하나를
기다리고 있었는데 표에는 각각 다른 선행(`#63`, `#67`)이 적혀 있어서 그렇게 보이지 않았다.

**표를 고치고 나서 그 위의 요약문을 두 번 틀렸다.** 첫 판은 "닫힘 하나, 부분 하나, 나머지
다섯"이라고 쓴 뒤 그 다섯을 "게이트 4·7은 없어서, 게이트 2·5·6은 배선이 없어서"로 갈랐다.
게이트 5는 **부분**이라 애초에 나머지 다섯에 속하지 않고, 그렇게 5를 넣는 바람에 게이트 3이
문장에서 통째로 사라졌다. 표의 일곱 행은 전부 맞는데 바로 위 한 문단만 틀린 상태였다.
**표 무결성은 기계로 재지만(파이프 개수·셀 수) 요약문의 산술은 아무도 재지 않는다** — 이
문서에서 사람만 검증할 수 있는 자리가 정확히 거기다. 고친 문장은 남은 다섯을 2·3·4·6·7로
명시하고, 그 다섯을 다시 셋(4·7 부재 / 2·3 같은 inventory / 6은 게이트 4 대기)으로 가른다.
두 번째 틀림은 그 문장이 `①②⑤`를 썼다는 것이다 — 같은 문서의 게이트 목록과 표는 `1.`~`7.`을
쓰므로, 독자가 문장과 표를 대조할 수 없게 된다.

**순환을 끊는 것과 단방향임을 기록하는 것은 다른 작업이다.** `#70`의 선행에서 `#67`을 뺐으므로
순환 자체는 사라졌지만, `#67` 행에는 `on_demand` 워커의 desired state가 도달하지 않는다는 한계의
해소처로 `#70`을 가리키는 화살표가 그대로 남는다. 그 화살표는 정상이다 — 완료 조건이 아니라
알려진 한계의 해소처이므로 단방향 의존이다. 그런데 **그 판단이 어디에도 적혀 있지 않으면**
다음에 `#67` 행을 읽는 사람은 `#70`을 가리키는 화살표만 보고 순환이 아직 있다고 읽고, 오늘 한
조사를 그대로 반복한다. 그래서 `#67` 행의 그 자리에 단방향이라는 확정을 붙였다. 이력이 아니라
**읽는 자리**에 있어야 효력이 있다는 것이 이 항목 전체의 교훈과 같다.

### 게이트

`rustc 1.98.0`(CI와 일치), `RUSTFLAGS="-D warnings"`, `cargo fmt --all -- --check`, clippy 두 피처
세트(`--all-targets -- -D warnings`), 그리고 각 세트마다 `cargo build -p fleet-cli` 후
`DATABASE_URL`을 주입한 `cargo test --workspace --no-fail-fast -- --test-threads=1`.

**두 번 돌렸다.** 1차는 7단계 전부 `exit=0`, 양쪽 피처 세트에서 65 suite·1330/1326건 통과였다.
그 뒤 위의 요약문·표기·`#67` 행 정정이 들어갔고, 게이트를 돌린 사실과 그 게이트가 **지금 트리를
가리키는 것**은 다르므로(agent.md §4.3) 정정 후 트리에 대해 다시 돌렸다. **2차 판정은 커밋
메시지에 적는다** — 판정을 이 파일에 적으면 그 문장이 트리를 바꾸고, 바뀐 트리는 방금 인용한
게이트가 판정한 적 없는 트리가 되어 무한 후퇴가 된다. 커밋 메시지는 게이트가 보는 트리 밖이라
그 후퇴가 끊긴다.

1차에서 조용한 skip이 없음도 확인했다. `0.00s`로 끝난 suite는 `verify_env_example.rs`와
`verify_examples.rs` 둘뿐이고 이들은 파일만 읽으므로 정상이며, DB를 쓰는 store 계열 8개는
합계 111건을 2.04s에, `cross_client`는 15건을 7.51s에 실제로 돌았다.

**이 게이트가 증명하지 않는 것이 이번에는 거의 전부다.** 코드 변경은 주석 네 줄의 로드맵 번호뿐이라
컴파일 결과가 바뀔 여지가 없고, 게이트는 "문서 정정이 코드를 깨지 않았다"만 말한다. 이 항목이
주장하는 것 — 어떤 차단이 만료됐고 어떤 것이 좁아졌는가 — 은 게이트가 아니라 grep과 파일 읽기가
근거다. 각 주장에 코드 위치를 적어 둔 이유가 그것이다. 특히 게이트 5의 "배선만 없다"는 주장은
`ping`의 시그니처를 읽은 것이지 그것으로 on_demand 워커를 실제로 깨워 본 것이 아니다.

### 남은 것

게이트 2·3이 기다리는 process inventory가 다음 착수 지점이다 — 워커가 **명령과 무관하게** 들고 있는
것을 여는 보고이고, `#67` 4c-A의 프로세스 관리자가 이미 그 생산자다. 게이트 5의 배선은 그것과
독립이라 병렬로 갈 수 있다. 게이트 4·7은 여전히 하부 구조가 없어 시험을 쓸 수조차 없고, 게이트 6은
게이트 4가 열려야 열린다.

## 같은 글리프가 두 목록을 세면, 규칙이 없는 것보다 규칙이 어긋난 것이 더 나쁘다

`#67` 행을 읽다가 원문자 게이트 번호가 두 목록을 센다는 것을 발견했다. `①②③`은 그 셀 자신의
"다음 게이트" 목록(2026-08-29 정의)을 세는데, `④`는 provisioning.md 「구현 게이트」의 4번
("명령·ACK·actor·generation의 감사 추적")을 센다 — 문면이 그 항목과 정확히 일치한다. 한 셀
안에서 같은 글리프가 두 출처를 가리키고 있었다. 직전 항목이 `#70`에서 고친 `①②⑤` 대 `1.`~`7.`
불일치와 같은 계열이며, 다른 점은 그때는 **표기가 서로 달라서** 대조가 끊겼고 이번에는
**표기가 같아서** 대조가 틀린 답을 준다는 것이다.

저장소가 실제로 따르는 규칙은 이렇다. **원문자는 `#67` 로드맵의 게이트 목록을 세고, 아라비아
숫자는 각 문서가 자기 「구현 게이트」를 셀 때 쓴다.** provisioning.md의 절 제목
`## 배정 슬롯 상한 (#67 구현 게이트 ①-A)`·`## 재배정 관측 술어 (#67 구현 게이트 ②)`,
control-plane 유예 표의 `게이트 ①-A/①-B`, `dashboard-api.md`·`agent-management.md`의
"`#67` 게이트 ②"가 전부 원문자로 로드맵 목록을 되받고, `#70`(observability-and-reconciliation.md)
1–7과 control-plane 1–7, provisioning 1–6은 전부 아라비아다. `docs/`에서 원문자 게이트 표기 67건을 전수로 확인했고 **어긋난 번호는
`④` 하나뿐**이었다(그 하나가 네 곳에 적혀 있었다). 그 하나가 만든 결과는 `#70` 게이트 3 행에서 한 문장으로 드러난다 —
"`#67` 게이트 ④"(provisioning 4번)와 "effect ledger가 없으므로(게이트 4)"(`#70` 4번)가 나란히
서 있었다. 고친 자리는 로드맵 `#67` 셀 둘(상태 칸·상세 칸), provisioning.md 절 제목과 본문,
그리고 그 `#70` 행이며, 규칙 자체는 목록을 배우는 자리 — "다음 게이트" 정의 바로 뒤 — 에 적었다.

**두 번째 결함은 게이트를 그것을 막는 것의 이름으로 불렀다는 것이다.** 세 곳이 "게이트
③(process inventory)"라고 적고 있었는데, ③의 이름은 "ACK 유실 `OutcomeUnknown`"이고 process
inventory는 그 게이트를 막는 **두 판독기 중 하나**다(나머지 하나는 effect ledger). 차단 요인으로
게이트를 부르면 나머지 절반이 시야에서 사라지고, 실제로 직전 항목의 "남은 것"이 그렇게 적혔다.
`#63` 행의 이름 없는 "게이트 ③"에도 이름을 붙였다.

**내가 이번에 두 번 틀렸고, 두 번째는 파일에 들어갔다가 되돌렸다.** 처음에는 접두사(「구현
게이트」 대 「게이트」)가 목록을 구분한다고 읽었는데, 같은 셀의 "구현 게이트 ①"·"구현 게이트
①-A-1"이 전부 로드맵 목록을 가리켜 반증됐다. 그다음 "정본 문서의 목록은 아라비아로 쓴다"는
규칙을 **선언해 파일에 써 넣었고**, provisioning.md가 자기 게이트를 `#67` 원문자로 되받는 절
제목 셋을 뒤늦게 보고 되돌렸다. 교훈은 이것이다 — **표기 규칙을 적는 것은 관찰이 아니라 규칙을
만드는 행위다.** 틀린 규칙은 원래의 모호함보다 나쁘다: 모호할 때 독자는 확인하지만, 규칙이
적혀 있으면 신뢰하고 확인하지 않는다. 규칙을 쓰기 전에 전수 조사가 선행 조건이고, 이번에는
`grep -rno "게이트 [①-⑦]" docs/ --exclude=log.md`의 67건을 다 읽은 뒤에야 참인 문장을
쓸 수 있었다.

**직전 항목의 "남은 것"을 정정한다.** 거기에 "게이트 2·3이 기다리는 process inventory가 다음
착수 지점이고 `#67` 4c-A의 프로세스 관리자가 이미 그 생산자다"라고 적었는데, 코드를 읽으니
아니다. 세 겹으로 틀렸다.

1. **인벤토리는 이미 있다.** 4c-B가 만들었다 — `AgentObservation::{Running,Failed}`를 워커가 매
   beat 권위 있는 전체 집합으로 보내고, `PgStore::apply_agent_observations`가 그 목록에 없는 것의
   관측을 지운다. 없는 것은 인벤토리가 아니라 **orphan을 표현할 자리**다.
2. **4c-A는 orphan의 생산자가 아니다.** `AgentProcessManager`의 `procs`는
   `Mutex<HashMap<AgentId, AgentProc>>`이고 디스크에 아무것도 남기지 않는다. 자식은
   `.kill_on_drop(true)`로 띄우는데 이것은 부모가 SIGKILL당하면 발동하지 않으므로, 재기동한
   워커는 **빈 맵**으로 시작해 물려받은 프로세스를 이름조차 부를 수 없다. 워커가 아는 집합은
   "이 인스턴스가 띄운 것"이지 "이 호스트에서 도는 것"이 아니다. orphan은 저장소에서도 조용히
   사라진다 — `apply_agent_observations` 2단계의 `WHERE id = $1 AND worker_id = $2`가 0행을
   갱신하고 오류를 내지 않는다.
3. **이름을 불러도 죽일 수단이 없다.** provisioning.md 유예 표가 이미 적어 둔 대로
   `fleet-worker`는 `#![forbid(unsafe_code)]`이고 `libc::kill`은 unsafe, `nix`는 의존성에 없어
   지금 SIGTERM을 보내지 못한다. 소비자가 종료 경로 없는 재조정기뿐인 orphan 보고는 "채울 방법이
   없는 칸"이며, 그것을 만들지 않는다는 것이 이 저장소의 원칙이다.

그래서 process inventory는 **구현 과제가 아니라 선행 설계 결정**이다(`nix`를 들일 것인가, 아니면
포트 탐색과 ACP shutdown으로 갈 것인가). 다음 착수 지점은 `#70` 게이트 5의 배선으로 바꾼다 —
수단(`WorkerTransport::ping`)이 이미 트레이트에 있고 소유도 2026-09-06에 단방향으로 확정했다.

### 게이트

문서만 바꾼 변경이지만 게이트는 축약하지 않는다: `rustc --version`(CI 고정 버전과 대조),
`RUSTFLAGS="-D warnings"`, `cargo fmt --all -- --check`, clippy 두 피처 세트
(`--all-targets -- -D warnings`), 그리고 각 세트마다 `cargo build -p fleet-cli` 후 `DATABASE_URL`을
주입한 `cargo test --workspace --no-fail-fast -- --test-threads=1`. **판정은 커밋 메시지에 적는다** —
여기에 적으면 그 문장이 트리를 바꾸고, 바뀐 트리는 인용한 게이트가 판정한 적 없는 트리가 된다.

**이 게이트가 증명하는 것은 거의 없다.** 이번 변경에는 코드가 한 줄도 없으므로 게이트는 "문서
정정이 코드를 깨지 않았다"만 말한다. 위의 세 정정 — 인벤토리가 이미 있다, `procs`가 인메모리다,
SIGTERM 수단이 없다 — 은 전부 `crates/fleet-core/src/agent.rs`,
`crates/fleet-store/src/postgres.rs:3427`, `crates/fleet-worker/src/agent_process.rs`를 읽은 것이
근거이고, **워커를 실제로 죽여 고아를 만들어 본 것이 아니다.** 표기 규칙의 "위반은 ④ 하나뿐"도
`docs/` 전수 grep의 결과이지 렌더링된 문서를 사람이 읽어 확인한 것이 아니다.

메타데이터는 건드리지 않았다. `last_verified`는 "그 날짜에 코드와 대조했다"는 뜻인데 이번에 한
것은 표기 정정이지 문서 전체의 재검증이 아니다. 다만 `roadmap.md`의 `last_verified`가 2026-09-01인
채로 2026-09-02 항목이 들어가 있는 것은 이번 이전부터의 미결이며, 여기서 함께 고치지 않았다.

### 남은 것

`#70` 게이트 5의 배선이 다음이다. 착수 전에 확인할 것 넷: `ping`의 실제 시그니처와 `acp` 피처
게이트 여부(`fleet-dashboard`에는 `acp`·`mtls`가 아예 없다), `SelectionError`의 기존 variant가
probe 도입으로 의미가 바뀌는지, `on_demand` 제외 필터를 **걷어내는 것**이 이 게이트의 합격
조건이라는 점(배선만 넣고 제외를 남기면 동작이 변하지 않아 닫힌 것처럼 읽히는 게이트가 된다),
그리고 probe를 전수로 돌릴지 순위 결정 후 선택된 하나만 돌릴지. 마지막 것은 dispatch 지연이
가장 느린 워커를 따라가느냐(전수) 대 후보를 타고 내려갈 수 있느냐(선택)의 맞바꿈이고, 정본의
표현이 "dispatch 직전"이므로 후자에 기운다.

게이트 5를 닫으면 `observability-and-reconciliation.md`의 게이트 표 5행과 **그 위 문단이 함께**
틀려진다 — 그 문단은 "닫힌 것은 게이트 1 하나, 부분은 게이트 5 하나"라고 세고 있다. 직전 항목이
기록한 대로 표의 무결성은 기계가 재지만 요약문의 산술은 아무도 재지 않으므로, 같은 편집에서
두 자리를 함께 고친다.

---

**병합 이음매 (2026-09-11).** 여기부터는 `claude/pam-phase2-task5` 브랜치가 같은 기간에
따로 쌓은 항목이다. 위 블록과 **날짜가 겹치고 되돌아간다** — 두 갈래가 병렬로 진행됐고
이 로그는 append-only라 둘을 시간순으로 섞지 않고 이어 붙였다. 두 블록이 같은 게이트를
다르게 판정한 자리는 [관측성 정본](architecture/observability-and-reconciliation.md)의
게이트 표가 정본이며, 그 표는 병합 시점에 양쪽 사실을 반영해 다시 썼다.

---
## 2026-09-02 — 조용히 죽인 것은 일어나지 않은 것과 구분되지 않는다

`#70` 게이트 3의 orphan 절반을 닫았다. 시작은 정본의 차단 사유를 그대로 믿지 않은 것이었다.

정본은 2026-08-28부터 이렇게 적어 왔다 — "비교할 process inventory가 없어 orphan을 지목할 수
없다." 그런데 `#67` 4c-B가 그 뒤에 도착해 있었고, Worker는 이미 매 beat `AgentObservation`을
올려 보내고 있었다. 그래서 실제로 없는 것이 무엇인지를 코드에서 다시 읽었더니, 없는 것은
inventory가 아니라 **보고할 어휘**였다.

`AgentProcessManager::reconcile`의 2단계는 명령 목록에 없는 프로세스를 종료한다. 그 자리의 주석은
부재가 세 가지를 뭉친다고 이미 적고 있었다 — 다른 Worker로의 재배정, 미배치, 확인이 끝난 회수.
**앞의 둘이면 같은 Agent가 두 곳에서 돌던 창이 실제로 열렸다는 뜻이다.** 그것은 `#67` 게이트 ②의
술어와 `036`의 트리거가 통째로 막으려는 상태이고, `036`은 그 방어가 뚫리는 경로까지 이미 적어
두었다("운영자가 틀렸다면 중복 실행이 그대로 발생한다. 판단의 주체를 사람으로 옮긴 것이지 위험을
없앤 것이 아니다"). 그런데 그 창이 열렸는지를 **관측할 방법이 없었다**. 종료는 `warn!` 한 줄로
끝났고, 오케스트레이터에는 아무것도 도달하지 않았다.

### 관측 목록에 섞을 수 없는 이유가 둘이고, 둘 다 이미 적혀 있었다

처음에는 `AgentObservation`에 variant를 하나 더하는 것이 자연스러워 보였다. 두 가지가 막는다.

첫째, `apply_agent_observations`는 목록에 **없는** Agent의 관측을 지운다. orphan의 id가 그 목록에
들어가면 "말했다"로 세어져, 정작 지워야 할 관측이 살아남는다. 둘째, `036`의
`agents_observation_requires_placement`가 `worker_id IS NOT NULL OR observed_status IS NULL`을
강제한다 — orphan은 정의상 이 Worker에 배정돼 있지 않으므로 **DB가 이미 그 컬럼에 적는 것을
금지하고 있다.** 즉 남길 자리는 `agents`의 컬럼이 아니라 감사 로그다.

그래서 `agent_orphans`는 `agent_observations`의 형제 필드로 따로 간다. 세 필드의 `serde` 모양이
서로 다른 것도 우연이 아니다 — `agent_acks`는 비면 생략(대역폭), `agent_observations`는
`Option`(생략과 `[]`의 **의미가 다르다**), `agent_orphans`는 다시 생략(사건 목록이라 둘이 같다).
직렬화 시험 하나가 세 단정을 나란히 들고 있는 이유가 그것이다.

### 재조정 루프가 원리적으로 볼 수 없는 고아

부재만 다루면 절반이다. Worker가 SIGKILL·전원 차단·패닉으로 죽으면 `kill_on_drop`의 Drop이 돌지
않아 자식이 살아남는데, 새 incarnation의 `procs`는 **메모리라 비어 있다.** 그 자식은 `reconcile`이
원리적으로 도달하지 못하는 곳에 있고, 유일한 흔적은 엉뚱한 자리에 나타난다 — 계속 쥐고 있는 포트
때문에 `free_port`의 bind가 실패해 새 Agent가 `NoFreePort`로 거절된다. **원인과 증상이 이만큼
떨어져 있으면 운영자는 포트 범위를 넓히는 잘못된 처방을 찾는다.**

그래서 근거를 메모리 밖에 둔다. `spawn`이 workspace에 `.fleet-agent.json`을 남기고, 기동 시
`sweep_stale_incarnation()`이 그것을 읽어 pid의 생사를 확인한다. sweep은 첫 beat **전에** 돈다 —
순서가 반대면 첫 `reconcile`이 옛 프로세스가 살아 있는 채로 같은 Agent의 **두 번째** 프로세스를
띄우고, 포트가 다르니 아무것도 그것을 막지 않는다.

### pid 재사용을 이름이 아니라 시작 시각으로 거른 이유

기록의 pid를 그냥 죽이면 이 코드는 **남의 프로세스를 죽이는 코드**가 된다. Linux 기본 `pid_max`가
32768이라 순환은 드물지 않다.

처음 고른 방어는 프로세스 이름 대조였고, **그것은 시험을 쓰다가 무너졌다.** 이 저장소의
`fake_grok`은 `#!/bin/sh` + `exec sleep 300`이다. `exec` 때문에 기동 직후와 정착 후의 이름이
다르고, 대조 시점이 그 경계에 걸리면 판정이 갈린다 — 즉 이름 대조는 래퍼 스크립트를 쓰는
모든 진입점에서 경합한다. 실제 `grok` 바이너리에서 지금 그 문제가 없더라도, 시험이 먼저
그것을 드러냈다는 사실 자체가 신호였다.

`sysinfo`의 `start_time()`은 그 경합이 없다. `exec`은 프로세스를 **새로 만들지 않으므로** 시작
시각이 보존된다. 기록의 시각과 관측된 시각이 5초 안이면 우리 것으로 본다. 두 값이 서로 다른
시계(우리 벽시계와 커널)에서 오므로 초 단위 반올림에서 1~2초는 정상적으로 벌어지고, pid가
재사용될 만큼 시간이 흐른 경우는 이 창을 한참 벗어난다. 시계가 고장난 호스트에서는 `now_unix()`가
0을 주고, 그 값은 대조를 통과하지 못한다 — **고장은 고아를 놓치는 쪽으로 틀리지 죽이는 쪽으로
틀리지 않는다.**

### 입양하지 않는 것이 아니라 입양할 수 없다

살아 있는 자식을 새 incarnation이 넘겨받을 방법이 없다. `Child` 핸들은 그것을 만든 프로세스에만
있고 새 incarnation에는 pid뿐이라 `try_wait()`이 성립하지 않는다 — 0단계가 없는 매니저가 된다.
그래서 종료가 유일하게 일관된 처리이며, 대가는 정직하게 적는다: **멀쩡히 돌던 Agent가 Worker
재기동 뒤 한 번 죽는다.**

### 감사 상한을 둔 이유

이 목록은 Worker가 통제한다. 인증된 Worker라는 것과 그 Worker가 보내는 값이 정직하다는 것은
다르다. 상한이 없으면 장악된 Worker 하나의 beat이 감사 테이블에 임의 개수의 줄을 쓰고, 감사는
다른 사건을 조회하는 자리이기도 하므로 그 오염은 **이 기능이 지키려는 것 자체를 무디게 만든다.**
32는 프로세스 상한(기본 4)보다 넉넉히 크고, 정직한 Worker가 넘길 경로가 없으므로 초과 자체가
이상 신호다 — 그래서 조용히 자르지 않고 경고를 남긴다.

감사 기록 실패는 `error!`로 남기되 heartbeat을 실패시키지 않는다. 위의 ACK·관측 블록과 같은
선택이지만 **이유가 반대 방향이다**: 저 둘은 다음 beat이 복구하므로 잃는 것이 적어서 그렇게 하고,
이쪽은 일회성이라 잃는 것이 큰데도 그렇게 한다 — 500을 내면 감사 저장 장애 하나가 Worker를
`Offline`으로 떨어뜨려 훨씬 큰 것을 잃기 때문이다.

### 검증 한계

**메트릭을 더하지 않았다.** 정본의 신호표는 Worker/Agent 축에 "orphan/unknown 수"를 요구하지만,
`/metrics`는 전량 store 파생이라 사건 카운터를 두려면 in-process 상태가 필요하고 그 값은 재기동마다
리셋된다. 그것은 표가 요구하는 "현재 orphan 수"(상태)와 다른 것(누적 사건)을 말하게 된다. 감사
줄에서 집계하는 쪽이 맞지만 그 조회 비용은 따로 판단할 문제라 이번 범위 밖에 둔다.

**실제 SIGKILL을 재현하지 않았다.** `a_process_left_by_a_previous_incarnation_is_swept`는 워커를
죽이는 대신 살아 있는 `sleep` 하나와 그것을 가리키는 기록을 심는다. 재현해야 하는 것이 "워커가
죽는 방식"이 아니라 그 결과로 남는 상태이기 때문인데, 그 결과 **`kill_on_drop`이 SIGKILL에서 실제로
돌지 않는다는 전제 자체는 시험되지 않았다** — 그것은 tokio의 문서에서 읽은 것이지 여기서 확인한
것이 아니다.

**`Unplaced`가 실제 재배정에서 나오는 것도 보지 않았다.** 시험은 명령 목록에서 Agent를 빼는 것으로
부재를 만든다. 오케스트레이터가 `assign_agent_worker`로 옮겼을 때 `list_agent_commands`의 술어가
정말 그 Agent를 옛 Worker의 목록에서 빼는지는 저쪽 시험이 덮는 부분이고, 두 시험을 잇는 E2E는 없다.

**OpenAPI 스키마는 손대지 않았다.** `crates/fleet-api/src/openapi.yaml`의 `HeartbeatRequest`는
`agent_acks`·`agent_observations`·`cpu_usage`·`ram_usage`를 이미 담고 있지 않다 — `#67` 4b와 4c-B가
필드를 더할 때도 그 파일을 갱신하지 않았다. 즉 이 스키마는 이번 변경 **이전부터** 실제 요청 모양과
어긋나 있고, 내 필드 하나만 더하면 "이 파일은 최신"이라는 잘못된 신호를 준다. 전부 맞추는 것은
이 작업의 범위를 넘으므로 어긋남을 여기에 기록만 하고 남긴다.

**sweep의 비용은 측정하지 않았다.** `ProcessesToUpdate::Some`으로 기록에 적힌 pid만 새로고침해
전체 열거를 피했지만(agent.md §3.3 기록 2가 `sysinfo`의 다른 API에서 겪은 종류의 비용을 이 경로에
들이지 않기 위해서다), 그 선택이 실제로 싸다는 것을 실측으로 확인하지는 않았다. 기동 시 한 번만
도는 경로라 beat 타이머에는 영향이 없다.

---

## 2026-09-02 — ingest: `#67` 게이트 ⑥ self-fencing 절반, 그리고 게이트의 술어를 다시 세운 일

**착수 전에 게이트가 무엇을 사는지부터 틀렸다는 것을 알았다.** 정본의 게이트 ⑥은
"partition 중 Worker self-fencing과 stale process cleanup E2E"이고, 그 근거 칸은 줄곧 중복 프로세스를
막는 것으로 읽혀 왔다. 그런데 그 중복은 하루 전(2026-09-01) 게이트 ②가 이미 닫았다 —
`assign_agent_worker`의 UPDATE에 `observed_status` 술어가 들어가, `running`으로 보고된 Agent는 다른
Worker로 옮겨지지 않는다. 즉 **막으려던 것이 이미 다른 곳에서 막혀 있었다.**

남은 노출을 코드로 다시 찾았다. `apply_agent_observations`의 호출자는 `fleet-api`의 heartbeat 핸들러
단 하나이고 관측을 노후화하는 경로가 없다. 그래서 파티션된 워커의 Agent는 `observed_status`가
`running`으로 굳고, 게이트 ②의 술어가 그 Agent를 **영구히** 붙잡는다. 한편 워커 쪽은
`warn!("heartbeat failed — will retry next interval")` 한 줄만 찍고 프로세스를 무기한 유지한다. 즉
남은 문제는 안전이 아니라 **liveness**였다 — 감독 없는 실행이 끝나지 않고, 그것을 끝낼 유일한 열쇠를
그 워커가 쥐고 있다.

**그 재판정이 설계를 바꿨다.** 유예가 안전을 사는 값이 아니므로 짧게 잡을 이유가 없다. 짧으면 잠깐의
네트워크 흔들림이 진행 중인 작업의 손실이 되고, 그 대가로 얻는 안전은 이미 지불돼 있어 없다. 기본값을
300초로 두고 오케스트레이터의 Offline 판정(`HealthConfig` 기본 15초 × 3 = 45초)보다 한참 길게 잡은
근거가 이것이다. 정본이 "이 게이트를 시간 기반 펜싱으로 닫지 않는다"고 적어 온 것도 다시 읽었다 — 그
거부는 시간 기반 펜싱이 **중복 방지 수단으로** 부적절하다는 뜻이었고, 중복 방지가 다른 곳에서 닫힌
지금 이 결정에는 걸리지 않는다.

**보고 경로를 `AgentOrphan`에 얹지 않았다.** 그 타입의 독스트링이 "오케스트레이터가 배정하지 않은
프로세스"라고 스스로를 정의하는데, self-fence된 Agent는 배정이 유효한 채로 멈춘 것이다. 세 번째
`reason`으로 넣으면 타입의 이름과 정의가 곧바로 거짓이 된다. 형제 필드 `agent_fenced`와
`agent.self_fenced` 감사로 분리했고, 상한 상수는 이제 두 목록을 덮으므로 `MAX_ORPHANS_PER_BEAT`에서
`MAX_AGENT_EVENTS_PER_BEAT`로 고쳤다.

**상태는 쓰지 않고 이유만 쓴다.** 게이트 ③의 orphan은 쓸 자리가 아예 없어서(미배치라 `036`이 금지한다)
감사만 남겼지만, 이쪽은 이유가 다르다 — 같은 beat의 `agent_observations`가 권위 있는 전체 집합이고 멈춘
Agent는 거기서 빠지므로 `apply_agent_observations`가 그 관측을 이미 지운다. 여기서 또 쓰면 같은 사실을
두 경로가 쓰게 되고 둘이 어긋날 수 있다.

**루프의 하한이 설정 검증만으로는 부족했다.** `worker.agent_fence_after_secs > heartbeat_interval_secs`를
파싱 시점에 강제하지만, 실제 beat 주기는 등록 응답이 올릴 수 있다(`runner.rs`의 `max`). 그 경우 유예가
주기에 추월당해 heartbeat 한 번의 실패가 곧바로 펜싱이 된다. 루프 안에서 `max(interval + 1s)`로 최소 한
번의 재시도를 남겼다.

**검증 한계 — E2E가 아니다.** 게이트가 요구하는 것은 실제 파티션을 만들고 회복까지 잇는 시험인데, 여기서
닫은 것은 워커 단위다: 루프를 닿지 않는 주소로 돌려 유예 전후의 거동을 갈랐고(변이 시험으로 "항상 펜싱"과
"절대 펜싱하지 않음" 양쪽에서 붉어지는 것을 확인했다), 재연결 뒤 관측이 지워져 재배정이 풀리는지는
저쪽 시험이 덮는 부분이라 두 시험을 잇는 E2E는 없다. 그래서 게이트는 **부분**이다.

**영영 돌아오지 않는 Worker는 그대로 남는다.** 펜싱은 프로세스를 멈추지만 관측을 지우는 것은 그 워커의
다음 heartbeat뿐이다. 워커가 복귀하지 않으면 `observed_status = 'running'`이 굳은 채 재배정이 영구히
막히고, 그때 남는 신호는 `HealthChecker`의 `Offline` 전이뿐이다. 푸는 경로는 운영자 주도의 관측
무효화인데, 그것은 게이트 ②가 세운 술어를 사람이 우회하는 문이라 같은 증분에서 결정할 일이 아니다.

**감사 줄은 늦게 도착한다.** 사건은 단절 중에 일어나고 보고는 재연결 뒤에야 실리므로 `created_at`은
종료 시각이 아니라 재연결 시각이다. 종료 시점이 필요하면 `detail.unreachable_secs`를 빼서 거슬러
올라가야 하며, 이 비대칭을 감사 상수의 독스트링에도 적어 두었다.
...
## 2026-09-03 — ingest: heartbeat 루프의 죽음을 보이게 만들었다 (`#67` 게이트 ⑥ 후속)

**어제 문서에 적은 단서를 없애는 작업이다.** 게이트 ⑥의 self-fencing을 넣으면서 036의
트레이드오프에 운영 규칙 하나가 붙었다 — "`last_seen`에서 `agent_fence_after_secs`가 지난 뒤에
Worker를 삭제하라". 그 근거는 "유예를 넘겼으면 프로세스는 이미 없다"인데, 반례를 같은 날 적어
두었다: 펜싱은 heartbeat 루프 **안에서만** 일어나므로 루프가 죽으면 유예를 아무리 넘겨도 Agent는
계속 돈다.

**그 죽음은 아무 신호도 내지 않았다.** 루프는 `runner.rs`에서 `tokio::spawn`으로 띄우고 종료
경로에서만 join한다. 즉 `run()`의 4단계는 `wait_for_signal().await` 하나였고, 루프가 어떻게 끝나든
운영자가 워커를 내릴 때까지 그 사실이 드러나지 않는다. 그동안 오케스트레이터는 45초 뒤
`Offline`으로 판정하지만, 그것은 "heartbeat이 없다"만 말하고 Agent 프로세스가 계속 돌고 있다는
것은 말하지 못한다.

**닫는 방법은 기다리는 대상을 하나 늘리는 것이었다.** `await_shutdown`이 OS 신호와 루프의
`JoinHandle`을 `select!`로 함께 기다린다. 루프는 shutdown 신호에만 끝나도록 쓰여 있으므로 신호보다
먼저 끝났다면 그것은 결함이다 — panic, 조용한 조기 반환, 취소 셋 다. 결함이면 같은 종료 경로를
타서 `shutdown_all()`이 Agent를 유예 5초로 정리하고, `run()`이 `Err`를 반환한다. `main`이 그것을
`ExitCode::FAILURE`로 옮기므로 systemd 같은 감시자가 재기동하며, 재기동의
`sweep_stale_incarnation()`(`#70` 게이트 3)이 혹시 살아남은 자식을 걷는다. **`Ok(())`로 끝내면
안 되는 이유가 여기 있다** — 감시자가 정상 종료로 읽고 재기동하지 않으면 그 호스트는 heartbeat
없는 워커로 남는다.

**시험은 hang이 아니라 실패로 끝나야 한다.** 이 결함의 증상이 "영원히 기다린다"이므로 단정을
타임아웃 안에 둔다 — `timeout(5s, await_shutdown(..)).await.expect(..)`. 세 건이고 셋째가 앞의 둘을
떠받친다: 살아 있는 루프는 깨우지 않는다는 단정이 없으면 "무엇이든 결함으로 만든다"는 구현도
앞의 둘을 통과하는데, 그 구현은 정상 워커를 기동 직후 죽인다. 변이 시험으로 갈랐다 — 감시를
지워 변경 이전 거동으로 되돌리면 앞의 둘만 붉어지고 셋째는 초록이다.

**덮지 못하는 것을 좁혀 적는다.** `JoinHandle`이 **끝나지 않는** 정지는 이 감시도 깨우지 못한다 —
`await`에서의 데드락, 블로킹 호출에 굶은 런타임, `SIGSTOP`. 그것까지 잡으려면 루프가 매 회차
올리는 진행 카운터를 감시해야 하고, 그 감시자는 `tokio::spawn`한 태스크일 수 없다(굶은 런타임에서
같이 굶는다) — **OS 스레드**여야 한다. 그리고 그 스레드가 취할 수 있는 조치는 프로세스를 죽이는
...
## 2026-09-04 — fix: heartbeat이 grok 바이너리 하나에 무기한 묶이던 경로

**어제 항목이 "고치지 않고 기록만 한다"고 적은 것을 고쳤다.** `detect_grok_version`은
`grok --version`을 타임아웃 없는 블로킹 `std::process::Command::output()`으로 불렀고, 그 호출은
`heartbeat_once` 안에 있다. `get_or_init`이라 한 번만 돌지만 그 한 번이 답하지 않는 바이너리에
걸리면 heartbeat 루프가 무기한 멈춘다.

**멈추는 것이 heartbeat만이 아니라는 점이 이 결함의 크기를 정한다.** 펜싱(`#67` 게이트 ⑥)은 그
루프 안에서 일어나므로 루프가 멈추면 유예를 아무리 넘겨도 Agent 프로세스가 멈추지 않는다. 그리고
그 정지는 `JoinHandle`이 끝나지 않는 종류라 어제 넣은 `await_shutdown` 감시도 잡지 못한다 —
정본에 "덮지 못한다"고 적어 둔 바로 그 부류의 **실재하는** 경로였다.

**추론이 아니라 밟았다.** 게이트 ⑥의 E2E를 만들면서 가짜 grok을 인자 무시 `sleep 300` 스크립트로
줬는데, 증상이 "5초 `tokio::time::sleep`이 300초 걸린다"였다. stdio 상속·런타임 굶주림·`MemStore`
데드락을 차례로 의심했고 전부 빗나갔다. 답을 준 것은 `gdb -p <pid> -batch -ex "thread apply all bt"`
한 장이었다 — `heartbeat_once → detect_version → Command::output → wait4`. **간헐로 보이는 정지에는
스택을 뜨는 것이 가장 짧은 길이다.**

`tokio::process` + `GROK_VERSION_TIMEOUT`(5초) + `kill_on_drop`으로 바꿨다. **`spawn_blocking`으로
감싸는 대안은 쓰지 않았다** — 그쪽은 취소가 되지 않아 타임아웃이 걸려도 블로킹 스레드가 자식과 함께
영원히 남는다. 즉 런타임은 살지만 스레드는 새는 절반짜리 수정이 된다. 캐시는 초기화가 async가 되면서
`OnceLock`에서 `tokio::sync::OnceCell`로 옮겼다.

**실패를 `None`으로 캐시하는 것은 유지했다.** 버전은 메타데이터이므로 이것 때문에 heartbeat을
실패시키면 느린 바이너리 하나가 워커를 `Offline`으로 떨어뜨려 훨씬 큰 것을 잃는다. 매 beat 재시도하지
않는 것도 의도적이다 — 답하지 않는 바이너리를 상대로 재시도하면 beat마다 타임아웃 하나씩을 쌓는다.
대가는 그 Worker의 버전이 재기동 전까지 미상으로 남는 것이고, 그렇게 적었다.

**시험의 단정을 시간이 아니라 다른 태스크의 진행으로 세웠다.** 블로킹의 증상은 "이 함수가 늦다"가
아니라 "그동안 아무도 못 돈다"이기 때문이다. 단일 스레드 런타임에서 `detect`를 백그라운드로 띄우고
200ms `sleep`이 제때 끝나는지를 본다. 세 번째 시험(정상 바이너리의 버전이 그대로 읽힘)이 앞의 둘을
떠받친다 — 없으면 "항상 `None`"인 구현도 통과하고, 그러면 모든 Worker의 버전이 미상이 된다.

**변이의 결과가 실패가 아니라 행(hang)이었다.** 예전 블로킹 구현으로 되돌리자 시험이 붉어지는 대신
400초 타임아웃에 걸려 종료됐다. 그것이 이 결함의 증상 그대로이므로 옳은 신호다 — 다만 CI에서 이런
회귀는 "실패"가 아니라 "잡이 멈춤"으로 나타난다는 뜻이기도 하다.

---

## 2026-09-04 — ingest: 게이트 ⑥의 나머지 절반(stale process cleanup) E2E, 게이트 닫힘

**게이트 ⑥이 요구한 것은 self-fencing "과" stale process cleanup의 E2E였다.** 앞의 절반은 오늘
앞선 항목이 덮었고, 이 항목이 뒤의 절반을 덮어 게이트를 닫는다. 둘은 프로세스를 멈춘다는 결과만
같고 실패의 종류가 다르다 — 저쪽은 워커가 살아서 스스로 멈추는 경로이고, 이쪽은 워커가 멈출 기회조차
없이 죽은 뒤의 잔해다.

**죽음을 `std::mem::forget`으로 만든다.** 매니저를 잊으면 `Child`가 Drop되지 않아 `kill_on_drop`이
발동하지 않는다 — SIGKILL당한 워커가 남기는 상태와 정확히 같다. 프로세스를 실제로 죽이는 대신 이
방법을 쓴 이유는 **시험 프로세스 자신이 그 워커이기 때문**이다. 그다음 같은 workspace 위에 새
매니저를 만들고 heartbeat 루프를 돌리면, 첫 beat **전에** 도는 sweep이 디스크의
`.fleet-agent.json`을 근거로 그 자식을 찾아 죽이고 `agent.orphan_terminated`
(`reason = stale_incarnation`)로 신고한다.

**단정 하나를 처음에 잘못 세웠다.** "판정이 끝났으니 기록 파일이 사라져야 한다"고 썼는데 실패했다.
sweep이 옛 기록을 지우는 것은 맞지만, **같은 beat의 `reconcile`이 같은 Agent를 다시 띄우며 새 기록을
쓴다** — 배정과 `desired=running`이 그대로이므로 그것이 옳은 동작이다. 물어야 할 것은 파일의 유무가
아니라 **누구의 것인가**였고, pid가 그것을 가른다. 파일 존재를 근거로 삼는 단정은 이렇게 조용히
틀린다.

**검증 한계**: sweep이 그 자식을 실제로 죽였는지는 이 E2E가 다시 보지 않는다. 죽은 자식은
부모(시험 프로세스)가 거두지 않아 **좀비**로 남고, 그 상태에서 `kill -0`은 성공하므로 이식성 있는
생사 판정이 되지 못한다. 그 단정은 `Child` 핸들을 쥔 `fleet-worker::agent_process`의 단위 시험이
맡는다 — E2E가 맡은 것은 그 사건이 오케스트레이터까지 닿는 **경로**다.

첫 beat 전의 sweep을 지운 트리에서 이 시험만 붉어지고 self-fencing 시험은 초록인 것을 확인했다.
둘이 서로 다른 실패를 덮는다는 증거다. 3회 연속 19.6초로 통과한다.

**게이트 ⑥을 닫는다.** 남는 것은 게이트가 묻는 것이 아니라 시험의 한계다 — Store가 `MemStore`라는
것(그 대가로 `DATABASE_URL` 없이 모든 잡에서 돈다), 그리고 `JoinHandle`이 끝나지 않는 정지는 이
시험도 `await_shutdown`도 잡지 못한다는 것(그 부류의 실재하던 경로 하나는 오늘 앞선 항목이 닫았다).

---

## 2026-09-05 — ingest: `#67` 게이트 ④의 둘, 그리고 내가 또 한 번 잘못 짚은 것

**먼저 정정.** 어제 마지막에 "게이트 ③·⑦의 차단 사유가 낡았을 가능성이 크다 — `OutcomeUnknown`이
이미 `lease.rs`와 `task.rs`에 있다"고 적었다. **틀렸다.** 두 곳 다 주석이고, `task.rs`는 내가 한 것과
똑같은 오독을 미리 경고까지 해 두었다 — *"이 variant가 있다고 `OutcomeUnknown` 상태 기계가 구현된
것이 아니다."* 식별자 grep이 코드와 산문을 구분하지 못한다는 것을 이 세션에서 두 번째로 밟았다.
게이트 ③·⑦의 차단 사유는 그대로 유효하다.

**그 옆에서 진짜로 낡지 않은 사유를 찾았다.** 게이트 ④의 근거 칸은 선행을 말하지 않는다 — "라이브
실행은 lease 인수인계만 관찰했다. 수동 승격 절차, Worker 재연결, reconciliation은 어느 것도 실행하지
않았다"이다. 즉 막힌 것이 아니라 **하지 않은 것**이었고, control lease API는 두 Store에 모두 있으며
`LeaseManager`·`LeaseObserver`·dispatcher 게이팅도 완비돼 있다.

`fleet-scheduler/tests/lease_failover.rs`가 셋 중 둘을 잇는다. 같은 Store 위에 실제 `LeaseManager`
둘을 띄우고, Primary를 `shutdown()`으로 내린 뒤(정본이 적은 두 만료 원인 중 "graceful release" 쪽)
Standby가 승격되는 것과, **그때서야** 밀린 `Pending` Task가 재조정되는 것을 본다. 가짜
리스(`LeaseObserver::with_status`)를 쓰지 않은 이유는 게이트가 묻는 것이 바로 그 승격이기 때문이다.

**변이 시험이 설계를 하나 드러냈고, 그 과정이 이 항목의 값이다.** 처음 만든 시험은 리스 게이팅을
지워도 통과했다. 두 번을 헛짚었다 — `Reconciler`의 게이트만 지웠을 때도, `Dispatcher`의 게이트만
지웠을 때도 초록이었다. 이유는 둘이 **직렬**이기 때문이다: 한쪽이 사라져도 다른 쪽이 막으므로
관측 가능한 동작이 변하지 않는다. 둘을 함께 지우자 승격 전에 Task가 `Failed`로 떨어지며 시험이
붉어졌다. **그것은 시험의 결함이 아니라 다중 방어의 증거다** — 동작을 보는 시험은 한 겹이 사라져도
초록인 것이 옳고, 이 시험이 고정하는 것은 특정 `if` 문이 아니라 "리스 없이는 밀린 일을 건드리지
않는다"는 성질이다.

**시간 상수 하나가 판별력을 통째로 없애고 있었다.** 재시도 상한이 기본값 20이면 소진까지 최소
6초가 걸리는데, "승격 전에는 아무 일도 없다"는 확인이 그 6초 안에 들어간다. 그래서 양쪽 게이팅을
다 지운 트리에서도 그 시점에는 아직 `Pending`이라 단정이 통과했다. 1로 낮추자 리스가 없을 때
곧바로 상태가 바뀌어 구멍이 사라졌다. **"변이가 시험을 죽이지 못한다"의 원인이 단정이 아니라
시험이 들여다보는 시각일 수 있다.**

**남은 셋째**: Worker 재연결. 워커 프로세스와 API 표면과 스케줄러(리스·Reconciler)를 한 시험에
합치는 조립이 필요하고, 그 골격의 절반(`fleet-worker`의 `self_fencing_e2e`)은 이미 있다. 이
증분 밖으로 둔다.

---

## 2026-09-05 — lint: `#70` 게이트 5의 사유는 유효했다, 그리고 그 자리의 함정

**재판정 결과: 사유가 낡지 않았다.** 게이트 ⑤의 남은 절반("probe 성공 후 dispatch 허용")은
`selector.rs`가 적은 대로 여전히 막혀 있다. 그 파일의 근거는 지금 읽어도 정확하다 — on_demand
워커는 heartbeat을 보내지 않고 `HealthChecker`가 그것을 알기에 강등하지 않으며, 등록 시점의
`build_worker`는 liveness_mode와 무관하게 `Online`을 쓰므로, 한 번 등록되면 프로세스가 죽어도
영구히 `Online`으로 남는다.

**그런데 그 자리에 함정이 있다.** `WorkerTransport::ping`은 이름도 반환형(`Duration`)도 probe처럼
보이는데 **왕복하지 않는다**. `AcpTransport`의 구현은 supervisor가 든 연결 상태를 확인하고
`Duration::from_millis(1)` 상수를 돌려준다. 트레이트 독스트링은 "워커 연결을 테스트 (헬스체크용)"
라고만 적혀 있어 그 사실이 드러나지 않았다. 이 게이트를 닫으려는 사람은 정확히 이 함수를 집게
되고, 연결만 서 있고 응답하지 않는 워커를 "probe 통과"로 읽게 된다.

트레이트와 구현 양쪽 독스트링에 파급을 적고, `acp_transport_integration`의
`ping_registered_worker_ok`에 단정을 더했다 — 두 번의 ping이 **같은 값**이고 그 값이 상수라는 것.
누군가 진짜 왕복을 넣으면 이 시험이 붉어지며 "그 변경이 게이트 ⑤의 probe를 만든 것인지"를
의도적으로 판단하게 만든다.

**처음 쓴 시험은 잘못된 방식이었다.** `include_str!`로 소스 텍스트에 상수가 있는지를 봤는데,
변이를 넣어도 통과했다 — 같은 문자열이 그 파일에 세 번 나오기 때문이다. 소스를 스캔하는 단정은
관측 가능한 성질을 보지 않으므로 애초에 시험이 아니다. 연결된 워커가 있는 통합 시험으로 옮겨
"두 번의 호출이 같은 값"이라는 **관측**으로 바꾸자 변이가 정확히 죽였다.

**쓸 수 있는 더 약한 신호는 있다.** `is_connected`는 supervisor가 유지하는 실제 연결 상태를
반영하므로 "등록되면 영구히 `Online`"보다는 낫다. 그러나 그것도 응답이 아니라 연결이므로 이
게이트가 요구하는 확인은 되지 못한다. `ping`과 `is_connected` 둘 다 프로덕션 코드에서 부르는 곳이
없다 — 시험에서만 쓴다.

**이 세션에서 차단 사유를 네 번 재판정했고 결과가 갈렸다**: `#70` 게이트 3은 틀렸고(고쳤다),
`#67` 게이트 ④는 선행이 아니라 미실행이었으며(닫았다), `#67` 게이트 ③·⑦과 이번 `#70` 게이트 5는
사유가 유효했다. 재판정 자체가 값이지 "낡았을 것"이라는 기대가 값이 아니다.

---

## 2026-09-05 — ingest: `#67` 게이트 ④의 셋째(Worker 재연결), 게이트 닫힘

**셋 중 남은 하나는 스케줄러만으로 만들 수 없었다.** 워커의 heartbeat이 닿는 곳은 리스를 쥔
스케줄러가 아니라 **API 표면**이기 때문이다. 그래서 `control_plane_failover_e2e`는 셋을 한 프로세스
안에 세운다 — 같은 Store 위의 API 인스턴스 둘, 그 사이를 오가는 리스, 그리고 진짜 워커.

**워커의 설정을 한 글자도 바꾸지 않는 것이 이 시험의 요점 하나다.** 워커와 오케스트레이터 사이의
TCP 링크에서 **뒤쪽만** A에서 B로 바꾼다. VIP나 로드밸런서 뒤에서 인스턴스가 교체되는 실제 모양이고,
덕분에 "워커가 재연결했다"가 설정 변경이 아니라 `last_seen`이 다시 오르는 **관측**이 된다. 워커에게
새 주소를 알려 주는 방식이었다면 그 단정은 시험이 스스로 만든 사실을 확인하는 것에 가까웠을 것이다.

`self_fencing_e2e`에서 만든 링크가 여기서 두 번째 쓸모를 냈다. 그때는 끊는 용도였고 여기서는 **옮기는**
용도다 — 연결 태스크를 `JoinSet`으로 소유하는 구조가 둘 다를 가능하게 한다(`abort`가 이미 맺어진
연결까지 끊으므로, `reqwest`의 연결 풀이 죽은 인스턴스를 계속 쓰지 않는다).

변이 둘로 갈랐다. 복구된 링크를 죽은 A로 되돌리면 재연결 단정이 붉어지고, 리스 게이팅 양쪽을 지우면
"승격 전에는 밀린 일을 건드리지 않는다"는 단정이 붉어진다. 후자가 **양쪽을 함께** 지워야 한다는 것은
어제 `lease_failover`에서 확인한 그대로다 — 두 겹이 직렬이라 한쪽만으로는 동작이 변하지 않는다.

**검증 한계**: 밀린 Task를 그 워커에게 실제로 dispatch하지는 않는다. 그러려면 ACP transport를 워커의
종단에 물려야 하는데, 이 시험이 묻는 것은 "새 Primary가 밀린 일을 집는가"이지 그것이 어디로 가는가가
아니다. dispatch 경로는 `fleet-scheduler`의 `dispatch_e2e`가 따로 덮는다.

`fleet-worker`에 `fleet-scheduler`를 dev-dep으로 더했다. 순환이 아니다 — `fleet-scheduler`는
`fleet-worker`를 의존하지 않는다. 이로써 `fleet-worker`의 시험은 API·스케줄러·워커 셋을 모두
조립할 수 있는 유일한 자리가 됐다.

**`#67` 게이트 현황**: ①·④·⑥ 닫힘, ②·③·⑤·⑦ 부분, 미착수 없음.

---

## 2026-09-06 — ingest: `#67` 게이트 ⑤의 생산자, 그리고 "읽는 쪽이 없으면 만들지 않는다"

**재판정: 사유는 유효했다.** `control_plane_lease`에 버전 컬럼이 없고 `ControlLease`에도 필드가
없었다. 정본이 적은 그대로다.

**그런데 생산자만 넣는 것에도 함정이 있었다.** 조사해 보니 이 저장소에는 `get_control_lease`의
**프로덕션 호출자가 0건**이고, CLI·API·MCP 어디에도 리스를 보여주는 표면이 없다. 그 상태로
컬럼만 더하면 아무도 읽지 않는 값에 마이그레이션 하나를 얹는 것이 된다 — 정본이 "생산자 없는
술어는 죽은 검사"라고 적은 것의 거울상이다. 그래서 읽는 쪽을 같은 증분에 넣었다: 마이그레이션
가드의 거절 메시지가 이제 막고 있는 인스턴스의 **버전**을 함께 말한다. 운영자가 그 값을 가장
알고 싶은 순간이 정확히 거기다 — 기동이 막혔고 무엇을 세울지 정해야 할 때.

**거절은 넣지 않았다. 정책이 없기 때문이다.** 버전이 다르다고 막는 술어는 롤링 업그레이드를
통째로 거절한다 — 새 바이너리는 정의상 옛 Primary가 리스를 쥔 동안 올라온다. 무엇을
"비호환"으로 볼지는 이 프로젝트가 선언한 적이 없고, 그 계약 없이 술어를 고르면 코드가 운영
절차를 임의로 정하는 셈이다.

**설계에서 두 번 갈린 자리.**

1. 값을 Store가 만들지 않고 **호출자가 넘긴다**. 여기서 `CARGO_PKG_VERSION`을 읽으면 그것은
   `fleet-store`의 버전이지 리스를 쥔 바이너리의 것이 아니다. 그리고 `LeaseManager::new`의
   **필수 인자**로 뒀다 — 빌더였다면 부르지 않은 호출부가 조용히 `NULL`을 남겨, 컬럼은 있는데
   프로덕션에서는 늘 비는 상태가 된다.
2. 가드의 읽기가 컬럼 이름을 **지목하지 않는다**. 그 함수는 마이그레이션 **전에** 도므로 037이
   없는 DB를 상대할 수 있고, 컬럼명을 그대로 쓰면 스키마를 지키려는 코드가 옛 스키마에서
   `UndefinedColumn`으로 깨진다. `to_jsonb(행) ->> 'binary_version'`은 없는 키에 NULL을 주므로
   추가 왕복 없이 두 상태를 견딘다.

**그 두 번째 판단이 곧바로 값을 했다.** `migration_lease_guard`의 시험 둘이 실제로 깨졌다 —
부분 마이그레이션 DB에서 `acquire_control_lease`와 `get_control_lease`가 새 컬럼을 이름으로
지목하기 때문이다. 프로덕션은 리스 획득이 항상 migration 뒤라 이 상황을 만들지 않지만, 그
시험들의 전제가 정확히 "바이너리가 DB보다 앞선 상태"다. 시험이 store API를 우회해 행을 직접
넣도록 고쳤다. **가드의 읽기까지 컬럼명으로 썼다면 가드 자신이 같은 이유로 깨졌을 것이다.**

**아직 도달하지 않는 경로를 적어 둔다.** 거절 메시지에 버전이 실리려면 037이 적용된 DB에서
동시에 마이그레이션이 밀려 있어야 하는데, 037이 마지막인 지금 그 조합은 자연스럽게 생기지
않는다. 038이 생기는 순간부터 실제 운영에서 나타난다. 시험은 `_sqlx_migrations`의 마지막
기록만 지워 그 조합을 만들어 읽는 쪽을 지금 고정한다 — 스키마는 037을 지난 채로 두고 sqlx에게만
밀린 것으로 보이게 하는 방법이다.

**게이트는 부분 그대로다.** 강제는 호환 계약이 정해질 때 넣는다.

## 2026-09-06 — `#67` 게이트 ⑥: 루프가 **돌지 않게 된** 경우의 상한 (OS 스레드 워치독)

게이트 ⑥의 self-fencing과 그 뒤에 붙인 `await_shutdown`은 둘 다 **루프가 돈다**는 것을
전제한다. 전자는 루프 안의 코드고, 후자는 `JoinHandle`의 완료를 신호로 쓴다. 그래서 두 부류가
어느 쪽에도 걸리지 않았고, 정본은 그 사실과 함께 처방까지 이미 적어 두고 있었다 — 진행
카운터를 OS 스레드가 감시하고 `.fleet-agent.json`의 pid를 직접 죽여야 한다고.

**그 처방을 그대로 만들었다.** `crates/fleet-worker/src/watchdog.rs`. heartbeat 루프가 매
바퀴의 **시작**에 `ProgressBeacon`을 올리고, 전용 OS 스레드가 그것을 본다.
`worker.watchdog_stall_after_secs`(기본 600초, `0`이면 끔) 동안 움직이지 않으면 기록된 pid를
동기적으로 죽이고 종료 코드 `70`으로 나간다.

**이 부류가 가설이 아니라는 증거는 이틀 전에 나왔다.** `detect_grok_version`이 타임아웃 없는
`std::process::Command::output()`으로 그 루프 안에서 300초를 블로킹했다. 그 300초 동안 펜싱
기한은 한 번도 평가되지 않았고, `JoinHandle`은 끝나지 않았으므로 `await_shutdown`도 깨어나지
않았다. 그 개별 결함은 고쳤지만 부류는 남아 있었다.

### 설계에서 선택지가 없었던 자리 셋

1. **감시자는 OS 스레드다.** `tokio::spawn`한 감시자는 굶은 런타임에서 같이 굶는다 — 잡아야
   할 바로 그 경우에 잡지 못한다.
2. **죽이는 것은 pid다.** `fence_all()`은 `async`이고, 지금 확정한 사실이 "그 런타임이
   진행하지 않는다"이므로 그것을 부르는 것은 정의상 걸린다. 그리고 `exit`은 소멸자를 돌리지
   않아 `kill_on_drop`이 자식을 거두지 않으므로, 죽이지 않고 나가면 Agent는 그대로 남는다 —
   그것을 회수하는 것은 감독자가 재기동해 줄 때뿐이고, **감독 없는 실행의 상한이 감독자의
   존재에 기대면 그것은 상한이 아니다.**
3. **비콘은 한 바퀴의 시작을 센다, heartbeat 성공이 아니라.** 성공을 세면 단절된 Worker가 곧
   죽는데 그것은 self-fencing이 훨씬 부드럽게 다루는 경우다(프로세스만 멈추고 Worker는 살아
   재연결을 기다린다). 즉 **파티션은 워치독의 대상이 아니다.**

### 프로세스 전체의 정지를 굶주림과 가르는 법

`SIGSTOP`·디버거 attach·VM 정지는 워치독 스레드까지 함께 멈춘다. 그때 비콘이 멈춰 있는 것은
정상이고, 재개 직후 그것을 굶주림으로 읽으면 워치독이 멀쩡한 Worker를 죽인다.

두 경우는 **워치독 자신의 잠을 재서** 갈린다. 굶주림이면 워치독은 제때 깨어나므로 자기 잠은
정확하고 비콘만 멈춰 있다. 프로세스 정지면 자기 잠도 함께 크게 늘어난다. 그래서 경과를 별도의
`Instant`가 아니라 **잰 잠의 누적**으로 들고 있는다 — 두 시간 원천이 있으면 그 둘이 어긋나는
자리가 곧 오판의 자리다.

판정이 애매하면 죽이지 않는 쪽으로 접는다. 놓친 굶주림은 오케스트레이터의 `Offline` 판정이
여전히 덮지만, 잘못 죽인 Worker를 되돌리는 것은 아무도 하지 않는다.

### 해제를 루프의 책임으로 옮겼다

처음에는 `runner.rs`가 shutdown 전파 직전에 `disarm()`을 부르게 썼다. 그 배치는 맞지만
**혼자서는 약하다** — beat을 올리는 곳과 그만 올린다고 알리는 곳이 갈라지면, 어긋난 채로도
컴파일과 테스트가 전부 통과한 뒤 종료가 느린 배포에서만 드러난다(워치독이 정상 종료 중인
프로세스를 죽이는 형태로). 그래서 루프 자신이 shutdown으로 반환할 때 해제하게 하고, 그것을
시험으로 고정했다. `runner.rs`의 해제는 남겨 둔다 — 그쪽이 덮는 것은 루프가 **이미 사라진**
경우(패닉·취소)라 겹치지 않는다.

### 시험과 변이

- 판정 로직 7건 — 잠을 주입해 결정론적으로 돌린다. 그중 둘이 프로세스 정지 구분을 고정하고,
  하나(`a_freeze_does_not_hide_a_stall_that_follows_it`)는 그 초기화가 **영구적이지 않다**는
  것을 고정한다. 이 단정이 없으면 "정지로 읽으면 항상 초기화"가 워치독을 통째로 무력화하는
  구현도 통과한다.
- pid 종료와 재사용 방어 4건 — 진짜 자식으로 돌린다. `#[tokio::test]`가 **아닌** 것이 첫
  시험의 요점이다. 하나는 두 기록 읽기 구현(async/sync)이 같은 트리에서 갈리지 않는 것을
  지킨다.
- 진짜 루프 2건 — 나머지가 전부 비콘을 손으로 올리므로, 루프가 비콘을 올린다는 것과 정상
  종료가 해제한다는 것을 지키는 것은 이 둘뿐이다.

변이 4건 전부 죽였다: 비콘을 올리지 않음 → `an_unreachable_control_plane_still_counts_as_loop_progress`,
해제하지 않음 → `a_graceful_shutdown_disarms_the_watchdog`, 정지를 굶주림으로 읽음 → 위 두 건,
pid 재사용을 거르지 않음 → `the_watchdog_spares_a_reused_pid_just_as_the_sweep_does`.

### 검증 한계

**실제로 굶은 tokio 런타임이 종료 코드 `70`을 내는 것은 시험하지 않았다.** 런타임을 의도적으로
굶히려면 그러기 위한 코드가 프로덕션에 있어야 하고, 그 비용이 이 한 걸음의 값보다 크다고
봤다. 근거는 기전이지 재현이 아니다 — agent.md §3.3의 두 기록과 같은 종류의 정당화다.

`SIGSTOP`은 덮지 않는다. 그것은 결함이 아니라 위 구분의 **의도된 반대편**이다.

## 2026-09-06 — `#70` 게이트 5: `ping`이 되지 못한 그것을 만들었다 (ACP probe)

어제 이 게이트를 재판정하면서 **함정 하나를 못박아 뒀다** — `WorkerTransport::ping`은
이름도 반환형(`Duration`)도 probe처럼 보이지만 왕복하지 않는다. 그때 남긴 문장이
"이 게이트를 닫으려면 응답을 실제로 받는 수단을 새로 만들어야 한다"였고, 이 증분이
그것이다: `WorkerTransport::probe`.

### 메서드 선택 — 부작용 없음이 유일한 기준이었다

`AcpTransport`가 `session/list`를 왕복시킨다. ACP에서 client가 agent에게 보낼 수 있는
요청 중 상태를 바꾸지 않는 것이 사실상 이것뿐이다. `session/new`는 세션을 만들어
용량을 소모하고 실패한 probe가 정리되지 않은 세션을 남긴다. `initialize`는 스펙상
연결당 한 번이라 두 번째 호출의 의미가 구현에 달려 있다. probe는 dispatch **전에**
불릴 것이므로 이 조건은 편의가 아니라 요건이다.

### 판정은 "답이 왔는가"이지 "성공했는가"가 아니다

grok이 `session/list`를 구현하지 않으면 `-32601`이 돌아온다. **그래도 probe는
성공이다** — 저쪽이 요청을 받아 해석하고 답을 만들어 보냈다는 뜻이므로, 살아
있다는 증거로는 정상 응답과 같은 값이다. 여기서 실패로 접으면 이 함수는 liveness가
아니라 "grok의 이 버전이 이 메서드를 아는가"를 재는 것이 되고, 멀쩡한 워커가 영구히
배정 대상에서 빠진다.

`ProbeOutcome`을 `Duration` 하나로 두지 않은 이유도 여기에 있다. 그러면 반환형이
`ping`과 같아지고, 그 순간 상수를 돌려주는 구현이 **타입에 대해 정직해진다** —
`ping`이 정확히 그렇게 되어 있다. `round_trip`은 재지 않으면 채울 수 없고,
`answered_with_error`는 구현자에게 거절 경로를 어떻게 분류했는지 밝히기를 요구한다.

### 처음 구현이 틀렸고, 그것을 잡은 것은 그 목적으로 쓴 시험이었다

Agent의 거절과 연결의 죽음이 SDK에서 **같은 `Err`로 도착한다.** 처음에는 SDK가
제공하는 `is_incoming_transport_closed`로 충분할 줄 알았는데, 그것은 EOF **뒤에**
보낸 요청에만 붙는 표식이었다. 이미 날아간 요청 중에 소켓이 닫히면 다른 모양이
온다:

```
거절:      code -32601, data None
연결 사망: code -32603, data "response to `session/list` never received: oneshot canceled"
```

`a_connection_that_dies_during_the_probe_is_not_an_answer`가 첫 실행에서
`Ok(ProbeOutcome { answered_with_error: true })`로 붉어졌다 — **죽은 연결이 "거절했으니
살아 있다"로 보고된 것이고, 이 probe가 막으려는 바로 그 일이다.** 시험을 먼저 쓰지
않았으면 이 결함은 실제 운영에서 dispatch 실패로 나타났을 것이다.

두 신호를 AND로 묶어 갈랐다.

1. **연결 상태**(구조적, 주 신호). 그 오류는 연결 teardown이 응답 oneshot을 취소하며
   만들어지고, 이 transport의 supervisor는 **같은 teardown에서** 상태를 쓴다 — 순서는
   우연이 아니라 인과다. 실측으로 확인했다(오류 도착 시점의 상태 = `Disconnected`).
2. **SDK의 문구 표식**(텍스트, 보조). 벤더된 SDK가 문구를 바꾸면 조용히 무력해지므로
   주 신호로 쓰지 않지만, 상태 전이가 늦는 경합에서는 이쪽이 먼저 잡는다.

### 다중 방어의 각 겹은 따로 시험하지 않으면 조용히 죽는다

변이로 확인한 것이 이 증분에서 가장 옮길 만한 것이다. 두 신호를 **함께** 지우면
통합 시험이 붉어지지만, **하나만** 지우면 어느 쪽이든 초록이었다 — 남은 하나가
혼자 통과시키기 때문이다. 게이트 ⑥의 두 리스 게이트가 직렬이라 변이가 죽지 않았던
것과 같은 모양이다. 다른 점은 그때는 그것이 옳은 동작이었고 여기서는 **각 겹이
독립적으로 살아 있어야 한다**는 것이다.

그래서 텍스트 쪽을 `error_is_an_agent_answer`로 떼어 내 단위 시험으로 따로
못박았다. 그 뒤 변이 4건이 전부 죽는다: 판정 전체 제거, 문구 표식 제거, EOF 표식
제거, 그리고 처음의 실제 결함(오류를 전부 답으로 읽기). 연결 상태 신호만은 여전히
조합에서만 덮인다 — 그쪽은 구조적이라 조용히 드리프트하지 않는 편이고, 비대칭을
이렇게 두는 것이 맞다고 봤다.

### 게이트는 아직 부분이다

probe는 생겼지만 `WorkerSelector`/`Dispatcher`에 배선되지 않았고, on_demand 워커는
그대로 후보에서 빠진다. 남은 것은 "probe 성공 후 dispatch 허용"이며, 거기서 정해야
할 것은 **probe 결과의 신선도를 어디에 들 것인가**다 — 저장소 컬럼(재기동에도 남고
운영자에게 보이지만 마이그레이션과 쓰기가 붙는다)이냐, dispatch 직전 1회 호출
(상태가 없고 항상 신선하지만 dispatch 지연이 붙는다)이냐. 이 증분은 그 결정을
하지 않는다.

## 2026-09-06 — `#70` 게이트 5: probe를 배선해 게이트를 닫는다

앞 증분이 `WorkerTransport::probe`를 만들었고 "selector/dispatcher에 배선되지 않았다"로
끝났다. 그 배선이다.

`WorkerSelector`가 `on_demand` 워커를 통째로 빼던 필터를 걷고, 대신 **고른 워커**가 그
모드면 dispatch 직전에 probe로 확인한다. `unchecked → probe → dispatch`가 완성됐다.

### 결정 1 — 신선도를 어디에도 들지 않는다

앞 증분의 마지막 문단이 남긴 선택지는 저장소 컬럼이냐 dispatch 직전 1회 호출이냐였다.
후자를 골랐다. 저장소에 두면 "그 결과가 얼마나 오래 유효한가"라는 **두 번째 파라미터**가
생기고, 그 값이 틀리면 죽은 워커가 유효기간 동안 살아 있는 것으로 남는다. 매번 새로 묻는
쪽에는 그 파라미터가 아예 없다.

같은 이유로 별도의 `Unchecked` 워커 상태도 만들지 않았다. 예전에 만들지 않은 이유("probe가
없으면 빠져나올 방법이 없다")는 이제 사라졌지만, 그 자리를 위 이유가 대신한다.

### 결정 2 — probe는 필터가 아니라 **결승전**에서 돈다

자연스러워 보이는 자리는 예전 liveness 필터가 있던 1.5단계다. 거기가 아니다. 그 자리의
다른 검사(라벨·모델·credential·회로·용량)는 전부 로컬 연산인데 **probe는 왕복**이다.
1.5단계에 두면 어차피 뒤 필터에서 떨어질 워커까지 전부 왕복시키고, dispatch 루프가 최대
1000건의 Pending을 도는 구조에서 그 비용이 선형으로 쌓인다.

그래서 순서를 나눴다 — 값싼 필터가 후보를 좁힌 뒤 승자에게만 건다. 결과는 같으면서 왕복
횟수가 "후보 수"가 아니라 "떨어진 on_demand 승자 수 + 1"이 된다.

**이 주장을 시험이 지키게 만드는 데 신경을 썼다.** 응답만 흉내 내는 double로는 한 줄도
검증되지 않는다 — 모든 후보를 왕복시키는 구현도 똑같이 초록이다. 그래서 `ProbeSpy`가
호출을 **순서대로 기록**하고, 두 시험이 그것을 단정한다: 라벨에서 떨어질 워커는 아예
물어보지 않는다(`probed().is_empty()`), periodic 워커는 후보에 있어도 물어보지 않는다
(`probed() == [on_demand 하나]`).

### 결정 3 — 폴백 규칙이 갈래마다 다르다

- **least-loaded**: 정렬 순서대로 내려가며 침묵한 on_demand 워커를 건너뛴다. 지목이 없는
  선택이므로 어느 워커에서 도는지는 계약이 아니고, 죽은 워커 하나가 살아 있는 워커로 갈
  Task를 막을 이유가 없다.
- **`server_hint` / `agent_id`**: 폴백하지 않는다. 지목한 워커가 응답하지 않는다는 사실을
  다른 워커로 조용히 덮으면 지목의 의미가 사라진다. hint는 이미 "없거나 사용 불가면 에러,
  폴백 안 함"이 계약이므로 여기서만 예외를 두면 규칙이 갈린다.

### 뒤집힌 단정 하나를 기록한다

`on_demand_exclusion_is_reported_before_label_mismatch`는 on_demand 워커가 라벨도 만족하지
않을 때 `AllUnprobed`를 요구했다. 근거는 "liveness는 적합성 이전의 문제"였다. **probe가
결승전으로 내려간 지금 그 자리에는 liveness 사실이 없다** — 물어보지 않았기 때문이다. 그
상태에서 `AllUnprobed`를 보고하는 것은 확인하지 않은 것을 확인했다고 말하는 것이므로,
실제로 확인한 사실(`NoMatchingLabels`)을 보고하는 쪽이 정확하다. 시험 이름도
`a_label_mismatch_is_reported_without_probing_at_all`로 바꿔 그 뒤집힘 자체를 단정한다.

**게이트를 옮기면 게이트가 지키던 단정도 함께 옮겨야 한다**는 것이 여기서 배운 것이다.
다섯 시험이 붉어졌을 때 그것들을 "고치는" 것과 "새 계약으로 다시 쓰는" 것은 다르다 —
전자는 통과시키는 것이고, 후자는 무엇이 바뀌었고 무엇이 그대로여야 하는지를 매번
결정하는 것이다. 다섯 중 넷은 안전 절반이 그대로 살아남았고(침묵하면 배정하지 않는다,
지목은 폴백하지 않는다), 하나는 위처럼 뒤집혔다.

### 시험과 변이

selector 32건(기존 5건을 새 계약으로 다시 쓰고 3건 추가). 새로 연 문과 그것이 지키는
문을 **쌍으로** 뒀다 — 응답하면 배정된다/침묵하면 배정되지 않는다, 응답하면 least-loaded가
이긴다/침묵하면 바쁜 쪽으로 폴백한다, 응답하면 hint가 닿는다/침묵하면 대체되지 않는다.
한쪽만으로는 부족하다: 앞엣것만 있으면 probe를 무시하는 구현이 통과하고, 뒤엣것만
있으면 예전처럼 통째로 빼는 구현이 통과한다.

변이 6건 전부 죽었다: probe 생략, 예전처럼 통째 제외, periodic까지 왕복, least-loaded
폴백 제거, hint 폴백, agent 지목 probe 생략.

### 남은 한계

probe 성공과 dispatch 사이는 원자적이지 않다. 모듈 최상단이 용량 카운트에 대해 적어 온
한계와 같은 종류이고 최종 방어선도 같다 — transport의 거절이다. `PROBE_TIMEOUT`(5초)만큼의
지연이 on_demand 워커로 가는 첫 dispatch에 얹히는 것도 그대로 남는다.

## 2026-09-06 — `#70` 게이트 4: 차단 사유가 순환이었다 (도구 호출 관측 신설)

effect ledger를 착수하려고 정본을 읽다가 **차단 사유가 순환이라는 것을 발견했다.** 게이트 ④의
칸에는 "effect ledger가 코드에 존재하지 않는다"라고 적혀 있었는데, 그것은 "원장이 없어서
원장을 못 만든다"는 말이다.

실제로 막힌 자리는 훨씬 앞이었다. `tool_effect`·`tool_id`를 코드에서 찾으면 `crates/` 안에
**0건**이고 전부 벤더 SDK다. 이유는 구조적이다 — 도구는 Worker의 grok 프로세스 **안에서**
돌고, 오케스트레이터는 ACP로 `session/prompt`만 보낸다. 즉 오케스트레이터는 개별 도구 호출을
볼 위치에 있지 않다.

**그런데 볼 수 있었다.** ACP는 `session/update`로 `ToolCall`·`ToolCallUpdate`를 알려준다.
`acp_transport.rs`의 알림 핸들러가 그것을 `_ => None`으로 **전부 버리고 있었다** — Task가 실제로
무엇을 했는지에 대한 오케스트레이터의 유일한 증거가 매번 폐기되고 있었던 것이다.

그래서 원장의 상태 기계를 짓는 대신 그 증거를 붙잡았다. **없는 것을 미리 만들지 않되, 있는데
버리고 있던 것은 줍는다.**

### 무엇을 만들지 않았는가를 먼저 적는다

여덟 상태(`Planned`/`Started`/`Applied`/`NoEffect`/`Compensating`/`Compensated`/`Unknown`/
`CompensationFailed`), provider idempotency key, external receipt — **하나도 만들지 않았다.**
이것은 원장이 아니라 원장의 **입력**이고, 게이트 ④는 여전히 차단이다.

특히 `status = Completed`는 **effect 증거가 아니다.** 정본이 "process output이나 모델의 '완료'
서술은 effect 증거가 아니다"라고 적은 그것이며, 증명은 provider receipt에서 오는데 그것은
여전히 없다. 이 구분을 코드 독스트링과 정본 양쪽에 박아 뒀다 — 나중에 이 이벤트를 보고
"부작용이 적용됐다"로 읽는 것이 가장 그럴듯한 오독이기 때문이다.

### 무엇을 버리는가가 이 증분의 안전 결정이다

ACP의 `ToolCall`은 `title`·`raw_input`·`raw_output`·`content`·`locations`를 함께 준다. 전부
버린다. 관측성 정본의 금지 목록(prompt·사용자 입력·repository URL·raw provider payload)에
걸리고, `title`은 "Read /home/user/.ssh/id_rsa"처럼 경로와 인자를 사람이 읽는 문장으로 담는다.
**이 관측은 durable 이벤트 로그로 가므로 한 번 새면 지우는 경로가 없다.**

남기는 넷은 그 위험이 없다: `tool_call_id`(세션 범위 불투명 식별자), `kind`·`status`(닫힌 어휘
10종·4종), `name`(도구의 프로그램적 이름 — 사용자 입력이 아니라 설정이며, 원장 명세가 이름을
댄 `tool_id`에 대응하는 유일한 필드다).

**이 단정을 필드 목록이 아니라 직렬화 결과로 시험한다.** mock이 `title`과 `rawInput`에 심은
`SECRET-PATH`가 도착한 이벤트에 없는지 본다. 필드를 세는 시험은 나중에 누가 필드를 더할 때
같이 고쳐지면서 조용히 통과하지만, 비밀을 찾는 시험은 그때 붉어진다.

### 곁가지 둘

- `name`을 읽으려고 SDK의 `unstable_tool_call_name`을 켰다. 선례가 정확히 하나 있다
  (`unstable_end_turn_token_usage`). unstable 표면은 예고 없이 바뀌지만 그 위험은 조용하지
  않다 — 필드가 사라지면 매핑 함수가 컴파일에서 깨진다.
- `fleet-cli`의 `event_type_str`이 `FleetEvent::event_type()`의 match를 한 벌 더 들고 있었다.
  새 variant를 더하며 **두 곳을 고치게 되어** 위임으로 바꿨다. 한쪽만 고치면 CLI가 조용히 옛
  이름을 찍는 구조였고, 저장된 `event_type` 칼럼과 CLI 표시가 이제 정의상 같다.

### 설계에서 갈린 자리 하나

갱신 알림(`ToolCallUpdate`)은 **바뀐 필드만** 싣는다. 빠진 필드를 앞 알림에서 이어 붙이려면
세션별 도구 호출 표를 들어야 하는데, 그것은 transport 계층이 소유할 상태가 아니다. 대신 가장
덜 아는 값으로 둔다 — `kind`는 `Other`, `status`는 `Pending`. **`status`를 `Pending`으로 두는
것이 중요하다**: 모르는 상태를 종료로 접으면 끝나지 않은 호출이 끝난 것으로 기록된다. 이어
붙이는 것은 `tool_call_id`가 있으니 읽는 쪽에서 하면 된다.

`worker_id`를 모르면 이벤트를 만들지 않고 버린다. 그 값은 이 관측을 나중에 process inventory와
대조할 때의 **축**이고, 틀린 축은 없는 축보다 나쁘다.

리스를 잃은 상태에서도 이 관측은 계속 기록한다. `Output`과 같은 부류라는 것이 일차 근거지만,
이쪽이 더 강하다 — 리스를 잃은 창은 새 소유자가 재조정해야 하는 구간이고, 그 재조정이 물을
"이 Task가 무엇을 했는가"의 증거가 정확히 이 이벤트다.

### 시험과 변이

9건. 변이 8건 전부 죽었다: 예전처럼 알림 버리기, `title`을 관측에 싣기, 빠진 `status`를
`Completed`로 지어내기, 빠진 `kind`를 `Read`로 지어내기, `Read`를 `Edit`으로 옮기기, 저장하지
않기, 워커를 모를 때 지어내기, fenced에서 막기.

**변이 하나가 필터에 걸리지 않아 살아남은 것처럼 보였다.** `-- --test-threads=1 tool`로 돌렸는데
`an_update_that_omits_fields_claims_the_least`에는 "tool"이 없어 실행되지 않았고, 결과는
`0 failed`였다. §4.3이 적어 온 "판정처럼 보이는 값" 계열의 또 다른 얼굴이다 — 여기서는 필터가
대상을 비웠다. **필터로 좁힌 변이 검증은 그 필터가 무엇을 골랐는지 확인하기 전까지 판정이
아니다.** 이름으로 다시 돌려 죽는 것을 확인했다.

### 남은 한계

호출당 이벤트 수에 상한이 없다. 폭주하는 Agent는 이벤트 로그를 채울 수 있다. 형제 경로
(`append_output`)가 같은 노출을 갖고 상한이 없으므로 여기만 다르게 두지 않았다 — 상한을 두려면
두 곳을 함께 정해야 하고, 그것은 이 증분의 질문이 아니다.

## 2026-09-06 — 게이트 ⑤ 상태 표시 정정, 게이트 ⑥ 차단 사유 재판정

### 내가 낸 오류 하나를 먼저 적는다

게이트 ⑤를 닫으면서 본문에는 "배선까지 마쳐 게이트를 닫는다"라고 적고 **상태 칸은
`부분`으로 남겨 뒀다.** 표를 스캔하는 사람이 먼저 보는 것이 그 칸이므로, 본문과 칸이
어긋나면 표는 틀린 것을 말한다. `닫힘`으로 고쳤다.

이 형태는 §4.3이 모아 온 "판정처럼 보이는 값" 계열과 같다 — 거기서는 값 옆의
메타데이터가 값을 무효화했고, 여기서는 요약 칸이 본문과 갈라졌다.

### 게이트 ⑥ — 사유가 낡았지만, 낡은 방향이 반대였다

오늘 두 번(게이트 ③·④) 차단 사유가 낡은 것을 발견해서 ⑥도 의심했다. 사유는
"`lease_generation`·`fencing_token`·`control_epoch`와 effect 경로가 필드로 존재하지
않는다"였고, 실측하니 두 군데가 부정확했다.

- `control_epoch`는 **실재한다** — 코드 48곳, 마이그레이션 셋(026·031·035).
- effect 경로도 같은 날 절반이 생겼다(`FleetEvent::TaskToolCall`).
- 진짜 없는 것은 `lease_generation`·`fencing_token` 둘뿐(각 0건).

**그런데 그것이 게이트를 열지 않는다.** 여기서 멈추고 `AuditEvent`에 `control_epoch`를
더했다면 게이트 ④에서 방금 피한 함정에 그대로 빠졌을 것이다 — 실측하니 감사 기록을
내는 곳은 35군데인데 전부 `fleet-api`(8)·`fleet-dashboard`(25)·`fleet-core`(2)이고
**`fleet-scheduler`는 0건이다.** 그리고 `fleet-api` 전체에 `lease` 언급이 0건이라 그
계층은 epoch를 실을 수단 자체가 없다. 즉 컬럼을 더하면 **35개 emitter 전부에서 항상
NULL**이다.

그래서 사유를 이렇게 다시 적었다: 이 게이트의 선행은 필드가 아니라 **제어면 결정의
감사 경로**다. dispatch·재배정·펜싱·breaker 전이는 지금 감사에 아무것도 남기지 않고,
그것을 남기게 만드는 일은 `FleetState`가 **이미 쥔** `control_fence()`를 감사와 잇는
것이다.

### 오늘 세 번 반복된 것

차단 사유 셋을 재판정했고 셋 다 부정확했지만 **틀린 방향이 매번 달랐다.**

- 게이트 ④: 순환이었다("원장이 없어서 원장을 못 만든다"). 진짜 차단은 더 앞이었고,
  뚫을 수 있었다.
- 게이트 ⑤: 유효했지만 함정이 하나 숨어 있었다(`ping`이 probe가 아니다).
- 게이트 ⑥: 사유가 낡았는데 **낡은 쪽이 유리한 방향이 아니었다** — 없다던 것이 있었고,
  그럼에도 게이트는 열리지 않았다. 오히려 사유를 그대로 믿었다면 "필드만 더하면 된다"는
  잘못된 다음 걸음으로 갔을 것이다.

**재판정의 값은 게이트가 열리는 데 있지 않다.** 셋 중 하나만 열렸고, 나머지 둘에서 얻은
것은 "다음 사람이 잘못된 걸음을 밟지 않게 하는 정확한 사유"다. 게이트 ⑥이 특히 그렇다 —
낡은 사유는 쉬운 작업(필드 추가)을 가리키고 있었고, 그것은 항상 NULL인 컬럼으로
끝났을 것이다.

### 이 증분은 문서만 바꾼다

`.rs` 파일을 하나도 건드리지 않았으므로 §4.3의 컴파일·시험 게이트는 이 diff에 대해
말할 것이 없다. 게이트를 돌리지 않았다는 사실과 그 근거를 여기 적어 둔다 —
`git diff --name-only`가 `.md` 둘뿐이다.

## 2026-09-06 — `#70` 게이트 6 선행: 제어면 결정이 처음으로 감사에 남는다

바로 앞 증분에서 이 게이트의 차단 사유를 다시 적었다 — 막힌 자리는 필드가 아니라
**제어면 결정의 감사 경로**이고, `fleet-scheduler`는 감사 기록을 하나도 내지 않는다는
것. 그 선행을 만들었다.

### 무엇을 남기고 무엇을 남기지 않는가

`FleetState::audit_control`이 생기면서 scheduler가 처음으로 감사를 낸다. 남기는 것은
셋이며 **전부 거절·유기 계열이다.**

- `control.dispatch_refused` — 리스가 없어 스스로 물러섰다.
- `control.write_fenced` — 리스가 있다고 **믿고** 쓰러 갔다가 저장소가 세대 불일치로
  막았다.
- `control.outcome_abandoned` — 리스를 잃어 워커 결과의 제어 처리를 건너뛰었다.

앞의 둘을 한 이름으로 접지 않은 것이 이 증분의 판단이다. **둘은 다른 사실이다**:
전자는 이 인스턴스가 스스로 물러선 것이고, 후자는 **다른 누군가가 그 사이에 세대를
가져갔다**는 직접 증거다. alert 표가 "둘 이상의 owner 관측"에 대해 요구하는 증거는
후자뿐이며, 접으면 조사가 갈리지 않는다.

**성공한 dispatch는 남기지 않는다.** 이미 `FleetEvent::TaskDispatched`가 있고
`tasks.dispatch_control_epoch`가 세대를 들고 있어 중복이다. 경로 재구성에 성공 기록도
필요하다는 반론이 가능하지만, 그것을 감사에 또 넣으면 같은 사실이 두 원장에 갈라져
어느 쪽이 정본인지 모호해진다.

### `None`이 두 가지 정상을 뜻한다

`audit_log.control_epoch`(마이그레이션 040)는 nullable이고, NULL이 두 경우다.

1. **운영자 행위** — 기존 35개 emitter. 사람이 한 일에는 제어면 세대가 없다.
2. **리스를 갖지 못한 채 내린 거절** — `LeaseStatus::Fenced`/`Stopped`에는 epoch 자체가
   없다.

2번에서 "세대가 없었다"는 것 자체가 증거이므로 `detail.lease_status`가 그것을 나른다.
`NOT NULL`이나 기본값을 걸었다면 그 행들이 **없던 세대를 가진 것처럼** 보였을 것이다 —
특히 `0`으로 접으면 "0번 세대"라는 없는 사실이 생긴다.

### 행위자는 cluster가 아니라 instance다

`LeaseObserver`에 `instance_id`를 새로 노출했다(`LeaseManagerHandle`을 거쳐 흐른다).
alert 표가 요구하는 것이 "둘 이상의 owner 관측"인데 **같은 cluster의 두 인스턴스는
cluster_id로 구분되지 않는다.** 두 인스턴스가 같은 DB에 감사를 쓰므로 이 필드가 없으면
행을 갈라 볼 수단이 없다.

`LeaseObserver::with_status`의 인자를 늘려 4개 호출부를 고쳤다. 선택 인자로 두지 않은
이유는 `LeaseManager::binary_version`과 같다 — 정체성을 조용히 빠뜨릴 수 있게 두면
그 실수는 컴파일도 되고 테스트도 통과한 뒤 조사가 필요한 순간에만 드러난다.

### 리스 없는 배포는 아무것도 남기지 않는다

단일 인스턴스 배포에는 세대도 경합도 없으므로 남길 사실이 없다. `lease_allows_control()`이
그 경우 항상 `true`인 것과 같은 이유다. **이 단정을 시험으로 고정한 이유**: 없으면
"모든 경로에서 무조건 감사한다"는 구현이 통과하고, 그러면 리스를 쓰지 않는 배포의 감사
로그가 의미 없는 행으로 찬다.

### 시험이 갈라져야 했던 자리

scheduler 단위 시험은 `MemStore`로 돈다. 그래서 그것들이 전부 초록이어도 **컬럼이 실제로
있는지, INSERT/SELECT가 그것을 싣는지는 한 줄도 검증되지 않는다** — 그 결함의 증상은
조용하다(조회는 성공하고 값만 비어 있다). `fleet-store`의 왕복 시험 2건이 그 구간을
맡는다. 그중 하나는 세대가 **있는** 기록과 **없는** 기록을 함께 넣는다 — 하나만 넣으면
"항상 7을 돌려주는" 구현도 통과한다.

변이 5건 전부 죽었다: 감사를 아예 남기지 않기, epoch를 싣지 않기, 거절과 fenced를 한
이름으로 접기, PgStore가 컬럼을 쓰지 않기, 리스 없는 배포에서도 감사하기.

### 게이트는 여전히 차단이다

`lease_generation`·`fencing_token`은 아직 없다. 여기서 닫힌 것은 "제어면 결정이 감사에
아무것도 남기지 않는다"는 **선행 하나**이며, 그것이 없었다면 필드를 더해도 전부 NULL인
컬럼이 됐을 것이다.

## 2026-09-12 — 검토 에이전트 둘을 만들고, 그들이 찾은 것을 고쳤다

`#70`에 남은 게이트 넷(2·4·6·7)은 전부 "이 메커니즘이 그 창을 실제로 닫는가" 종류인데,
저장소의 기존 네 에이전트는 전부 프로세스·거버넌스 판정자(게이트/보안/문서정본/커밋)라
그것을 볼 에이전트가 없었다. 둘을 만들었다 — `expert-control-plane`(분산 정합성),
`expert-gate-evidence`(게이트 근거 대조). 아래는 그들이 찾은 것과 그중 고친 것이다.

### 병합이 만든 문서 결함 넷 (고침)

2026-09-12 병합은 main의 병합 전 산문을 머리로, 브랜치 사실을 꼬리로 이어 붙였다. 게이트
3·4·5·6 네 행이 그 형태이고, **머리와 꼬리가 충돌하지 않는 것은 4뿐**이었다.

1. **게이트 6의 차단 사유가 폐기된 것이었다.** "`lease_generation`·`fencing_token`이 아직
   없다"고 적혀 있었으나, 2026-09-01 처분표가 그 둘을 `agents.command_generation`으로 **충족
   처리**했다(031이 이미 DB가 발행하는 Agent별 단조 증가 값을 만들었고 이름만 다르다).
   grep 0건은 "아직 없다"가 아니라 "영영 만들지 않기로 한 것"이라 차단 근거가 되지 못한다.
   `da523a6`이 이미 한 번 걷어낸 사유가 되살아난 것이다. 이 칸에 남은 것은 effect 경로
   하나이고 선행은 게이트 4 단 하나다.
2. **게이트 5의 첫 근거 문장이 지금 트리에서 거짓이었다.** "1.5단계가 `on_demand`를 후보에서
   제외한다(시험 4건)"인데, 2026-09-06에 그 제외는 probe로 대체됐고(1.5단계는 이제 liveness를
   거르지 않는다) 시험도 6건이다. 상태 `닫힘`은 정당하나 근거가 병합 전 트리의 것이었다.
3. **게이트 3이 한 칸 안에서 서로 다른 두 절반을 지목했다.** 머리는 "남은 것은 inventory
   보고", 꼬리는 "남은 절반은 ACK 유실과 grant expiry". 꼬리가 맞다 — orphan 절반은 닫혔다.
   머리를 지우지 않고 시점 표시를 달았다(더 정밀한 조사를 버리지 않기 위해).
4. **요약 문단이 표와 어긋났다.** "게이트 2와 3은 같은 하나를 기다린다"와 게이트 6의 위
   1번 사유 둘 다. 요약 문단은 표를 스캔하지 않는 사람이 먼저 읽는 자리다.

### 게이트 2의 선행이 게이트 3이라는 판정을 뒤집었다

표는 "그 물음의 답이 게이트 3이 지목하는 inventory와 같으므로 선행은 게이트 3"이라고 적어
왔다. **두 inventory는 축이 다르다.** 게이트 3은 **Agent 프로세스 축**이다 — `fleet-worker`가
자기가 띄운 것을 heartbeat의 `agent_orphans`로 보고한다. 게이트 2가 필요로 하는 것은 **task
실행 축**이고, 오케스트레이터는 워커의 `endpoint`(grok agent serve)에 **직접** 붙으므로
`fleet-worker` 프로세스는 그 세션을 보지 못한다. 상대도 채널도 다르다 — **게이트 3을 끝까지
닫아도 게이트 2는 열려 있다.**

두 에이전트가 서로를 모르는 채 같은 결론에 닿았다. 근거 감사 쪽은 "`agent_orphans`는 워커가
자기가 띄운 것에 대해 말하는 자기 보고이고 오케스트레이터가 거는 전수 조회가 아니다"로,
제어면 쪽은 "채널과 상대가 다르다"로 왔다.

게이트 2의 실제 선행은 **task 실행 신원의 내구화**다. ACP `session_id`는
`fleet-transport`의 인메모리 맵에만 있고 `fleet-store`·`fleet-core`에 grep 0건이라, 재시작하면
"내가 띄운 세션"을 가리킬 이름 자체가 사라진다. `session/list` 왕복은 이미 `probe`가 하고
있으나 **응답 본문을 버린다** — 인벤토리의 오른쪽 절반이 이미 도착하고 있는데 붙일 왼쪽이 없다.

### archive의 제어 구멍을 닫았다 (어느 게이트에도 귀속되지 않았던 것)

게이트 4·7이 archive를 다루지만 둘 다 **무엇이 막아야 하는가**에 관한 것이다. 그 아래
**누가 archive할 수 있는가**가 비어 있었다: `update_project_status`는 조건 없는 `UPDATE`였고
MCP·Dashboard 어느 핸들러도 `lease_allows_control()`을 부르지 않았다(grep 0건). **lease를 잃은
인스턴스가 Project를 archive할 수 있었다** — dispatch와 cancel은 `#62`·`#63`에서 막혔는데
제어면 결정 중 archive만 술어 없는 쓰기로 남아 있었다.

기존 fence와 같은 모양으로 닫았다(`AND EXISTS`를 `UPDATE`와 같은 문장 안에). `ArchiveProgress`에
`Fenced`를 더해 `Draining`과 구분한 것이 이 수정에서 판단이 필요했던 자리다 — 뭉개면 호출부가
"아직 막는 것이 있다"로 읽고 재시도하는데 fenced 인스턴스의 재시도는 영원히 성공하지 않는다.

시험은 실제 Postgres다. fence가 SQL 술어라 `MemStore`로는 검증되지 않고, 낡은 fence의 거절만
보면 "id가 없어서 0행"과 구분되지 않으므로 **살아 있는 fence로 같은 전이가 통과하는 대조**를
함께 뒀다.

### 고치지 않고 기록만 한 것

- `advance_project_archive`는 여전히 check-then-act다. 두 blocker SELECT와 최종 `UPDATE`가 별개
  문장이고 반대편 제출 경로도 그러해서 **archive 확정 뒤 Task가 들어오는 순서**가 남는다.
  fence는 "누가"를 닫았고 이 창은 "언제"에 속한다.
- `project_has_active_tasks`가 `Failed(ResultLost)`와 `Cancelled`를 종료로 친다. 그런데
  `ResultLost`의 정의는 "워커에서 아직 돌고 있을 수도 있다"이다 — 진행 중일 수 있는 실행 위로
  archive가 통과한다. 이것은 게이트 7의 본체다.
- `AcpTransport::cancel`은 세 경로에서 확인 없이 `Ok(())`를 준다(연결 없음, ack 없는
  notification, 세션 미발견). **오케스트레이터가 한 번이라도 재시작하면 `sessions` 맵이 비어
  모든 cancel이 조용히 성공하고** store에는 `Cancelled`가 적힌다. 워커의 세션은 계속 돈다.
  게이트 7이 failover 없이도 매일 일어나는 이유다.
- 마이그레이션 038이 결번이다(037 → 039, 040). 삭제된 적 없고 만들어진 적도 없다.

### 다음에 무엇을 할지에 대한 판정

넷 전부의 공통 선행이 **실행 신원**이라 순서를 잘못 잡으면 스키마를 다시 쓰게 된다: 원장(4)을
먼저 만들면 idempotency key의 주체가 `task_id`뿐이라 재시작 전후 두 시도가 같은 키를 갖고,
컬럼(6)을 먼저 만들면 값이 항상 NULL이며(037·040이 이미 밟은 함정), `CancelUnconfirmed`(7)만
먼저 만들면 탈출 조건이 확인 채널인데 신원이 없어 **archive가 영원히 막힌다**.

권고는 `task_attempts` 행이다. 다만 **이 판정에는 여기서 실측할 수 없는 전제가 하나 있다** —
grok이 `session/list`를 실제로 구현하는가. SDK는 v1 표준 메서드로 갖고 있으나(피처 게이트
아래가 아니다), probe의 독스트링 자신이 "grok이 구현하지 않아도 이 probe는 성립한다
(`-32601`)"고 적고 **응답 본문을 버린다.** 즉 **아무도 그 응답을 본 적이 없다.** `-32601`이면
대조할 오른쪽이 없으므로 우선순위는 "워커 쪽에 조회 가능한 표면을 만드는 것"으로 먼저
옮겨야 한다. 이 컨테이너에는 grok 바이너리가 없어 실측하지 못했다.

## 2026-09-13 — 실행 신원을 내구화했다. 그리고 그 전에 판정 하나를 뒤집었다

`#70`의 차단된 네 게이트(2·4·6·7)가 전부 같은 하나를 기다린다는 것이 2026-09-12
검토의 결론이었고, 그 하나가 **task 실행 신원**이었다. 이번에 그것을 넣었다.

### 착수 직전에 뒤집은 것

전날의 권고는 **`task_attempts` 테이블 신설**이었다. 착수 전에 로드맵을 훑다가
`#97`("`TaskAttempt` 목표 엔티티 존치 여부 재판정")이 **이미 완료**로 닫혀 있는 것을
봤다. 판정은 **흡수**였다 — 2026-08-26에 `TaskAttempt`를 `Task`에 흡수시키고 26개
파일을 그에 맞춰 고쳤으며 `#95`의 `attempt_id`도 그때 제외했다.

즉 전날의 권고는 **닫힌 판정을 정면으로 거스르는 것**이었다. 제어면 에이전트가 그
판정을 읽지 않고 설계했고, 나도 그대로 보고했다.

판정 자체가 여전히 옳은지 코드로 확인했다. 근거는 "무재시도 정책 아래에서 Task와
실행 시도는 1:0..1"인데, 그것이 문서상의 정책이 아니라 코드에서 참이었다:

- `Task::allowed_predecessors(Pending)`이 **빈 배열**이다. 아무것도 `Pending`으로
  들어올 수 없으므로 `Dispatched → Pending`이 불가능하고, 한 번 dispatch된 Task는
  영원히 한 번만 dispatch된다.
- 재시도(`#38`)는 `NoWorker`/`CircuitOpen`에만 걸린다 — **워커에 닿기 전**이고 Task는
  `Pending`으로 남는다. 실행이 둘 되는 경로가 아니다.

따라서 `task_id`가 곧 실행 신원이다. **제어면 에이전트가 선택지 B(Task 행에 컬럼)를
물리친 이유 — "이전 attempt의 session_id가 덮어써진다" — 는 성립하지 않는다. 이전
attempt가 없다.** 물리쳐졌던 B가 맞는 답이었다.

여기서 배울 것: **에이전트가 코드를 아무리 정확히 읽어도 닫힌 판정을 모르면 그
판정을 되돌리는 설계를 낸다.** `expert-control-plane`의 고정 사실에 "없는 것은 진짜로
없다"는 넣었지만 "없기로 **결정된** 것"은 넣지 않았다. 부재의 두 종류가 grep으로는
구분되지 않는다.

### 넣은 것

마이그레이션 041이 `tasks.acp_session_id`를 더하고, `WorkerEvent::SessionOpened`가
`session/new` 성공 직후 신원을 나르며, dispatcher가 fence를 걸어 기록한다.

쓰기 술어가 둘이다. `acp_session_id IS NULL`은 낙관적 최적화가 아니라 **흡수 판정의
불변식을 DB에서 강제**하는 것이다 — 두 번째 세션 id는 덮어쓸 값이 아니라 위반이고,
덮어쓰면 먼저 열린 세션이 추적 불가능한 고아가 된다. fence는 세션을 연 인스턴스가
이미 제어권을 잃었다면 그 기록도 그 인스턴스의 것이 아니라서다.

`SessionOpened`를 `Output`/`ToolCall`과 달리 fence 아래 두는 것이 이 변경에서 판단이
필요했던 자리다. **그 둘은 관측이고 이것은 신원**이다 — 관측은 틀려도 나중에 버리면
되지만, 신원이 틀리면 재시작 뒤의 cleanup이 남의 세션을 지목한다.

**창은 닫히지 않았다.** `session_id`는 `Pending → Dispatched` CAS보다 늦게 생기므로 같은
문장에 실을 수 없고, 그 사이의 크래시는 여전히 이름 없는 세션을 남긴다. 남은 폭은
`session/new` 왕복이다. 이것을 "닫았다"고 적으면 이 저장소가 반복해 온 그 오류다.

### 검증

시험 셋을 `both_backends!`로 뒀다(`MemStore` + 실제 Postgres). 낡은 fence의 거절만
단정하면 "항상 거절한다"는 구현으로도 통과하므로 **살아 있는 fence로 같은 기록이
통과하는 대조**를 함께 뒀다. 전송 계층은 `acp_transport_integration.rs`가 실제
`session/new` 왕복에서 `SessionOpened`가 나오는지 단정한다 — 그 단정이 없으면 이벤트를
발행하지 않아도 그 시험은 그대로 통과한다.

**pg 경로가 실제로 돌았는지 별도로 확인했다.** `both_backends!`는 한 테스트 안에서
mem을 먼저 돌리고 `DATABASE_URL`이 있을 때만 pg를 돌리므로, 통과 개수만으로는 pg가
돌았는지 알 수 없다(§4.3의 조용한 skip과 같은 계열이다). 시험 하나만 단독 실행한 뒤
DB를 직접 조회해 `sess-abc`가 남아 있는 것을 봤다.

게이트: rustc 1.98.0 · `RUSTFLAGS="-D warnings"` · fmt/clippy 두 세트 exit=0 ·
두 세트 78 suites 0 failed(1404 / 1400 passed).

### 다음

소비자가 아직 없다. cancel의 재시작 후 폴백(게이트 7)이 가장 값싸고, Reconciler의
inventory 대조(게이트 2)가 그다음이다. 후자의 오른쪽 절반은 여전히 grok이
`session/list`를 구현하는가에 달려 있고 그것은 아직 실측되지 않았다.

## 2026-09-14 — cancel 폴백: 재시작한 오케스트레이터가 취소를 실제로 보낸다

전날 내구화한 실행 신원의 첫 소비자다. 게이트 7의 절반이 닫혔다.

### 무엇이 틀려 있었나

`AcpTransport::cancel`은 `task_id`로 인메모리 세션 맵만 뒤졌다. 그 맵은 프로세스와 함께
비워진다. 즉 **오케스트레이터가 한 번이라도 재시작하면 그 뒤의 모든 취소가 대상을 찾지
못한 채 `Ok(())`를 돌려줬다.** 저장소에는 `Cancelled`가 적히고 워커의 grok 세션은 계속
돌았다. failover가 아니라 평범한 재배포마다 일어나는 일이다.

세 경로가 전부 `Ok(())`였던 것도 같은 결함의 다른 얼굴이다 — 연결이 없을 때, 통지를
보냈을 때, 세션을 못 찾았을 때. 그중 첫 번째에는 주석이 "idempotent success"라고 적혀
있었는데, **그것은 거짓이다.** 워커가 재연결되면 그 세션은 그대로 돌고 있고 우리는
취소를 한 번도 보내지 못했다.

### 고친 것

호출부가 `CancelRequest`에 `tasks.acp_session_id`와 `worker_id`를 함께 싣는다. 인메모리
맵이 모르면 그 단서로 해당 워커의 연결에 직접 보낸다. 세션 id만으로는 어느 연결로
보낼지 알 수 없어 워커가 함께 필요하다.

반환형을 `CancelDelivery`로 바꿔 `Sent`/`NoSession`/`Unreachable`을 가른다. `Sent`가
**확인이 아니라는 것**을 타입 문서에 못 박았다 — ACP cancel은 ack 없는 notification이라
"받았다"도 "멈췄다"도 뜻하지 않는다.

### 이 변경에서 판단이 필요했던 자리 둘

**(1) `NoSession`과 `Unreachable`의 경계.** 처음 구현은 "세션은 아는데 그 워커가 등록돼
있지 않다"를 `NoSession`으로 접었다. 시험을 쓰다가 내가 **시험을 구현에 맞춰** 적고
있다는 걸 알아챘다 — 이름은 `..._is_unreachable`인데 단정은 `NoSession`이었다. 접으면
이 enum이 존재하는 이유가 사라진다: 저쪽은 "보낼 것이 없다"이고 이쪽은 "저쪽이 계속
돌고 있을 수 있다"이다. 구현을 고치고 시험을 의도에 되돌렸다.

**(2) `audit_control`이 아니라 `audit_decision`.** 미전달을 감사에 남기려고
`audit_control`을 부르려다, 그 함수가 **리스가 없으면 아무것도 남기지 않는다**는 것을
봤다. 독스트링의 근거는 "단일 인스턴스 배포에는 세대도 경합도 없으므로 남길 사실이
없다"인데, 그 근거는 경합 사실(`write_fenced`, `dispatch_refused`)에는 맞지만 **미전달
취소에는 맞지 않는다.** 미전달은 경합과 무관하고 오히려 단일 인스턴스에서 더 흔하다 —
재시작하면 맵이 비고, 리스와 아무 상관 없이 취소가 전달되지 못한다. 그대로 실었다면
가장 흔한 경우가 통째로 사라졌을 것이다. 리스와 무관하게 남기는 `audit_decision`을
따로 두고 둘의 갈림을 양쪽 독스트링에 적었다.

### 검증

핵심 시험은 `a_restarted_orchestrator_cancels_using_the_stored_session`이다. **새
`AcpTransport`를 만들어 dispatch 없이 워커만 등록한다** — 인메모리 세션 맵이 비어 있는,
재시작 직후와 정확히 같은 상태다. 폴백이 없으면 여기서 `Sent`가 나올 수 없다.

`Unreachable`과 `NoSession`도 각각 시험이 있다. 셋을 함께 두는 이유는 하나만 두면
"항상 그 값을 돌려주는" 구현으로도 통과하기 때문이다.

게이트: rustc 1.98.0 · `RUSTFLAGS="-D warnings"` · fmt/clippy 두 세트 exit=0 ·
두 세트 78 suites 0 failed(1406 / 1402 passed).

### 아직 닫히지 않은 것

게이트 7의 남은 절반은 `CancelUnconfirmed` 상태와 그 상태에서 빠져나오는 **확인
채널**이다. 지금은 `Unreachable`이어도 Task를 `Cancelled`로 확정한다 — 감사에 흔적을
남길 뿐 상태 기계는 그대로다. 그 상태를 지금 만들면 탈출 조건이 없어 archive가
영원히 막히고, 탈출 조건인 확인은 세션이 실제로 사라졌는지 되묻는 왕복을 요구한다.
그것은 `session/list`이고, grok의 지원 여부는 여전히 실측되지 않았다.

## 2026-09-14 — 게이트 4: 원장을 만들지 않기로 하고, 원장의 입력을 고쳤다

게이트 4(effect ledger)에 착수했고, 정본을 읽고 **지금 원장을 만들면 안 된다**는 결론에
닿았다. 그 판단과 대신 고친 것을 남긴다.

### 왜 만들지 않았나

[실행 일관성](architecture/tasks/execution-consistency.md)이 막는 것을 셋 적고 있고, 셋 다
코드로 넘을 수 있는 종류가 아니다.

1. **생산자가 구조적으로 없다.** 정본은 "Worker가 호출 **전에** 기록해야 한다"고 요구하는데
   도구는 워커의 grok 프로세스 **안에서** 돈다. 오케스트레이터도 `fleet-worker`도 그 호출을
   하지 않으므로 "호출 전"이라는 시점 자체를 가질 수 없다. 우리가 보는 것은 ACP가 사후에
   알려 주는 관측뿐이다.
2. **증명이 없다.** `status = Completed`는 Agent의 보고이지 부작용이 적용됐다는 증명이
   아니다(정본이 직접 그렇게 적는다). 증명은 provider receipt에서 오고 아직 없다.
3. **외부 idempotency key의 앵커가 설계 미결이다.** 무재시도 아래에서 실패는 새 Task가 되어
   Task ID가 바뀌고 키가 달라진다. 정본과 `#62`가 이것을 **설계 미결**로 올려 두고 "없는 것을
   미리 만들지 않는다"고 적었다.

무시하고 여덟 상태 테이블을 만들면 037·040에서 밟은 함정(생산자 없는 컬럼)과
`OutcomeUnknown`·`CancelUnconfirmed`에서 확인한 함정(출구 없는 상태)을 **동시에** 밟는다.

`expert-control-plane`에 어제 넣은 고정 사실("부재에는 두 종류가 있고 grep은 구분하지
못한다")이 이번에 실제로 작동했다. 앵커가 없다는 것은 grep으로 보면 (1)이지만 정본을
읽으면 (2)다.

### 대신 고친 것 — 원장의 입력이 terminal 뒤에 사라지고 있었다

`WorkerEvent::ToolCall` → `FleetEvent::TaskToolCall`은 원장이 아니라 **원장의 입력**이다(정본
표현). 그 입력이 정작 가장 필요한 구간에서 기록되지 않고 있었다.

소비자가 `worker_id`를 **Task 상태에서 되찾았다.** `current_worker_of`는
`TaskStatus::Dispatched`만 매치하므로, Task가 terminal로 확정되는 순간부터 도착하는 도구
호출은 축을 찾지 못해 전부 버려졌다 — `Failed(TaskFailure)`가 `worker_id`를 실제로 들고 있는
경우까지 포함해서. 하필 그 구간이 게이트 4가 존재하는 이유다: `FailureKind::ResultLost`의
정의가 "워커에서 아직 돌고 있을 수도 있다"인데 **정말로 돌고 있다는 유일한 증거가 정확히
그 순간부터 인멸됐다.**

원래 판단("워커를 지어내지 않는다 — 틀린 축은 없는 축보다 나쁘다")은 옳다. 틀린 것은 축을
**가변적인 Task 상태에서 추론**하려 한 쪽이다. 이제 축은 그 관측을 배달한 연결이 이벤트에
실어 보낸다 — 지어낸 값이 아니라 가장 권위 있는 출처의 값이다.

"terminal 뒤에 왔는가"를 수신 시점에 판정하지는 않는다. 도구 호출마다 store를 읽으면
스트리밍 경로에 왕복이 생긴다. 이벤트와 Task의 terminal 시각이 둘 다 남으므로 사후로 도출된다.

### 결함을 고정하고 있던 시험

`a_tool_call_for_a_task_with_no_worker_is_dropped`가 **버리는 동작을 단정**하고 있었다. 즉
이 결함은 시험으로 보호받고 있었다. 이름은 "워커를 모르는 Task"인데 실제로 그 시험이 만든
것은 `Pending` Task였고, 같은 구현이 `Failed`(워커를 아는)에 대해서도 똑같이 버린다는 사실은
어느 시험도 보지 않았다.

`a_tool_call_after_the_task_went_terminal_is_still_recorded`로 바꾸고, 왜 예전 단정이
틀렸는지를 독스트링에 남겼다. 실제 ACP 왕복 시험에도 "도구 호출은 자기를 배달한 워커를
싣고 와야 한다"는 단정을 더했다.
...
**여전히 차단이다.** 원장도 여덟 상태도 receipt도 그대로 없다. 닫힌 것은 "원장이 나중에 읽을
로그에서 가장 중요한 구간이 비어 있었다"는 결함 하나다. 게이트를 실제로 여는 것은 위 세
가지 중 (3) 앵커의 설계 결정이며, 그것은 코드가 아니라 판단을 기다린다.

## 2026-09-14 — 게이트 4를 의도적 보류로 확정했다 (D0 답)

앞 항목에서 게이트 4에 필요한 결정을 넷으로 정리하고, 그보다 앞선 질문 하나(D0)를 물었다:
**이 fleet이 실제로 비가역 외부 부작용을 내는가.** 답은 "외부 메시지 발송이나 DB
마이그레이션은 아직 진행하지 않았다"였다.

그래서 게이트 4는 **차단으로 두되, 방치가 아니라 판정으로** 남긴다. 원장이 증명할 대상 자체가
아직 없고, 그 상태에서 여덟 상태 테이블을 만들면 정의되지 않은 개념 셋(effect scope·policy
revision·redrive 앵커)을 발명하게 된다. 셋 다 "무엇이 같은 부작용인가"의 답이고, 그 답은 어떤
외부 provider를 쓰는지가 정해져야 나온다.

### 이 판정에 딸린 단서 하나

**부재가 강제된 것이 아니다.** 도구 수준 allow-list가 없고(`allow_list`/`sandbox` grep 0건)
`ToolInvocationKind`는 `Execute` 하나로 `ls`와 외부 API 호출을 똑같이 표현한다. 즉 오늘도
Agent가 `bash`로 비가역 호출을 **할 수는 있고**, 하지 않고 있을 뿐이다.

그래서 D1("부작용 등급을 누가 매기는가")은 원장의 선행이 아니라 **원장보다 앞선 공백**이다.
등급을 매길 주체가 없으면 원장에 상태가 있어도 재호출 정책의 입력이 없다. 그 공백을 닫는 것은
로드맵 `#64`(Agent 실행 격리, P1 · 설계 확정·구현 대기)의 범위다.

### 재개 조건

비가역·보상불가 부작용을 내는 워크로드를 도입하는 순간, **원장은 그 워크로드보다 먼저 있어야
한다.** 부작용이 먼저 나가고 원장이 나중에 붙으면 그 사이의 실행은 영원히 증명 불가 구간으로
남는다. 이 순서를 정본과 게이트 표 양쪽에 적었다 — 다음 세션이 "나중에 붙이면 된다"로 읽지
않도록.

필요한 결정은 그때 순서대로 (1) 부작용 등급 주체, (2) redrive 앵커, (3) effect scope 정의,
(4) HMAC 키 소유·회전이다. 각각의 선택지와 실패 방식은 이 세션의 대화에서 정리했고 정본의
「원장을 언제 만들 것인가」가 요지를 갖는다.

**배제한 후보 하나를 남긴다**: 클라이언트 `idempotency_key`는 redrive 앵커로 쓸 수 없다.
`(created_by, idempotency_key)`가 저장소 유일 제약이라 같은 키 재제출은 **기존 Task를
반환**하지 새 Task를 만들지 않는다. 앵커는 정의상 새 Task를 만들면서 살아남아야 하므로 정반대다.

## 2026-09-14 — `#64` 착수 전 실측: 일곱 게이트 중 여섯이 선행 미구현

`#64`(Agent 실행 격리)에 착수했고, 코드를 대조한 뒤 **이 환경에서 진행할 수 없다**는 결론에
닿았다. 판정과 근거를 정본의 「게이트별 실측 상태」에 표로 남겼다. 여기에는 경위만 적는다.

### 무엇을 확인했나

정본을 먼저 읽는 습관이 이번에도 값을 했다. 착수 직전에 세 가지가 드러났다.

1. **`AgentTemplate.isolation_class`는 의도적 유예이고 선행이 `#52`다.** 근거는 "등급을 표현할
   타입이 코드에 없다 — `ReadOnly` 같은 이름이 저장소 전체에 0건. 검증할 집합 없이 자유
   문자열로 만들면 값이 아무 뜻도 갖지 않는다". 다만 **그것은 `#64`의 isolation과 다른 축**이다
   — 저쪽은 도구 등급이고 이쪽은 `host_trusted`/`container_required`로 정본에 완전히 정의돼
   있다. 두 이름이 비슷해 섞기 쉽다.
2. **`Worker`에 capability 개념이 없다**(`worker.rs`에 `capabilit*` 0건). 정본의 "Worker는 요구
   capability가 없으면 거절한다"를 걸 자리가 없다.
3. **개발 컨테이너에 docker 데몬이 없다**(`/usr/bin/docker`는 있으나 소켓 없음). 즉 container
   실행 경로를 만들어도 **돌려볼 수 없다.** 이 저장소 규칙("검증되지 않은 수정을 고쳤다고
   보고하지 않는다")상 그것은 만들면 안 되는 종류다.

### 유일하게 이미 성립하는 절반

게이트 3(다른 Task 자원 불간섭)의 **프로세스 축은 이미 지켜지고 있다.**
`terminate_recorded_agents_blocking`이 spawn record로 대상을 좁히고 `is_our_child`(시작 시각
비교)로 PID 재사용까지 거른다. 정본이 금지한 광범위 종료(`tmux kill-server`·`killall`류)는
저장소에 0건이다(검색된 `rm -rf`는 전부 shell-quoting 시험의 픽스처다).

### 왜 isolation 필드만 먼저 만들면 안 되나

`#64`의 실체는 `fleet-worker`에 container 실행 어댑터를 만드는 것이고 나머지 여섯 게이트가
전부 그 위에 얹힌다. 어댑터 없이 필드만 두면 둘 중 하나가 된다 — 기본값을 정본대로
`container_required`로 하면 **오늘의 모든 Task가 거절되고**, `host_trusted`로 하면 정본을
어기는 기본값이 된다. 어느 쪽도 격리가 아니다. 게이트 4·7에서 배운 "출구 없는 상태"·"생산자
없는 컬럼"과 같은 계열이다.

### 앞선 진술 정정

바로 앞 항목에서 "도구 수준 allow-list가 없다(grep 0건)"고 적었는데 **부정확했다.**
`AgentTemplateBody.tools`가 실재한다 — Postgres에 저장되고 Dashboard가 렌더링한다. 정확한
사실은 "**읽어서 제한하는 코드가 없다**"이다. 결론(오늘 `bash`로 무엇이든 할 수 있다)은
같지만, 없는 것과 **강제되지 않는 것**은 다른 사실이고 후자가 더 위험하다 — 통제처럼 보이기
때문이다.

그리고 이 정정이 새 가능성을 연다: `ToolInvocation.name`이 이제 기록되므로 **탐지형
통제**(허용 목록 밖 호출을 감사에 남기는 것)는 container 없이 지금 만들고 검증할 수 있다.
예방형(호출 자체 차단)만이 container mount 경계를 기다린다.

## 2026-09-14 — 선언만 되어 있던 tool allow-list를 탐지형 통제로 만들었다

`#64` 실측에서 드러난 구멍이다. `AgentTemplateBody.tools`는 Postgres에 저장되고 Dashboard가
렌더링하지만 **읽어서 제한하는 코드가 없었다.** 강제되지 않는 allow-list는 없는 것보다
위험하다 — 통제처럼 보이기 때문이다.

이제 `Dispatcher`가 관측된 도구 호출을 pin된 템플릿 revision의 목록과 대조하고, 밖이면
`agent.tool_outside_allowlist`로 감사에 남긴다.

### 탐지이지 예방이 아니다

도구는 워커의 grok 프로세스 **안에서** 이미 실행된 뒤이고 오케스트레이터는 ACP 알림으로
사후에 볼 뿐이다. 호출 자체를 막을 수단이 없다 — 그것은 `#64`의 container mount·egress
경계이고 그대로 막혀 있다. 그래도 남기는 이유는 **위반이 일어났다는 사실만이라도 durable해야**
하기 때문이다.

### 판정하지 않는 셋을 "위반 아님"과 구분했다

이 변경에서 가장 틀리기 쉬운 자리다. 셋 다 "위반 아님"이 아니라 **"판정 불가"**다:

1. Task에 `agent_id`가 없다 — 적용할 템플릿 자체가 없다.
2. pin이 없거나 목록이 **비어 있다**. `tools`는 `#[serde(default)]`라 기본이 빈 벡터이고 기존
   템플릿 대부분이 여기 해당한다. **빈 목록은 "아무 도구도 허용 안 함"이 아니라 "선언하지
   않았다"**이다 — deny-all로 읽으면 모든 호출이 위반으로 쏟아져 신호가 죽고 통제가 무의미해진다.
   미선언 자체가 공백이라는 사실은 별개로 다뤄야 하고 호출마다 남길 일이 아니다.
3. `invocation.name`이 `None`(SDK의 `unstable_tool_call_name`이 주지 않은 경우) — 이름을 모르면
   대조할 수 없다. 위반으로 접으면 거짓 양성, 통과로 세면 거짓 음성이다.

### 캐시를 revision id로 건 이유

`#86`의 요지가 **revision immutability**라 pin된 본문은 바뀌지 않는다. 그래서 무효화 문제가
없다. Agent id로 캐싱했다면 pin이 옮겨갈 때 낡은 목록을 쓰게 된다. 캐시를 두는 이유는 이
판정이 스트리밍 경로에서 돌기 때문이다 — 도구 호출마다 Agent와 revision을 조회하면 왕복 둘이
붙는다.

### 순서: 관측을 먼저 남기고 대조는 나중에

처음에 대조를 `append_event` **앞**에 두고 주석에는 "이벤트를 남긴 뒤에 한다"고 적었다 —
주석과 코드가 어긋나 있었다. 주석의 논리가 옳다: 대조는 store 조회를 하므로 그쪽이 실패하거나
느려도 "이 도구가 호출됐다"는 사실 자체는 먼저 durable해야 한다. 그것이 원장의 입력이고
허용 목록 판정은 그 위에 얹히는 부가 정보다.

### 시험이 잡아낸 도메인 규칙

시험을 쓰다가 템플릿이 **draft로 태어나고 draft는 pin을 받지 않는다**는 규칙에 걸렸다
(`Conflict("... is draft and does not accept new pins")`). 시험도 실제 경로(published로 전이
후 pin)를 밟게 고쳤다.

시험 넷을 둔 이유는 하나만 두면 "항상 그 답을 내는" 구현으로도 통과하기 때문이다 — 허용된
도구가 조용한지, 목록 밖이 남는지, 빈 목록이 deny-all이 아닌지, 이름 없는 호출이 판정되지
않는지.

게이트: rustc 1.98.0 · `RUSTFLAGS="-D warnings"` · fmt/clippy 두 세트 exit=0 ·
두 세트 78 suites 0 failed(1410 / 1406 passed).
...
## 2026-09-15 — MCP 호출자 신원을 런처 주장 수준으로 메웠다 (`#58`)

앞 항목의 보안 검토가 "MCP `fleet_cancel_task`에 caller principal·Project scope 검증이 없고,
이번 주 cancel 폴백이 그 공백의 실효 피해를 키웠다"로 끝났다. 그 절반을 메운다.

### 먼저 내 표현을 정정한다

앞에서 이것을 "인가 공백"이라고 뭉뚱그렸는데 **부정확했다.** `fleet_cancel_task`는
`PermissionKind::TaskCancel`을 **요구한다** — `server.rs`에 도구별 capability 행렬이 있고
핸들러 진입 전에 막는다. 없는 것은 capability가 아니라 **(a) 호출자 신원과 (b) Project scope**다.

그리고 **(b)는 의도적 보류**다. 로드맵 `#58`이 적었듯 `fleet-core::auth`에 User를 Project로
한정하는 멤버십 개념이 없어서, 지금 검사를 넣으면 비교 대상이 없어 **항상 통과하는 죽은
분기**가 된다. 멤버십 모델 설계 승인이 선행이다.

### (a)는 메울 수 있었다 — 전제가 바뀌었기 때문이다

마이그레이션 024가 멱등성 유일성 스코프를 `created_by`로 잡은 근거는 명시적이다: "정본이
말하는 principal에 해당하는 값이 오늘 코드에 존재하지 않는다 ... 지금 principal 컬럼을 만들면
아무도 채우지 않는 열이 된다." 그래서 **모든 MCP 제출이 `"mcp"` 한 버킷을 공유**했고, 키
네임스페이스가 MCP 클라이언트 단위가 아니라 오케스트레이터 단위였다.

그 결과가 단순한 불편이 아니다. `(created_by, idempotency_key)`가 유일 제약이므로 **서로 다른
두 MCP 호출자가 같은 키를 쓰면 뒤에 온 쪽이 앞 호출자의 Task를 돌려받거나 409를 받는다** —
키 네임스페이스가 호출자 간 채널이 된다.

`FLEET_MCP_PRINCIPAL`(선택)이 그 전제를 바꾼다. 있으면 `created_by = mcp:<principal>`,
없으면 기존과 같은 `"mcp"`다.

### 이것이 인증이 아니라는 것을 세 곳에 적었다

런처가 env로 주장하는 값이고 신뢰 출처가 `FLEET_MCP_CAPABILITIES`와 **같다** — 더 낮지도
높지도 않다. 그런데도 유효한 이유는 **이 값으로 권한이 생기지 않기** 때문이다. 착수 전에
`created_by`가 인가 판정에 쓰이는지 확인했고, 저장소에서 술어로 쓰이는 자리는 멱등성 조회와
선택적 `list_tasks` 필터 둘뿐이었다. 만약 어딘가에서 "이 리소스를 볼 수 있는가"를 `created_by`로
판정했다면 런처가 그 값을 정하게 두는 것은 권한 상승이므로 하지 말았어야 한다.

### 접두사가 요지다

`mcp:` 접두사가 없으면 런처가 Dashboard 사용자의 username과 같은 값을 주장해 **그 사용자의
멱등성 네임스페이스를 점유**할 수 있다. 같은 키로 제출했을 때 상대가 내 Task를 돌려받거나
409를 받는다. 접두사는 두 네임스페이스가 절대 겹치지 않게 한다. 시험 이름도 그것을 단정한다.

빈 값은 `None`으로 접는다 — 빈 신원은 신원이 아니고, `"mcp:"`라는 무의미한 버킷을 하나 더
만들 뿐이다.

### 호환성

신원을 주지 않는 배포는 **기존 동작 그대로**다. 기존 행들이 `"mcp"` 버킷에 있으므로 기본값을
바꾸면 과거 제출과 네임스페이스가 갈라진다 — 그래서 기본값을 유지하고 시험으로 고정했다.
`"mcp"` 리터럴과 비교하는 소비자가 저장소에 없다는 것도 확인했다(접두사가 깨뜨릴 코드 없음).

게이트: rustc 1.98.0 · `RUSTFLAGS="-D warnings"` · fmt/clippy 두 세트 exit=0 ·
두 세트 78 suites 0 failed(1417 / 1413 passed).
...
## 2026-09-16 — ingest — `session/list` 인벤토리와 게이트 2의 절반 (`#70`)

`#70` 게이트 2가 차단에서 부분으로 옮겨졌다. **그 차단 사유는 틀린 것이었다.**

### "실측되지 않았다"가 아니라 "읽지 않았다"였다

정본과 로드맵이 두 번 적은 사유는 "`session/list`를 grok이 지원하는지 아직 실측되지
않았다"였다. ACP는 그 지원을 `initialize` 응답의
`agentCapabilities.sessionCapabilities.list`로 **선언**하고, 그 응답은 **모든 연결에서
이미 도착하고 있었다** — `acp_transport.rs`가 `if let Err(e) = init`로 실패만 보고
`Ok`의 본문을 통째로 버리고 있었을 뿐이다.

같은 결함이 두 겹이었다. 게이트 5의 `probe`도 `session/list`를 왕복시키며 본문을
버리는데, 그쪽은 "답이 왔는가"만 묻기 때문에 그것이 옳은 처분이다. 그 옆에서
handshake까지 본문을 버리고 있었고, 그래서 같은 답이 두 번 도착했다가 두 번 버려졌다.

**이것이 이번 주에 세 번째다.** `#95`의 대기 사유("`ToolContext`에 principal이 없다")는
직전 커밋이 이미 없앤 것이었고, `#64`의 allow-list도 "필드가 없다"가 아니라 "저장되고
표시되기만 하고 아무도 읽지 않는다"였다. **차단 사유는 코드가 바뀌어도 스스로 갱신되지
않는다.** 정본이 이미 "차단 사유는 매번 다시 확인한다"고 적어 두었는데, 그 문장이 가리키는
것은 선행이 도착했는지뿐 아니라 **사유 자체가 애초에 참이었는지**다.

### 세 값을 가른다

`SessionInventory`는 `Reported(Vec<String>)`·`Undeclared`·`Refused` 셋이다. 셋을 하나로
뭉개면 "메서드를 모른다"와 "세션이 하나도 없다"가 같은 빈 결과로 도착하고, 그러면 이
게이트가 하려는 판정 자체가 불가능해진다. **빈 목록이 답이라는 것이 요점이다** — 재시작한
오케스트레이터가 `Dispatched`로 기억하는 Task의 세션이 목록에 없다는 사실은 그 목록이
권위 있을 때만 결론이 된다.

`Undeclared`와 `Refused`를 가른 것은 2026-09-14의 `NoSession`/`Unreachable`과 같은
판단이다. 둘 다 인벤토리를 주지 않지만 하나는 저쪽이 자기 자신에 대해 정직하게 말한 것이고
다른 하나는 선언과 행동이 어긋난 것이라, 후자에는 보고할 대상이 있다.

### 왼쪽은 이미 있었다

2026-09-13의 `tasks.acp_session_id`가 대조의 왼쪽이고, 그 절이 "아직 소비자가 없다"고
적은 소비자가 이번 것이다. Reconciler의 네 분기 중 앞의 셋은 전부 **워커에 관한 사실**로
판정하고, 넷째 `Some(_) => continue`가 구멍이었다 — 워커는 멀쩡한데 그 위의 **실행**만
사라진 경우를 어느 분기도 보지 않아 그 Task는 완료되지도 실패하지도 않은 채 `Dispatched`로
영구히 남았다.

### `ExecutionVanished`를 새로 만든 근거는 재제출 안전성 하나다

`ResultLost`는 "아직 돌고 있을 수도, 이미 끝났을 수도 있다"라 재제출하면 같은 일을 두 번
시킬 수 있다. 이쪽은 워커가 **지금 들고 있는 것 전부**를 답한 뒤의 부재이므로 실행이
끝났다는 것이 확정이고, 남은 미지는 "어떻게 끝났는가"뿐이다. 한 이름으로 뭉치면 운영자가
재제출해도 되는 경우와 안 되는 경우를 구분할 수 없다.

### 부정 단정이 이 변경의 절반이다

`Undeclared`를 빈 목록으로 읽는 구현은 **오늘의 모든 배포에서 진행 중인 모든 작업을
회수한다** — 실제 Agent 다수가 이 capability를 광고하지 않기 때문이다. 시험 두 건이 그것을
고정했고 구별력은 실측했다: `Undeclared`를 빈 목록으로 접도록 고치자 그 두 건이 실패했다.

transport 쪽에서는 "광고하지 않은 Agent에게는 **보내지도 않는다**"를 수신 기록으로
단정한다. mock은 물어보면 답할 수 있게 켜 두고 광고만 끈다 — 그 단정이 없으면 보내고
결과를 버리는 구현도 통과한다.

### 하네스가 가릴 뻔한 것

reconciler 시험의 첫 판은 워커를 **저장소에만** 등록했다. `list_sessions`는
`WorkerNotRegistered`로 실패했고 구현이 그것을 권위 없음으로 접으므로, 인벤토리를 한 번도
읽지 않은 채 "회수하지 않았다"가 통과할 뻔했다. `#95`에서 dispatch가 같은 이유로
`isError`로 끝나던 것과 같은 모양이다 — **transport를 쓰는 시험은 워커를 양쪽에 등록해야
한다.**

### 검증 한계

오늘 이 capability를 광고하는 Agent가 실제로 있는지는 관측되지 않았다. 다만 그것이 이제
런타임이 읽어 로그에 찍는 값이 됐으므로(`declares_session_list`) 배포 한 번이면 답이
나오고, 문서가 미지로 안고 있을 값이 아니게 됐다. 게이트가 요구하는 "failover 뒤
inventory-first가 신규 dispatch보다 **먼저**"라는 순서는 없다 — 지금 조회는 재조정 sweep
안에서 일어날 뿐 dispatch 경로를 막지 않는다.

---

## 2026-09-16 (2) — ingest — 고아 세션 회수와 `041` 인덱스의 첫 질의 (`#70`)

앞 항목이 만든 인벤토리 조회는 한 방향으로만 쓰였다. **거울상이 남아 있었고, 그쪽이 실제로
새고 있었다.**

### 새던 자리

`Dispatcher::cancel`이 `CancelDelivery::Unreachable`을 받으면 워커는 취소 통지를 받은 적이
없는데 저장소에는 `Cancelled`가 적힌다. 2026-09-14에 그 처분을 감사로 관측 가능하게 만들었지만
**그 뒤가 없었다**: 그 Task는 종료 상태라 `reap_stale_dispatched`의 `Dispatched` 필터에 걸리지
않고, 전송 계층의 세션 맵은 프로세스와 함께 비워지며, 워커의 세션은 계속 돌면서 토큰을 쓴다.
그 실행을 지목할 수 있는 코드가 한 곳도 없었다.

"실행은 남았는데 Task가 끝난" 쪽은 `Dispatched`만 훑어서는 **원리적으로** 볼 수 없다. 대상이
이미 종료 상태라 그 필터에 걸리지 않기 때문이다. 그래서 반대 방향의 sweep이 따로 필요했다.

### 인덱스는 먼저 들어왔고 질의는 없었다

`041_task_acp_session.sql`은 `idx_tasks_acp_session`을 만들며 주석에 용도를 적어 두었다 —
"재시작 뒤 '내가 띄웠다고 믿는 세션'을 여는 조회의 축". **그 조회는 한 번도 작성되지 않았다.**
`Store`에는 쓰기만 있었다. `find_task_by_acp_session`이 그 질의이며, 그래서 새 마이그레이션이
없다.

**이번 주에 반복되는 모양이 하나 더 생겼다.** 지금까지는 "차단 사유가 낡았다"였는데(`#95`·`#64`·
`#70`), 이것은 "선행이 이미 들어와 있었고 소비자만 없었다"이다. 둘은 같은 뿌리다 — 무엇이
있는지를 문서가 아니라 코드에서 확인하지 않으면, 있는 것을 없다고 적은 문장이 그대로 남는다.

### 셋을 가르는 것이 전부다

워커가 답한 세션 하나하나에 "이건 누구 것인가"를 물으면 셋이 나오고 셋이 서로 다른 처분을
요구한다: 살아 있는 Task의 것(건드리지 않는다), 끝난 Task의 것(고아 — 취소한다), 그리고
**아무 Task도 지목하지 않는 것**(손대지 않고 감사에만 남긴다).

셋째를 둘째로 접으면 안 된다. `None`이 나오는 경우가 셋이고 그중 둘은 죽이면 안 되는
것이다 — dispatch가 진행 중이라 `acp_session_id`가 아직 커밋되지 않았거나, 다른 제어면이 연
세션이거나. 관측만 남기는 것이 여기서 가능한 가장 강한 처분이다.

**이 갈래 덕분에 두 sweep 연속 확인 같은 장치가 필요 없어졌다.** 처음에는 집합 차이로
설계했고(인벤토리 − 살아 있는 세션 집합), 그러면 진행 중인 dispatch가 고아로 보여 두 번
연속 목격을 요구해야 했다. 역방향 조회는 집합 차이가 아니라 **정확한 질의**여서 그 경우가
`None`으로 떨어지고, 이미 안전한 쪽에 놓인다. 구조가 옳으면 방어 장치가 줄어든다.

### 내가 앞 항목에서 틀리게 적은 것

앞 항목의 정본 문단에 "게이트 7의 확인은 `Unreachable`로 끝난 취소의 세션이 인벤토리에서
사라졌는지를 되묻는 것"이라고 적었다. **틀렸다.** `CancelDelivery::Unreachable`은 "그 워커가
이 프로세스에 등록돼 있지 않다"는 뜻이고, 등록돼 있지 않으면 `list_sessions`도 같은 이유로
실패한다. 되물을 상대가 아예 없는 분기다.

확인이 놓일 자리는 `Sent` 쪽이고, `CancelDelivery::Sent`의 독스트링이 처음부터 그렇게 적고
있었다 — "ack 없는 notification이라 저쪽이 받았다도 실행이 멈췄다도 뜻하지 않는다". 코드를
읽고 나서야 알았고, 읽지 않았으면 도달할 수 없는 분기에 확인 로직을 넣었을 것이다.

### 취소에 진짜 `task_id`를 싣는 이유

`AcpTransport::cancel`의 1단계는 인메모리 맵을 `task_id`로 훑는다. 합성한 id를 실으면 그
조회가 엉뚱한 세션을 집을 수 있고 감사에는 존재하지 않는 Task가 남는다. 역방향 조회가 진짜
주인을 주므로 문제가 생기지 않으며, `None`인 경우에는 애초에 취소를 보내지 않으므로 실을
id가 없어 곤란해지는 자리도 없다. 설계가 맞물린 자리다.

### 검증

구별력은 실측했다 — 살아 있는 Task 가드를 없애자 `a_session_of_a_running_task_is_never_cancelled`가,
`None` 가드를 없애자 `a_session_no_task_claims_is_reported_but_left_running`이 각각 실패했다.
`MockTransport`가 받은 취소를 기록하도록 고쳤는데, 취소는 notification이라 결과가 상태에 남지
않아 그 기록이 없으면 "무엇을 취소하라고 했는가"를 시험이 볼 방법이 아예 없다.

---

## 2026-09-17 — ingest — 순서는 전제조건이라야 순서다 (`#70` 게이트 2 닫힘)

어제 두 커밋이 수단을 만들었다(인벤토리 조회, 양방향 회수). 남은 것은 게이트 문언의 한
단어였다 — 복구가 신규 dispatch보다 **먼저**.

### 배경 루프로는 순서가 만들어지지 않는다

`Reconciler`는 첫 tick을 한 주기(기본 30초) 뒤에 돈다. 그 사이의 dispatch는 아무것도 묻지
않은 채 나간다. **주기를 줄이는 것은 답이 아니다** — 창이 좁아질 뿐 닫히지 않고, 닫혔는지
확인할 방법도 없다. 루프와 dispatch가 경합하는 한 "먼저"는 관측이지 보장이 아니다.

그래서 순서를 dispatch의 **전제조건**으로 옮겼다. 구조적으로 어길 수 없는 자리에 두면
타이밍을 시험할 필요가 없어진다.

### 전역 배리어를 만들지 않은 이유

어제 이 작업을 예고하며 "리스 획득 시점을 아는 1회성 단계가 필요하다"고 적었다. 설계에
들어가서 그것이 **성립하지 않는다**는 것을 확인했다: "모든 워커가 재연결했다"는 시점이
존재하지 않는다. 워커는 자기가 join할 때만 transport에 등록되고(오케스트레이터가 저장소를
보고 먼저 다이얼하지 않는다 — `handlers.rs`의 register 경로가 유일한 진입점이다), 영영
돌아오지 않는 워커도 있다. 전역 배리어를 두면 그런 워커 하나가 fleet 전체의 dispatch를
영구히 막는다.

물어볼 수 있는 시점은 **그 워커에게 실제로 일을 주려는 순간**뿐이고, 그때가 곧 "먼저"가
성립해야 하는 유일한 자리다. 게이트 문언의 "failover 뒤"는 전역 단계처럼 들리지만 실제로
요구하는 것은 워커별 순서다.

### "물어봤다"가 통과 조건이다

`Undeclared`를 막으면 **오늘의 거의 모든 배포에서 dispatch가 영구히 막힌다.** 게이트가
요구하는 것은 "복구했다"가 아니라 "복구를 먼저 시도했다"이고, 물어볼 수 없는 상대에게
물어본 뒤 통과시키는 것이 그 요구의 정직한 이행이다. 거절하는 것은 **묻지 못한** 경우뿐이며
그때는 어차피 보낼 수도 없다.

이 구분을 놓치면 게이트를 닫으면서 fleet을 멈춘다. 부정 단정 시험 하나가 그 줄을 고정한다.

### 위치가 두 방향으로 묶여 있었다

처음에는 워커 선택 직후(브레이커 **앞**)에 두고 "브레이커가 열린 워커는 영영 복구되지
않는다"는 주석까지 달았다. **기존 시험 하나가 그 배치를 반증했다**: 못 닿는 워커의 dispatch가
회로를 열어야 하는데, 복구 검사가 앞에 있으면 transport 실패보다 먼저 반환해 브레이커 기록을
건너뛴다. 그러면 selector가 그 워커를 영영 계속 고르고 모든 dispatch가 거절된다.

브레이커 뒤로 옮기고, 복구 실패도 transport 실패와 **같은 자리에** 브레이커 실패를 남기도록
했다. 원래 주석의 걱정은 과했다 — 회로가 열린 워커는 여기 오지 않으므로 복구할 필요도 없고,
그 워커의 어긋난 Task는 주기 스윕이 본다. CAS 앞인 것은 반대 이유다: 상태만 옮겨 두고
보내지 못한 실행이 생기면 안 된다.

### 기존 시험 둘이 바뀌었고, 둘 다 하네스였다

`a_write_the_store_fenced_is_audited_with_the_epoch_we_held`는 워커를 저장소에만 올려서 CAS에
닿기도 전에 `RecoveryUnavailable`로 끝났다 — transport에도 등록하도록 고쳤다. 어제 두 번
겪은 것과 **같은 함정이 세 번째**다.

`invalid_request_does_not_open_the_circuit_but_a_worker_fault_does`는 의도적으로 등록하지 않은
워커로 "워커 결함"을 만드는데, 이제 그 결함이 transport dispatch보다 먼저 걸린다. 에러
이름만 `Transport` → `RecoveryUnavailable`로 바꿨고 **그 시험의 주장(회로가 열린다)은 그대로
통과한다**. 주장이 아니라 이름만 바뀐 것을 확인하고 나서 고쳤다.

### 검증

구별력은 셋 다 실측했다 — 조회 실패를 통과시키자, 세대 기록을 무시하자, 정리를 생략하자
각각 해당 시험이 실패했다.

**검증 한계**: 오늘 이 capability를 광고하는 Agent가 실제로 있는지는 여전히 관측되지 않았다.
게이트 문언은 닫혔지만 그 인접 사실은 배포 한 번이 답한다.

---

## 2026-09-17 (2) — ingest — 확인은 알 수 있었는데 시계가 없었다 (`#70` 게이트 3)

게이트 3의 남은 절반은 둘이었다 — ACK 유실과 grant expiry. 앞쪽이 닫혔다.

### 값은 있었고 판독자가 없었다

`031`이 `command_generation`/`last_acked_generation`을 만들었고 `command_delivered()`·
`start_pending()`이 그 위에 술어까지 올려 두었다. 그런데 **그 술어들의 호출부는 MCP 응답을
조립하는 두 줄뿐이었다.** 운영자가 `fleet_list_agents`를 직접 호출해 눈으로 보지 않는 한,
Worker가 명령을 영영 집어가지 않아도 아무 신호가 나지 않았다.

**이번 주 세 번째로 같은 모양이다.** 게이트 2의 차단 사유는 "실측되지 않았다"가 아니라
"읽지 않았다"였고(`initialize` 응답), 고아 세션 회수에서는 `041`의 인덱스에 질의가 없었으며,
여기서는 술어에 판독자가 없었다. 셋 다 "없다"가 아니라 **"있는데 쓰이지 않는다"**였다.

그 셋을 묶는 교훈은 하나다: **무엇이 막고 있는지는 문서가 아니라 grep이 말한다.** 세 번 다
문서에 적힌 차단 사유("실측되지 않았다", "만들어야 한다")를 믿었으면 없는 것을 새로 만들었을
것이고, 실제로 필요했던 것은 이미 있는 것을 읽는 열 줄이었다.

### 막고 있던 것은 시계였다

"확인됐는가"는 두 정수의 비교로 알 수 있었지만 **"얼마나 오래 확인되지 않았는가"는 잴 값이
없었다.** `updated_at`으로는 잴 수 없다 — 관측 보고를 포함해 행을 건드리는 모든 쓰기에서
움직이므로, 명령을 집어가지 않는 Worker라도 heartbeat이 오는 동안 미확인 시간이 영원히 0에
가깝게 유지된다. **정확히 탐지하려는 상황에서 탐지가 되지 않는 지표다.**

`042`가 `agents.command_issued_at`을 만들고 세대가 실제로 오르는 그 자리에서만 함께 찍는다.
세 UPDATE 중 둘이 조건부 CASE라 시각도 같은 CASE에 넣었다 — 조건이 갈리면 값을 바꾸지 않은
호출이 시계를 되감는다. 이것은 `#67` 4b가 `command_control_epoch`를 같은 CASE에 넣은 것과
같은 이유이며, 그 주석이 이미 적어 둔 함정이다.

### 재발행하지 않는 것이 요구사항이다

게이트 문언의 "자동 중복 실행 없이"가 요구하는 자리가 정확히 여기다. 확인이 없다는 것은
"도달하지 않았다"가 아니라 **"모른다"**이고, 모르는 상태에서 행동을 반복하는 것이 정확히
중복 실행의 경로다 — Worker가 첫 명령을 늦게 집어가면 같은 Agent가 두 번 뜬다.

그래서 스윕이 하는 일은 감사 한 줄뿐이다. 시험 하나가 **세대가 오르지 않는지**를 단정한다:
세대가 오르면 그것이 곧 재발행이므로, 그 단정이 이 스윕의 안전성 전체를 지킨다.

### 042 이전 행

`command_issued_at`이 `None`인 행은 잴 근거가 없다. `None`을 "오래됐다"로 읽는 구현은
마이그레이션 직후에 모든 미확인 Agent를 한꺼번에 쏟아낸다 — 부정 단정으로 고정했고 구별력도
실측했다.

마이그레이션이 기존 미확인 행을 `updated_at`으로 채우는 것은 `041`의 NULL 고수와 모순되지
않는다. 저기서 채울 뻔한 값(세션 id)은 **추측**이었지만 이 `updated_at`은 증명 가능한
**상한**이다 — 명령이 그 행을 마지막으로 손댄 시각보다 나중에 나갔을 수는 없다. 상한으로
재면 판정이 늦어질 뿐 거짓 양성이 생기지 않는다. 같은 규칙이 정반대 결론을 내는 자리라
마이그레이션 주석에 그 이유를 적었다.

### 하네스

postgres가 내려가 있어 store 통합 시험이 두 번 타임아웃했다. 실패가 아니라 **연결 대기**라
출력이 비어 있었고, `pg_isready`로 확인한 뒤 올리니 6.8초에 50건이 통과했다. 조용한 실패의
또 다른 얼굴이다 — 이쪽은 개수도 시간도 아니라 **출력 자체가 없는** 형태다.

---

## 2026-09-18 — ingest — 버려지던 유일한 증거 (`#61` 4단계)

`#70`의 남은 게이트가 전부 게이트 4의 의도적 보류 하나에 걸려 있어 그 항목 밖에서 다음을
골랐다. `#61`의 대기 사유를 읽고 코드와 대조했더니 **또 낡아 있었다.**

### 네 번째다

로드맵 `#61` 행은 "3~5단계는 heartbeat 없이 명령을 전달할 별도 control-stream/bounded poll
인프라(`#67` 영역, 미구현) 없이는 켤 수 없다"고 적고 있었다. **3단계의 probe는 2026-09-06에
이미 들어왔다** — `WorkerTransport::probe`가 `session/list`를 왕복시키고
`WorkerSelector::select`의 8단계가 `on_demand` 승자에게 그것을 건다. 귀속도 틀렸다: probe는
`#67`이 아니라 `#70` 게이트 5 소관이고, 그 정정 자체가 게이트 5 행에 이미 적혀 있었다.

이번 주 같은 모양이 네 번이다 — `#95`, `#64`, `#70` 게이트 2·3, 그리고 이것. 앞의 셋은
"있는데 쓰이지 않는다"였고 이번 것은 **"다른 항목이 이미 해결했는데 이 항목의 행이 모른다"**
이다. 항목 사이를 넘나든 해결은 양쪽 행을 다 고쳐야 하는데, 고치는 쪽은 자기 행만 고친다.

### 설계 문서의 세 군데가 어긋나 있었다

정본 [Worker liveness](./architecture/worker-liveness-policy.md)는 (1) 3단계가 아직 없다고
하고, (2) `WorkerStatus::Unchecked` 도입을 3단계의 **선행 조건**이라 부르고, (3) 3단계 완료
전까지 API가 `on_demand` 등록을 `422`로 거절한다고 적는다. 셋 다 지금 트리와 다르다:
probe는 있고, `Unchecked`는 **폐기된 대안**이며(probe 없이는 빠져나올 수 없는 도달 불가
상태가 된다), `422` 거절은 **구현된 적이 없다**.

세 번째가 특히 고약하다. 코드에 있던 것은 정확히 그 문단 자신이 "게이트가 아니다"라고 적은
그것 — worker.toml 주석 한 줄 — 이었다. **문서가 안티패턴을 이름으로 지목했는데 그 안티패턴이
그대로 배포됐다.**

### 막던 것은 수단이 아니라 버려지던 사실이었다

4단계는 "`last_activity_at`과 probe 결과를 기록한다"이다. 여기에 필요한 수단은 전부 있었다.
`on_demand` 워커는 heartbeat을 보내지 않아 `last_seen`이 join 시점에 멈추므로 dispatch 직전
probe가 그 워커에 대해 얻을 수 있는 **유일한** 생존 증거인데, 그 결과가 `selector.rs`의
`debug!`/`warn!` 한 줄로 끝나고 아무 데도 남지 않았다. 운영자 화면은 등록 이후로 영영
갱신되지 않은 값을 보여 준다.

`#70` 게이트 2가 "이미 도착한 답을 읽지 않았다"였다면 이것은 그 거울상이다 — **얻어 놓고
버렸다.**

### `last_seen`을 재사용하지 않은 이유

그 컬럼의 뜻은 "마지막 **하트비트** 수신 시각"이고 `HealthChecker`와 `Reconciler`의 offline
유예 판정이 전부 그 뜻에 기댄다. probe 성공을 거기에 적으면 하트비트를 보내지 않는 워커가
보낸 것처럼 보이고, `offline_worker_grace`가 조용히 뜻을 잃는다. 컬럼 하나를 아끼려다 판정
둘을 망가뜨리는 교환이다. store 시험이 두 시계가 갈리는지를 두 백엔드에서 단정한다.

**왕복 시간은 남기지 않았다.** 그것은 metric의 축이고, 워커별 지연을 행에 적으면 읽는 쪽이
그 값으로 판정하려 든다. 여기서 필요한 사실은 "언제 답했는가" 하나다.

**043은 backfill하지 않는다.** `042`가 기존 행을 `updated_at`으로 채운 것과 반대 결론인데,
저기서 채운 값은 같은 사실의 증명 가능한 **상한**이었고 여기서 채울 `last_seen`은 **다른
사실**이기 때문이다. 같은 규칙이 이틀 사이에 정반대 결론을 냈고, 두 마이그레이션 주석이 각각
그 이유를 적는다.

### 하네스가 가릴 뻔한 것

selector 시험의 `MockStore`는 `record_worker_activity`의 트레이트 기본 구현(`Unsupported`)을
그대로 쓰고 있었다. selector는 기록 실패를 로그로만 남기고 진행하므로, 기본 구현을 그대로
두면 **기록하지 않는 구현도 모든 시험을 통과한다** — 그 자리가 정확히 이 증분이 닫으려는
구멍이다. mock에 구현을 넣고 나서야 단정이 의미를 가졌다.

### 검증 한계

5단계(1,000 idle worker가 heartbeat 요청 0건)는 미착수다. 그 나머지 절반(죽은 워커가 probe
실패 뒤 dispatch되지 않음)은 게이트 5 시험이 이미 덮는다. 그리고 `placement.rs`는 여전히
`on_demand` 워커를 Agent 배치에서 뺀다 — 근거가 liveness가 아니라 모드 계약이라 probe로
풀리지 않는다. **Task dispatch는 되고 Agent 배치는 안 되는 것**이 지금 이 모드의 정확한
경계이며, 그 문장을 worker.toml 주석에도 그대로 적었다.

---

## 2026-09-21 — ingest — 강제되지 않던 "required" (`#65` 게이트 1)

`#70`의 남은 게이트가 전부 게이트 4의 의도적 보류에 걸려 있고 남은 P1 대부분이 인프라
(PKI·container runtime·effect ledger)에 막혀 있어 P2로 옮겼다. 셋 중 `#65`를 골랐고,
착수 전에 정본을 코드와 대조했다.

### 정본과 코드가 정면으로 어긋나 있었다

[하네스 구성](./architecture/agents/harness-composition.md)의 조립 규칙은 이렇게 적는다 —
"필수 Skill은 시작 전에 revision/hash까지 고정하고 **누락 시 실행을 거절한다**."

`skill_loader.rs`는 정반대였다. 독스트링에 **"soft-fail"**이라고 적혀 있었고, 파일이 없으면
`warn!` 한 줄을 찍고 그 스킬을 건너뛰었다. 그래서 `skills_required`에 보안 감사 스킬을
선언한 Task가 그 스킬 없이 실행됐고, 운영자에게는 아무 신호도 가지 않았다.

**이름이 이미 답을 적고 있었다.** 필드 이름이 `skills_required`인데 required가 강제되지
않았다. `#64`의 `AgentTemplateBody.tools`가 "저장되고 표시되지만 읽어서 제한하는 코드가
없었다"와 같은 모양이고, 그 항목이 적어 둔 문장이 여기에도 그대로 적용된다 — **강제되지
않는 allow-list는 없는 것보다 위험하다. 통제처럼 보이기 때문이다.**

### 반환형이 거절을 원천 차단하고 있었다

`inject_skills(prompt, skills) -> String`. 이 시그니처에는 **누락을 실을 자리가 없다.**
호출부가 아무리 거절하고 싶어도 무엇이 빠졌는지 알 방법이 없고, 애초에 빠졌다는 사실조차
받지 못한다. 그래서 고칠 것은 정책이 아니라 **타입**이었다 —
`SkillInjection { prompt, missing }`.

거절 자체는 로더가 하지 않는다. 거절은 Task 상태 전이를 동반하므로 `Dispatcher`의 일이고,
로더는 파일시스템만 아는 순수 조립기로 남는다.

### 조립을 CAS 앞으로 옮긴 것이 절반이다

예전 조립 지점은 **CAS 뒤**(6단계 transport dispatch 직전)였다. 거기서는 거절하려 해도
Task가 이미 `Dispatched`다 — 상태만 옮겨 두고 아무도 소유하지 않는 실행이 남는다.

조립을 한 번만 하는 것도 의도다. "먼저 존재를 확인하고 나중에 읽는다"로 나누면 그 사이에
파일이 사라질 수 있고, 그러면 확인을 통과한 Task가 스킬 없이 나간다 — 확인이 무의미해지는
창이다. 이번 주 `#70`에서 배운 것과 같은 모양이다.

### 조립 지점이 둘이었다

`fleet task submit --skill X`가 **제출 시점에** 주입한 프롬프트를 저장하면서
`skills_required`도 함께 저장했다. dispatch가 그 목록을 보고 **다시** 주입하므로, CLI로
제출한 Task는 같은 스킬 본문을 두 번 싣고 `<TASK>`가 중첩된 프롬프트로 워커에 갔다.

정본이 "**실행 직전에** 하나의 immutable snapshot으로 조립한다"고 적은 이유가 이것이다.
제출 시점에 구우면 (1) 그 뒤 스킬 파일이 바뀌어도 낡은 사본이 실행되고, (2) 저장된
`prompt`가 사용자가 쓴 것과 달라져 재현 가능성이 깨진다. 조립 지점을 dispatch 하나로 모았다.

`fleet-worker`에도 거의 같은 로더가 하나 더 있었다. **호출부가 한 곳도 없었고**(grep 0건)
주석과 soft-fail 동작만 미묘하게 달랐다. 지웠다 — "required"의 정의가 둘이면 고친 쪽이
무의미해진다.

### `SkillMissing`을 새로 만든 근거

`InvalidRequest`와 나눈 것은 **해소 방법이 다르기 때문**이다. 저쪽은 요청 자체가 규칙을
어겨 요청을 고쳐 다시 제출하는 길밖에 없지만, 이쪽의 요청은 멀쩡하다 — 오케스트레이터에 그
스킬 파일이 없을 뿐이고 배포하면 같은 요청이 통과한다. `CredentialMissing`이 같은 근거로
`WorkerUnavailable`에서 갈라진 것과 정확히 같은 계열이다.

**브레이커에 기록하지 않는다.** 워커는 이 요청을 본 적이 없고, 기록하면 클라이언트가 같은
Task를 반복 제출하는 것만으로 멀쩡한 워커의 회로를 열 수 있다 — `#69`가 `InvalidRequest`를
면제하며 "검증을 추가하면서 새 DoS 경로를 만드는 셈"이라고 적은 그 함정이다. 부정 단정으로
고정했고 구별력도 실측했다.

### 하네스

`MockTransport`가 받은 dispatch 요청을 기록하도록 고쳤다. 조립 결과(프롬프트)는 그 요청
안에만 있고 어디에도 저장되지 않으므로, 적어 두지 않으면 "무엇을 보냈는가"를 단정할 방법이
없다. 이번 주에 `cancel`에서 같은 이유로 같은 것을 했다.

env var(`FLEET_SKILLS_DIR`)를 쓰는 시험 셋은 `--test-threads=1`에 기댄다. CI와 로컬 게이트가
둘 다 그 플래그를 쓰므로(§agent.md 3.2) 이 파일 안에서는 안전하다.

### 검증 한계

게이트 2(revision 재현)는 열려 있다. 스킬의 revision/hash를 남길 자리가 없다 — `tasks`에
execution snapshot 컬럼이 없고 지금 남는 것은 `skills_required`라는 **이름 목록**뿐이라
"그때 무엇이 실행됐는가"를 재구성할 수 없다. 게이트 3(prompt-injection)은 스킬 본문 안의
`</SKILL>`를 이스케이프하지 않지만, 그 파일들은 운영자가 배포하는 것이라 오늘의 위협 모델
에서 신뢰 경계 안이다 — 사용자 입력이 스킬이 되는 경로가 생기면 그때 강제가 필요하다.
게이트 4는 무재시도 정책(`#97`) 아래에서 가를 두 번째 실행이 없어 사실상 해당 없음이다.

---

## 2026-09-22 — ingest — 이름은 신원이 아니다 (`#65` 게이트 2)

어제 게이트 1(필수 Skill 누락 거절)을 닫으면서 게이트 2가 "스킬의 revision/hash를 남길
자리가 없다"로 막혀 있다고 적었다. 그 자리를 만들었다.

### `skills_required`는 이름 목록이다

이름은 그때 무엇이 실행됐는지를 말해 주지 않는다. 같은 `security-audit`라도 어제의 본문과
오늘의 본문이 다르면 다른 실행이고, 그 파일은 오케스트레이터의 디스크에 있어 **언제든
바뀐다**. 사고가 났을 때 "그 Task는 어떤 감사 규칙으로 돌았는가"를 물으면, 지금까지는
답할 수 없었다.

`044`의 `tasks.skill_snapshot`이 조립 시점에 실제로 주입된 본문의 sha256을 고정한다.

### 무엇을 재는가가 설계의 절반이다

**frontmatter를 뺀 뒤의 본문**을 잰다. 그것이 실제로 프롬프트에 들어간 바이트이기 때문이다.
파일 전체를 재면 메타데이터만 바뀌어도 다른 실행으로 보이고, "같은 입력이었는가"라는
질문에 거짓으로 답한다. 시험 둘이 이 둘을 각각 고정한다 — 본문이 바뀌면 해시가 달라지고,
frontmatter만 바뀌면 같다.

본문 자체를 담지 않았다. 주입된 본문은 `tasks.prompt`에 이미 있고(중복), Skill이 커지면
Task 행이 그만큼 커진다. 해시는 "같은가"를 답하기에 충분하고, "무엇이었는가"는 그 해시를
가진 파일을 찾는 문제다.

### 쓰기 지점과 거절 지점이 서로 반대다

게이트 1의 **거절**은 CAS **앞**이어야 한다 — 차지한 뒤에 거절하면 아무도 소유하지 않는
실행 상태가 남는다. 게이트 2의 **기록**은 CAS **뒤**여야 한다 — 차지하지 못한 Task의
조립을 기록하면 실행하지 않은 조립이 "그때 실행된 것"으로 읽힌다.

같은 함수 안에서 두 지점이 갈리고, 갈리는 이유가 정확히 반대다. 이 대칭을 주석에 적어
뒀다. 한쪽만 보고 다른 쪽을 옮기면 조용히 깨지는 종류다.

### `NULL`과 `[]`

앞은 "조립 기록이 없다"(044 이전 행, dispatch된 적 없는 행), 뒤는 "조립했고 Skill이
없었다"이다. 접으면 이 게이트가 답하려는 질문이 "dispatch 됐는가"와 섞인다.

`042`에서 `command_issued_at`의 `None`을 "오래됐다"로 읽지 않기로 한 것과, `043`에서
backfill하지 않기로 한 것과 같은 계열이다 — **없는 것과 비어 있는 것은 다르다**가 이번 주
세 번째로 설계를 가른 자리다. 두 백엔드 시험이 JSONB 왕복에서 그 구분이 살아남는지를
단정한다.

### 검증 한계

게이트 3(prompt-injection)은 남아 있다. 스킬 본문 안의 `</SKILL>`를 이스케이프하지 않으므로
본문이 자기 블록을 닫고 나올 수 있다. 다만 그 파일들은 운영자가 오케스트레이터에 배포하는
것이라 오늘의 위협 모델에서 신뢰 경계 **안**이다 — 사용자 입력이 스킬이 되는 경로(예: Agent가
스킬을 제출하는 표면)가 생기면 그때는 강제가 필요하고, 그 전에 여는 것은 없는 위협에
코드를 쓰는 일이다.

게이트 4(재시도 snapshot 동일성)는 무재시도 정책(`#97`) 아래에서 가를 두 번째 실행이 없어
사실상 해당 없음이다. 게이트 2가 닫혔으므로 그 칸은 이제 "재시도가 생기면 이 스냅샷이 그
비교의 기준이 된다"로 읽으면 된다.

---

## 2026-09-23 — query — 보류는 옳았고, 하한이 없었다 (effect ledger 문헌 대조) + jev 설치 절차 공유

**분류**: `query`(문헌 조사 결과를 근거 문서로 파일링) + `ingest`(도구 설치 절차 정본화)

### 무엇을 물었는가

게이트 4(`#70`)는 2026-09-14에 **의도적 보류**로 판정됐다 — "증명할 비가역 부작용이 아직
없으므로 원장이 증명할 대상 자체가 없다". 그 판정은 **우리 코드만 보고** 내린 것이었다.
같은 모양의 문제를 다룬 문헌이 이 판단을 지지하는지 확인하지 않은 채였다.

5개 각도 · 22개 소스 · 109개 주장 추출 · 25개 적대적 검증 → **12 확정 / 13 기각** → 10개
finding. 전문은 [effect ledger 설계의 문헌 대조](./reviews/effect-ledger-research-2026-09-23.md).

### 보류는 유지된다. 빠져 있던 것은 **하한**이다

Azure Compensating Transaction 패턴은 원장을 인프라의 **하드 전제조건**으로 규정하지만,
그 전제는 *증명·보상 대상 effect가 존재할 때* 발동한다. 그래서 보류 자체는 문헌과
모순되지 않는다.

빠져 있던 것은 **기록이 외부화보다 늦으면 그 건은 영구 미증명 잔여가 된다**는 조건이다.
원장은 첫 비가역 effect를 도입하는 **바로 그 변경과 동시에** 존재해야 하고, "나중에
소급해서"는 선택지가 아니다. 정본의 「재개 조건」이 이미 **먼저**라고 적고 있어 이 하한보다
강했으므로 문언은 그대로 두고, 그것이 합의된 원칙이지 측정된 결론이 **아니라는** 사실만
덧붙였다 — 사전 도입과 소급 도입의 비용 비교 증거는 **어떤 소스에도 없었다.**

### 설계가 바뀐 자리 — `Cancelled`는 "아무 일도 없었음"이 아니다

1987 Sagas 원문이 직접 적는다: *"if a transaction fires a missile, it may not be possible
to undo this action"*. 그리고 처방이 undo가 아니라 **추가 실세계 행위**다 — 편지를 보상하려면
사과 편지를 한 통 **더** 보내고, 수표를 보상하려면 은행에 지급정지를 **보낸다**. Azure도
같다: *"doesn't necessarily return the system data to its state at the start"*.

즉 보상으로 도달한 종점은 "흔적이 없음"이 아니라 **"흔적이 둘 남았음"**이다. 우리
`CancelUnconfirmed`의 출구는 `Cancelled` 하나뿐이었고, 그 한 이름이 성격이 다른 두 결말을
담고 있었다. 정본에 미결로 올렸다 — `Cancelled`는 **`Started` effect 없음이 증명된
경우에만**, 보상 성공은 `Compensated`로. 증명의 출처가 원장이므로 이 분리는 원장을
도입하는 변경이 함께 들고 온다.

### 우리 상태 기계가 옳다는 확인 둘

`PartiallyApplied`는 제거할 결함이 아니라 **원리적 경계 조건**이다 — 이기종 외부
엔드포인트의 원자 커밋은 tool 쪽 TCC/2PC를 요구하는데 임의 REST·SMTP·syscall이 그것을
주지 않으므로 Saga·Temporal·SAFEFLOW·OCC가 **전부 같은 벽**에 멈춘다. Helland(CIDR 2007)는
불확실성을 인프라가 아니라 **애플리케이션의 파트너별 durable 상태로 reify**하라고 적는다 —
`OutcomeUnknown`/`PartiallyApplied`/`CancelUnconfirmed`는 결함이 아니라 설계다.

단 Helland는 **협조적 파트너**를 전제한다. 우리 상대(메일 provider, git remote)는 activity를
들지 않으므로, 이 문헌은 "durable 상태로 만들라"까지만 주고 **해소 프로토콜은 주지 않는다.**

### 기각이 확정만큼 많았다 — 그리고 그 기각이 우리를 겨눈다

**"멱등 키를 호출부가 아니라 제어면이 파생해야 한다"는 가설의 근거가 하나도 살아남지
못했다.** Atomix(adapter-defined) · IETF 초안(client 생성) · Stripe(클라이언트 생성) ·
Helland(수신 엔티티 dedup) 네 갈래 전부 0-3 기각. 확인된 업계 표준은 오히려 **호출부 생성 +
수신자 소유 dedup**이다.

정본의 HMAC 파생식은 제어면이 키를 만든다. **기각은 "반대가 참"이 아니라 "근거 미확보"**
이므로 설계를 뒤집지 않지만, 재개할 때 그 결정은 **인용이 아니라 우리 제약(무재시도·관측
전용)으로부터의 자체 논증으로** 서야 한다. 이것을 적어 두지 않으면 다음 세션이 "문헌이
그렇게 한다"로 정당화하게 된다 — 문헌은 그 반대를 말한다.

### 원장이 우리에게 무엇이 될 수 없는가

문헌의 예방 레버는 거의 전부 **제어면이 effect를 발행·게이팅한다**는 전제 위에 있다. 이
오케스트레이터는 도구 호출을 사후 알림으로 **관측만** 한다. 그러므로 우리 원장은 예방
장치가 아니라 **탐지·증적·에스컬레이션 장치로만** 성립한다. 막는 것은 `#64`의 allow-list
쪽에 남는다. 이 구분을 잃으면 원장을 만들어 놓고 막지 못하는 것을 막았다고 읽게 된다.

### 남은 공백 — 숫자가 전부 미답이다

비terminal 상태가 **무한정 쌓이지 않게 하는 구체적 장치**(체류 타임아웃, alert 임계,
에스컬레이션 SLA, 잔여 큐 상한)는 어떤 소스도 주지 않았다. 확보된 증거는 "사람에게 올린다"
까지이고, 1987 Sagas는 오히려 수리 완료까지 pending이 **무한정** 유지된다고 적는다.
`OutcomeUnknown`/`CancelUnconfirmed`가 이 저장소가 이미 밟아 본 "출구 없는 상태" 함정을
피하려면 그 수치를 **우리가** 정해야 한다. 저장 비용·보존 정책·고카디널리티 상관 키도 같다 —
유일한 스키마 문헌(SagaLLM)은 전체 페이로드와 LLM 추론 체인을 불변 로그에 담으면서 비용을
한 줄도 다루지 않는다.

### 읽을 때의 한계도 함께 남긴다

Atomix의 80% 누출률은 **동료심사 전 preprint의 자체 벤치마크**다 — 저자들이 직접 재구현한
베이스라인이지 실제 Temporal 배포 측정치가 아니고 독립 재현이 없다. **질적 결론은 강하고
숫자는 약하다.** Azure는 처방적 지침이지 현장 보고가 아니며, MS가 effect ledger를 승인했다고
읽으면 과대 인용이다. 그리고 **우리 조합(사후 관측 전용 + 무재시도 + 비협조 provider)을 직접
다룬 1차 문헌이 수집분에 없다.**

### jev — 공유한 것은 설정이 아니라 절차다

설계 결정을 기록·평가하는 `jev` MCP 서버를 팀이 쓸 수 있게 올렸다. **올리지 못한 것은
`.mcp.json`이다** — 이 저장소는 그 파일을 의도적으로 `.gitignore`에 두고 있고, 사유가 그
자리에 적혀 있다: 같은 파일이 이 머신에서 프로덕션 orchestrator 호스트로 가는 SSH 접근
경로를 함께 담는다. jev 항목 하나 때문에 그 무시를 풀면 다음 `--scope project` 등록이 호스트
경로를 추적 대상 설정으로 되돌린다.

`.claude/settings.json`에 서버 정의를 넣는 우회도 없다 — Claude Code 2.1의 settings 스키마에는
`mcpServers` 키가 없고, 있는 것은 `.mcp.json`을 **참조**하는 `enabledMcpjsonServers` 계열뿐이다.

그래서 저장소가 공유하는 것은 설정 파일이 아니라 **절차**다: `scripts/setup-jev-mcp.sh`가
각 머신에서 local scope로 등록하고, [jev 설치와 사용 경계](./engineering-patterns/jev-decision-records.md)가
왜 그 모양인지와 **무엇이 머신 밖으로 나가는지**를 적는다. `TYPESAFE_API_KEY`가 있으면 결정의
제목·맥락·선택지·근거가 `api.typesafe.ai`로 나가므로, 시크릿·고객 데이터·내부 호스트명이 들어간
결정은 제출하지 않는다. 키는 어떤 설정 파일에도 쓰지 않고 환경에서만 읽는다.

상류 README와 코드의 스키마가 다르다는 것도 함께 적었다(README `primitive`/`instruction`/
`options` vs 코드 `type`/`instructions`/`criteria`, `SystemOneClient` vs `JevClient`). **코드
쪽이 맞다** — README를 보고 직접 호출을 짜면 400으로 튕긴다.

### 이 변경의 게이트

Rust 소스를 한 줄도 건드리지 않았다(문서 5건 + 신규 셸 스크립트 1건). `cargo` 게이트는
이 트리에 대해 판정할 대상이 없으므로 돌리지 않았고, 대신 `bash -n scripts/setup-jev-mcp.sh`로
문법을 확인했다. **이것은 게이트를 건너뛴 것이 아니라 적용 대상이 아니라는 판정이며,**
다음 Rust 변경은 `agent.md` §4.3 전체를 그대로 받는다.
