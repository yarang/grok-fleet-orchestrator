...
`#70` 게이트 ③의 orphan 절반을 닫았다. 시작은 정본의 차단 사유를 그대로 믿지 않은 것이었다.
...
`sweep_stale_incarnation()`(`#70` 게이트 ③)이 혹시 살아남은 자식을 걷는다. **`Ok(())`로 끝내면
...
## 2026-09-05 — lint: `#70` 게이트 ⑤의 사유는 유효했다, 그리고 그 자리의 함정
...
**이 세션에서 차단 사유를 네 번 재판정했고 결과가 갈렸다**: `#70` 게이트 ③은 틀렸고(고쳤다),
`#67` 게이트 ④는 선행이 아니라 미실행이었으며(닫았다), `#67` 게이트 ③·⑦과 이번 `#70` 게이트 ⑤는
...
## 2026-09-06 — `#70` 게이트 ⑤: `ping`이 되지 못한 그것을 만들었다 (ACP probe)
...
## 2026-09-06 — `#70` 게이트 ⑤: probe를 배선해 게이트를 닫는다
...
## 2026-09-06 — `#70` 게이트 ④: 차단 사유가 순환이었다 (도구 호출 관측 신설)
...
## 2026-09-06 — `#70` 게이트 ⑥ 선행: 제어면 결정이 처음으로 감사에 남는다
...
`audit_log.control_epoch`(마이그레이션 038)는 nullable이고, NULL이 두 경우다.
