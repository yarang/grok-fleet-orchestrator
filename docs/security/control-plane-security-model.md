...
| Cloudflare 전용 배포의 권한 | **완료(`#74`)** — principal→capability 매핑이 없으면 인증 통과 주체가 아무 capability도 받지 못한다(fail-closed). `--cf-principal-capabilities`/`FLEET_CF_PRINCIPAL_CAPABILITIES`로 매핑을 설정하며, `FLEET_CF_AUDIENCE`가 설정됐는데 매핑이 비어 있으면 기동을 거부한다 | 없음 — 완료 |
