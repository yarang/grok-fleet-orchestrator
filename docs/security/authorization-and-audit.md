...
>
> **2026-09-15 — 그중 신원 절반을 런처 주장 수준으로 메웠다.** `FLEET_MCP_PRINCIPAL`(선택)이
> 있으면 MCP가 만드는 리소스의 `created_by`가 `mcp:<principal>`이 되고, 없으면 기존과 같은
> `"mcp"`다. **이것은 인증이 아니다** — 런처가 env로 주장하는 값이고 신뢰 출처가
> `FLEET_MCP_CAPABILITIES`와 같다. 그래도 유효한 이유는 이 값으로 **권한이 생기지 않기**
> 때문이다: `created_by`는 인가 주체가 아니라 멱등성 스코프이자 선택적 조회 필터다(저장소에서
> 술어로 쓰이는 자리가 그 둘뿐임을 확인했다). 접두사 `mcp:`는 런처가 Dashboard 사용자의
