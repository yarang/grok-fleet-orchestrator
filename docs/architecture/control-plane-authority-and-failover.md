...
| 4. Primary 종료 뒤 수동 승격 · Worker 재연결 · pending reconciliation E2E | **부분** | 라이브 실행은 lease 인수인계만 관찰했다. 수동 승격 절차, Worker 재연결, reconciliation은 어느 것도 실행하지 않았다. **2026-09-05 — 셋 중 둘을 덮었다**: `fleet-scheduler/tests/lease_failover.rs`가 같은 Store 위에 실제 `LeaseManager` 둘을 띄우고, Primary를 `shutdown()`(정상 해제)으로 내린 뒤 Standby가 승격되는 것과, 그때서야 밀린 `Pending` Task가 재조정되는 것을 잇는다. 가짜 리스(`LeaseObserver::with_status`)를 쓰지 않는다 — 게이트가 묻는 것이 승격 자체이기 때문이다. **변이 시험이 설계를 하나 드러냈다**: 리스 게이팅은 `Reconciler::sweep`과 `Dispatcher::dispatch` 두 겹이고 **직렬**이라, 한쪽만 지우면 다른 쪽이 막아 관측 가능한 동작이 변하지 않는다(양쪽을 각각 지워 실측). 둘을 함께 지워야 승격 전에 Task가 `Failed`로 떨어져 시험이 붉어진다 — 이 시험이 고정하는 것은 특정 `if` 문이 아니라 "리스 없이는 밀린 일을 건드리지 않는다"는 성질이다. **남은 셋째**: Worker 재연결. 워커 프로세스와 API 표면이 함께 있어야 하고 그 골격은 `fleet-worker`의 `self_fencing_e2e`에 있으나, 스케줄러(리스·Reconciler)까지 한 시험에 합치는 조립은 이 증분 밖이다 |
