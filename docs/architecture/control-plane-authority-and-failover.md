...
| 4. Primary 종료 뒤 수동 승격 · Worker 재연결 · pending reconciliation E2E | **닫힘** | 라이브 실행은 lease 인수인계만 관찰했다. 수동 승격 절차, Worker 재연결, reconciliation은 어느 것도 실행하지 않았다. **2026-09-05 — 셋 중 둘을 덮었다**: `fleet-scheduler/tests/lease_failover.rs`가 같은 Store 위에 실제 `LeaseManager` 둘을 띄우고, Primary를 `shutdown()`(정상 해제)으로 내린 뒤 Standby가 승격되는 것과, 그때서야 밀린 `Pending` Task가 재조정되는 것을 잇는다. 가짜 리스(`LeaseObserver::with_status`)를 쓰지 않는다 — 게이트가 묻는 것이 승격 자체이기 때문이다. **변이 시험이 설계를 하나 드러냈다**: 리스 게이팅은 `Reconciler::sweep`과 `Dispatcher::dispatch` 두 겹이고 **직렬**이라, 한쪽만 지우면 다른 쪽이 막아 관측 가능한 동작이 변하지 않는다(양쪽을 각각 지워 실측). 둘을 함께 지워야 승격 전에 Task가 `Failed`로 떨어져 시험이 붉어진다 — 이 시험이 고정하는 것은 특정 `if` 문이 아니라 "리스 없이는 밀린 일을 건드리지 않는다"는 성질이다. **2026-09-05 — 셋째도 덮어 게이트를 닫는다**: `fleet-worker/tests/control_plane_failover_e2e.rs`가 같은 Store 위의 API 인스턴스 둘과 그 사이를 오가는 리스, 그리고 진짜 워커를 한 프로세스에 세운다. **워커의 설정은 한 글자도 바뀌지 않는다** — 워커와 오케스트레이터 사이의 TCP 링크에서 뒤쪽만 A에서 B로 바꾸므로(VIP·로드밸런서 뒤에서 인스턴스가 교체되는 모양), "재연결"이 설정 변경이 아니라 `last_seen`이 다시 오르는 **관측**이 된다. 변이 둘로 갈랐다 — 복구된 링크를 죽은 A로 되돌리면 재연결 단정이, 리스 게이팅 양쪽을 지우면 "승격 전에는 건드리지 않는다"는 단정이 각각 붉어진다. 3회 연속 3.2초로 통과한다. **검증 한계**: 밀린 Task를 그 워커에게 실제로 dispatch하지는 않는다 — 이 시험이 묻는 것은 새 Primary가 밀린 일을 집는가이지 그것이 어디로 가는가가 아니며, dispatch 경로는 `fleet-scheduler`의 `dispatch_e2e`가 덮는다 |
| 5. schema/binary 비호환 Standby 기동 거부 | **부분** | **schema 절반은 닫혔다** — 아래 "기동 호환성 게이트" 참고. **binary 버전 검사는 미착수** — 버전을 DB에 쓰는 생산자가 없다 |
| 6. partition 중 Worker self-fencing과 stale process cleanup E2E | **닫힘** | **2026-09-02: self-fencing 절반이 구현됐다 — 아래 문단들은 그 이전의 기록이고, 현재 상태는 맨 끝에 있다.** Worker self-fencing 미구현 — heartbeat이 실패해도 워커는 `warn!("heartbeat failed — will retry next interval")`만 찍고 Agent 프로세스를 무기한 유지한다(`crates/fleet-worker/src/registration.rs`). **다만 오늘의 노출 경로는 운영자뿐이다**(2026-09-01 실측): 오프라인 워커의 Agent를 자동으로 재배정하는 경로가 없어 — `assign_agent_worker`의 호출자는 Dashboard와 MCP 핸들러 둘뿐이고 `reconcile.rs`에는 Agent 처리가 아예 없다 — 중복 프로세스 창은 사람이 재배정을 실행할 때만 열린다. 이 게이트를 시간 기반 펜싱으로 대신 닫지 않는 이유는 아래 유예 표의 `Agent self-fencing` 칸에 있다. **2026-09-01 추가 실측**: 그 칸이 기록하듯 닫는 자리는 재배정 쓰기의 술어이고, 술어가 볼 사실은 4c-B의 인벤토리가 이미 저장하고 있다(`agents.observed_status`, 032). 남는 창은 프로세스 기동과 그것을 보고하는 다음 beat 사이의 한 주기다. **2026-09-01 구현(`#67` 게이트 ②)**: 그 술어가 `assign_agent_worker`의 UPDATE 안으로 들어갔다 — `worker_id = $2 OR observed_status IS NULL OR observed_status = 'failed'`. 즉 위에서 "오늘의 노출 경로는 운영자뿐"이라고 적은 그 경로가 닫혔다. **게이트 자체는 여전히 미착수다** — 워커는 지금도 heartbeat 실패에 프로세스를 유지하며, 닫힌 것은 오케스트레이터가 그 프로세스 위에 두 번째를 얹지 않는다는 것뿐이다. **2026-09-02 구현 — self-fencing 절반**: 워커가 마지막으로 성공한 heartbeat 이후 `worker.agent_fence_after_secs`(기본 300초)를 넘기면 자기 Agent 프로세스를 전부 멈추고(`AgentProcessManager::fence_all`), 연결이 돌아온 beat에 그 사실을 `agent_fenced` 필드로 실어 `agent.self_fenced` 감사로 남긴다. **이 구현의 근거는 위 문단이 적은 "중복은 이미 막혔다"에서 나온다** — 막을 것이 중복이 아니므로 이 유예는 안전을 사는 값이 아니라 **감독 없는 실행의 상한**이고, 그래서 오케스트레이터의 Offline 판정(기본 45초)보다 한참 길게 잡는다(짧게 잡아 얻는 안전은 이미 지불돼 있고, 잃는 것은 잠깐의 흔들림에 날아가는 진행 중인 작업이다). 동시에 게이트 ②의 술어를 **풀 수 있게** 만든다: `observed_status`를 지우는 유일한 경로가 그 워커의 다음 heartbeat이므로, 멈추고 재연결해야 비로소 그 Agent가 다른 Worker로 갈 수 있다. (**2026-09-04 정정 — 이 문장은 틀렸다.** 게이트 ⑥의 E2E(`fleet-worker::self_fencing_e2e`)가 실제로 돌려 보니 재연결은 관측을 **지우는 것이 아니라 되살린다** — 오케스트레이터가 여전히 그 Agent를 그 Worker에서 원하므로(`desired=running`) 워커가 곧바로 다시 띄우고 관측도 `running`으로 돌아온다. 배정이 그대로인 한 그것이 옳은 동작이다. 갇힌 Agent를 실제로 푸는 것은 운영자가 배치를 거두거나(`desired=stopped` → 워커가 멈추고 보고 → 관측 소거) Worker를 지우는 것(036)이고, 펜싱이 더하는 것은 그 판단에 붙는 **시간 상한**이지 술어의 해제가 아니다. 시험이 이 오류를 잡았다.) workspace는 지우지 않는다 — 배정이 유효하므로 지우면 단절이 작업 손실이 된다. **2026-09-03 정정 — 위 문단이 처음 적은 "남은 절반"은 틀렸다.** 거기서 "영영 돌아오지 않는 Worker의 Agent는 관측이 `running`으로 굳어 재배정이 영구히 막히고 그것을 푸는 경로가 이 증분에 없다"고 적었는데, **036이 정확히 그 경우를 위해 설계돼 있다** — `workers`의 `ON DELETE SET NULL`이 `worker_id`를 비우고 `agents_clear_placement_and_observation_trg`가 그 전이에서 관측 세 컬럼을 함께 지운다. `crates/fleet-store/tests/agents.rs`의 `deleting_the_worker_clears_the_observation_too`가 삭제 뒤 `assign_agent_worker`가 `SlotClaim::Claimed`를 돌려주는 것까지 증명한다. 즉 푸는 경로는 **Worker 삭제**이고, 이미 있고, 시험도 있다. 이 self-fencing이 그 경로에 무엇을 더하는지는 게이트 ② 행에 적었다. **2026-09-03 — 루프 자체의 죽음을 닫았다.** 펜싱이 이 루프 안에서만 일어나므로 루프가 죽으면 유예가 무의미해지는데, 그 죽음은 지금까지 보이지 않았다(`tokio::spawn` 후 종료 경로에서만 join). `await_shutdown`이 신호와 `JoinHandle`을 함께 기다려 신호보다 먼저 끝난 루프를 결함으로 분류하고, `shutdown_all()`로 Agent를 정리한 뒤 `Err`로 끝낸다 — `main`이 그것을 `ExitCode::FAILURE`로 옮기므로 감시자가 재기동하고, 재기동의 `sweep_stale_incarnation()`이 남은 자식을 걷는다. 시험 3건(패닉·조용한 조기 반환·살아 있는 루프는 깨우지 않음)이고 감시를 지운 트리에서 앞의 둘만 붉어지는 것을 확인했다. **덮지 못하는 것**: `JoinHandle`이 끝나지 않는 정지 — 데드락, 블로킹 호출에 굶은 런타임, `SIGSTOP`. 그것까지 잡으려면 진행 카운터를 **OS 스레드**가 감시해야 하고(`spawn`한 태스크는 굶은 런타임에서 같이 굶는다) 그 스레드가 취할 수 있는 조치는 `.fleet-agent.json`의 pid를 직접 죽이는 것뿐이다 — `process::abort`는 소멸자를 돌리지 않아 `kill_on_drop`이 자식을 죽이지 못한다. **2026-09-06 — 그 워치독을 만들었다**(`crates/fleet-worker/src/watchdog.rs`). 위 문장이 요구한 그대로다: 진행 비콘(heartbeat 루프가 매 바퀴의 **시작**에 올린다)을 전용 OS 스레드가 지켜보고, `worker.watchdog_stall_after_secs`(기본 600초, `0`이면 끔) 동안 움직이지 않으면 `.fleet-agent.json`의 pid를 동기적으로 죽인 뒤 종료 코드 `70`으로 나간다. 비콘이 heartbeat **성공**이 아니라 한 바퀴의 시작을 세는 것이 이 설계의 핵심 선택이다 — 성공을 세면 단절된 Worker가 곧 죽는데 그것은 이 게이트의 self-fencing이 훨씬 부드럽게 다루는 경우이고(프로세스만 멈추고 Worker는 살아 재연결을 기다린다), 워치독이 끼어들면 그 복구 경로가 사라진다. 즉 **파티션은 워치독의 대상이 아니다**(`an_unreachable_control_plane_still_counts_as_loop_progress`가 이것을 고정한다). `SIGSTOP`은 의도적으로 밖에 남긴다 — 프로세스 전체가 멈추면 워치독 스레드도 함께 멈추므로 비콘이 멈춘 것이 정상이고, 워치독은 **자기 잠의 초과**로 그 경우를 알아보고 기준점을 다시 잡는다. 판정이 애매하면 죽이지 않는 쪽으로 접는다: 놓친 굶주림은 오케스트레이터의 `Offline` 판정이 여전히 덮지만, 잘못 죽인 Worker를 되돌리는 것은 아무도 하지 않는다. **검증 한계**: 실제로 굶은 tokio 런타임이 종료 코드 `70`을 내는 것은 시험하지 않았다 — 런타임을 의도적으로 굶히려면 그러기 위한 코드가 프로덕션에 있어야 한다. 시험이 덮는 것은 판정 로직(잠 주입, 7건), pid 종료와 재사용 방어(진짜 자식, 4건), 그리고 진짜 루프가 비콘을 올리고 정상 종료가 해제한다는 것(2건)이다. **2026-09-04 — E2E가 생겼다.** `crates/fleet-worker/tests/self_fencing_e2e.rs`가 진짜 오케스트레이터를 in-process로 띄우고 그 앞에 TCP 링크를 두어, 링크를 끊는 것으로 파티션을 만든다. 한 시험이 배정→기동→관측 도달→파티션→유예 초과→프로세스 정지→(정지했어도 여전히 옮길 수 없음)→회복→`agent.self_fenced` 감사 도달→재기동으로 수렴까지를 잇는다. 펜싱을 끈 트리에서 붉어지는 것을 확인했고 3회 연속 14.4초로 통과한다. **오케스트레이터 태스크를 `abort`하는 방식은 파티션이 되지 못한다** — accept 루프만 끊기고 이미 맺어진 keep-alive 연결은 살아남아 heartbeat이 계속 성공한다. 그래서 연결 태스크를 `JoinSet`에 담아 함께 떨어뜨리는 링크를 쓴다. 이 시험이 위에 적힌 "게이트 ②의 술어를 풀어 준다"는 서술이 틀렸음을 잡아냈다. **2026-09-04 — 나머지 절반도 덮었다.** 같은 파일의 `a_hard_killed_worker_reports_the_surviving_agent_on_its_next_incarnation`이 워커의 거친 죽음(매니저를 `std::mem::forget`해 `kill_on_drop`이 발동하지 않게 만든다 — SIGKILL이 남기는 상태와 같다)부터 다음 incarnation의 sweep, 그리고 `agent.orphan_terminated`(`reason = stale_incarnation`)가 오케스트레이터에 닿는 데까지 잇는다. **기록 파일이 사라졌는지를 묻지 않는다** — sweep이 옛 기록을 지운 뒤 같은 beat의 `reconcile`이 같은 Agent를 다시 띄우며 새 기록을 쓰므로(배정이 그대로이니 옳은 동작이다), 물어야 할 것은 그 파일이 누구의 것인가이고 pid가 그것을 가른다. 첫 beat 전의 sweep을 지운 트리에서 이 시험만 붉어지는 것을 확인했다(self-fencing 시험은 초록 — 둘은 서로 다른 실패를 덮는다). **그래서 게이트를 닫는다.** 남는 것은 이 게이트가 묻는 것이 아니라 시험의 한계다: Store가 `MemStore`라는 것(관측 소거와 재배정 술어는 두 Store 모두에 있고 각각 `fleet-store`가 증명하므로 이 시험이 맡은 것은 그 둘을 잇는 순서이지 Store의 구현이 아니며, 덕분에 `DATABASE_URL` 없이 모든 잡에서 돈다), 그리고 `JoinHandle`이 끝나지 않는 정지(데드락·굶은 런타임·`SIGSTOP`)는 이 시험도 `await_shutdown`도 잡지 못한다는 것. 그리고 sweep이 살아남은 자식을 실제로 **죽였는지**는 E2E가 다시 보지 않는다 — 죽은 자식은 부모(시험 프로세스)가 거두지 않아 좀비로 남고 그 상태에서는 `kill -0`이 성공하므로 이식성 있는 생사 판정이 되지 못한다. 그 단정은 `Child` 핸들을 쥔 `fleet-worker::agent_process`의 단위 시험이 맡는다. **2026-09-04 — 그 부류의 실재하던 경로 하나는 닫았다**: `heartbeat_once`가 부르던 `detect_grok_version`이 타임아웃 없는 블로킹 `std::process::Command::output()`이라, `--version`에 답하지 않는 grok 바이너리 하나가 heartbeat 루프를 무기한 멈추고 그와 함께 펜싱도 멈췄다(그 정지는 `JoinHandle`이 끝나지 않아 `await_shutdown`도 잡지 못한다). `tokio::process` + 5초 타임아웃 + `kill_on_drop`으로 바꿨다. `spawn_blocking`을 쓰지 않은 이유는 그쪽이 취소되지 않아 타임아웃 뒤에도 블로킹 스레드가 자식과 함께 남기 때문이다. (2) **이것은 Agent 프로세스 전용이다.** `GrokRunner`(싱글턴 grok)는 heartbeat 루프와 독립이라 파티션된 Worker는 Agent가 멈춘 뒤에도 **Task는 계속 실행한다** — "Worker self-fencing"이라는 게이트 이름이 전체를 덮는 것처럼 읽히지만 여기서 덮은 것은 Agent 프로세스뿐이며, Task 쪽의 안전은 `#62`의 dispatch 세대 술어가 결과 반영을 막는 것으로 따로 서 있다 |
| 7. 동시 slot claim · ACK 유실 · Worker reincarnation에서 Agent 중복 process 없음 | **부분** | **slot claim은 Store의 두 진입점 모두에서 닫혔다** — `#67` 4a 후속(①-A-2)이 `workers` 행을 `FOR UPDATE`로 잠근 아래에서 다시 세고 조건부로 배정한다(`PgStore::create_agent`·`assign_agent_worker`, [Agent 프로비저닝](agents/provisioning.md)의 §"배정 슬롯 상한"). 잠금이 두 자리에 따로 걸려 있으므로 증명도 둘이다 — `fleet-store`의 `concurrent_creates_cannot_exceed_the_cap`과 `concurrent_placements_cannot_exceed_the_cap`이 각각 8-way 경합으로 증명하며, `FOR UPDATE`를 한 자리씩 지운 트리에서 각 3회 전부 붉어지는 것을 먼저 관찰했다. **API 표면이 저장된 사실을 따르는지는 별도로 닫았다** — Dashboard·MCP의 생성 핸들러가 선점 실패 시 응답(과 Dashboard의 경우 감사 로그)에서 배정을 거두는지를 `MemStore::dropping_placements()` 주입으로 결정적으로 증명한다(각 표면 GREEN 3/3, 되맞춤을 한 표면씩만 지우면 RED 3/3). 경합으로는 그 분기에 닿지 못한다 — MemStore에는 `.await` 양보점이 없어 2-way·8-way 각 12회 모두 0건이었다. 두 증명의 합성은 실검증에서 닫았다 — 살아 있는 서버에 8-way로 던져 배정 1건·되맞춤 2건·후보 없음 5건으로 갈렸고, 8건 전부 응답·감사 로그·저장된 행이 일치했다. **나머지 절반은 열려 있다** — `worker_execution_lease`가 없다. Agent 엔티티는 `#49` 1단계(2026-08-28)로 생겼으므로 이 칸의 "Agent 엔티티가 없다"는 더 이상 이유가 아니다. **reincarnation의 관측 절반은 `#67` 2단계(2026-08-29)가 닫았다** — `workers.incarnation_started_at`이 워커 재시작을 오케스트레이터 관측으로 기록하고 reconciler가 그보다 앞선 dispatch를 고아로 회수한다. 그러나 그것은 **Task 회수**이지 Agent process 중복 방지가 아니다 — 워커 안에서 살아남은 프로세스를 죽일 수단은 여전히 없다. **재배정 경로의 중복은 `#67` 게이트 ②(2026-09-01)가 닫았다** — 관측 술어가 UPDATE 안에 있어 `running`으로 보고된 Agent는 다른 Worker로 옮겨지지 않고 `SlotClaim::ObservedRunning`으로 거절된다. 슬롯 상한과 같은 자리에서 같은 방식(잠금 아래의 조건부 쓰기)으로 닫혔지만 증명은 다르다 — 상한은 경합으로, 이 술어는 관측을 심고 이동을 시도하는 결정적 테스트로 증명한다(`fleet-store`의 `running_observation_blocks_moving_to_another_worker` 외 5건, `MemStore` 병렬 6건). **ACK 유실은 여전히 열려 있다** — 게이트 ③의 몫이다 |
...
**함께 드러난 비용.** `#78`이 워커의 정상 종료에서 deregister를 의도적으로 제거했으므로(재기동 한 번에 `worker_operational_credentials`와 `worker_credentials`가 CASCADE로 사라져 register가 영구 401이 되던 결함, `crates/fleet-worker/src/runner.rs`) 오늘 `agents.worker_id`를 NULL로 만드는 경로는 관리자의 `DELETE /v1/workers/{id}` 하나다. 그런데 030의 `ON DELETE SET NULL`은 `worker_id`만 지우고 `observed_status`는 남기므로, 단절된 Worker를 지운 뒤에도 그 Agent들은 `running` 관측을 이고 있어 새 술어에 막힌다. 이것은 결함이 아니라 이 술어가 선택한 **안전 쪽 실패**였다. **2026-09-01에 결정했다 — Worker 삭제가 그 Worker의 Agent 관측도 함께 지운다**(마이그레이션 036). 삭제는 이미 자격증명을 CASCADE로 파괴하는 명시적 파괴 행위이므로 "이 Worker는 없다"는 운영자의 선언으로 취급한다. 대가는 정직하게 적는다 — 운영자가 틀렸다면(실은 살아 있는데 제어평면과만 단절) 중복 실행이 그대로 발생한다. 판단의 주체를 사람으로 옮긴 것이지 위험을 없앤 것이 아니다. (**2026-09-03 추가 — 그 위험에 시간 상한이 생겼다.** 게이트 ⑥의 self-fencing이 들어오면서, 제어면과 끊긴 Worker는 `worker.agent_fence_after_secs`(기본 300초)를 넘기면 자기 Agent 프로세스를 스스로 멈춘다. 따라서 운영자가 그 유예를 **넘겨 기다린 뒤** 삭제하면, "실은 살아 있는데 제어평면과만 단절"이었더라도 그 시점에는 이미 프로세스가 없다 — 위 문단이 사람에게 떠넘긴 판단이 그만큼 안전해진다. 운영 규칙으로 적으면: **`last_seen`에서 `agent_fence_after_secs`가 지난 뒤에 삭제하라.** 다만 이 추론은 펜싱이 실제로 일어난다는 것에 기댄다 — 펜싱은 heartbeat 루프 안에서만 일어나므로 루프가 사라지면 유예를 넘겨도 프로세스는 멈추지 않는다. **2026-09-03에 그 경로를 닫았다**: `await_shutdown`이 OS 신호와 루프의 `JoinHandle`을 함께 기다려, 신호보다 먼저 끝난 루프를 결함으로 분류하고 워커를 fail-closed로 내린다(panic과 조용한 조기 반환이 걸린다). 남은 구멍은 `JoinHandle`이 끝나지 않는 경우 — 데드락·런타임 굶주림·`SIGSTOP` — 이고, 거기서는 이 규칙이 성립하지 않는다. **2026-09-06에 그중 앞의 둘을 닫았다**: OS 스레드 워치독(`crates/fleet-worker/src/watchdog.rs`)이 heartbeat 루프가 매 바퀴 올리는 진행 비콘을 지켜보다가 `worker.watchdog_stall_after_secs`(기본 600초) 동안 움직이지 않으면 `.fleet-agent.json`의 pid를 동기적으로 죽이고 종료 코드 `70`으로 나간다. 따라서 이 운영 규칙은 그 부류에서도 성립하며, 기다릴 값이 `agent_fence_after_secs`가 아니라 `watchdog_stall_after_secs`로 바뀔 뿐이다. `SIGSTOP`은 여전히 밖에 있다 — 워치독은 자기 잠의 초과로 프로세스 전체의 정지를 알아보고 그 경우 **일부러 발동하지 않는다**(디버거를 붙인 멀쩡한 Worker를 죽이지 않기 위해서다).) 관측 삭제를 애플리케이션이 아니라 **트리거**에 두는 이유는 030이 `assigned_at`에 대해 적은 것과 같고 여기서 더 강하다: 운영자의 직접 `DELETE FROM workers`는 애플리케이션을 거치지 않으므로, `PgStore::delete_worker`에만 두면 그 경로가 stale한 `running`을 남겨 이 결정이 없애려는 바로 그 상태를 만든다. 036은 백필과 `agents_observation_requires_placement` CHECK도 함께 넣는다 — 트리거는 앞으로의 전이만 고치고, CHECK가 있어야 "트리거가 옳다"가 가정이 아니라 DB가 지키는 사실이 된다. **2026-09-02 정정: 트리거의 발동 조건은 Worker 삭제(`worker_id → NULL`)가 아니라 `NEW.worker_id IS DISTINCT FROM OLD.worker_id`, 즉 배치가 바뀌는 모든 경우다.** 처음엔 030의 `assigned_at` 조건을 그대로 재사용해 `NEW.worker_id IS NULL`에만 걸었는데, Worker A→B 이동은 양쪽이 non-NULL이라 발동하지 않아 **A의 관측이 B로 따라붙었다** — 관측은 특정 Worker가 특정 프로세스에 대해 한 진술이므로, 배치가 옮겨가면 그 진술은 무효인데도 "B가 시도하고 실패했다"로 읽혔다. `<>`가 아니라 `IS DISTINCT FROM`인 이유는 한쪽이 NULL이면 `<>`가 NULL을 주어 삭제 경로에서 거울상 버그가 생기기 때문이고, `assigned_at`이 030의 조건을 유지하는 이유는 같은 UPDATE가 `assigned_at = now()`를 쓰기 때문이다. 그리고 **같은 Worker로의 재배정에서는 관측을 남긴다** — 지우면 "현재 Worker로 재배정 → 관측 소멸 → 어디로든 이동"이라는 2단계로 이 술어가 뚫린다.)

**호출부의 `Stopped` 400 가드를 함께 걷었다**(2026-09-01). 술어가 생기면 살아 있는 Agent를 옮기는 유일한 안전한 순서가 "회수 → 프로세스가 죽고 관측이 비워짐 → 이동 → 재기동"이 되는데, 그 두 번째 걸음에서 `status`가 `stopped`다. 가드를 남기면 어떤 순서로도 이동이 불가능해지고, 게이트 ②는 이동을 안전하게 만든 것이 아니라 **없앤 것**이 된다. 가드의 원래 근거("원장이 세지 않으니 아무도 읽지 않는 값을 쓰는 것이 된다")도 이미 참이 아니다 — `list_agent_commands`는 미확인(`last_acked_generation < command_generation`)인 동안 `stopped` 행도 싣는다. 상한을 우회하지도 않는다: 배정 시점의 계산은 지나가지만 실제 기동은 워커가 자기 상한으로 막고 `failed`/`cap_reached` 관측으로 되돌려 준다. **시간 기반 펜싱으로 대신하지 않는다** — "N회 연속 heartbeat 실패"는 "나는 낡았음을 증명할 수 있다"가 아니라 "나는 아무에게도 닿지 못한다"라서 술어가 다르고, 경쟁자가 없는 단절에서도 멀쩡한 프로세스를 죽인다. 4c-B가 `None`(권위 있는 목록 없음)과 `Some(vec![])`(비우라는 명령)을 구분한 판단을 정면으로 뒤집는 셈이기도 하다 | `#49` 2단계 |
...
임의로 정하는 셈이다. 그래서 관측만 만들고 강제는 미룬다 — `#70` 게이트 3이 orphan을 막기
