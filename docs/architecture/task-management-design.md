...
flowchart LR
    ...
    Attempt --> Agent["eligible Agent / Worker"]
...
현재 구현의 `Task`는 `Pending`, `Dispatched`, `Completed`, `Failed`, `Cancelled` 상태를 가진다. `tasks.project_id` 저장 컬럼과 `ProjectId` 타입은 준비되어 있지만 Project 엔티티·FK·`TaskRequest.project_id` 전달은 아직 구현되지 않았다.
...
모든 제출, dedupe 결과, dispatch 결정, 취소 요청/확정, terminal 결과에는 actor, request id, Task id, Project id, policy revision, Attempt generation을 감사 이벤트로 남긴다.

## 구현 순서와 검증 게이트

1. Project 테이블·상태·TaskRequest의 `project_id` 전달을 함께 구현한다.
