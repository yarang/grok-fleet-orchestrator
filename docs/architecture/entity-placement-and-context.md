...
가진 논리 엔티티이고, Agent process는 TaskAttempt를 실행할 때만 Worker가 시작하는 일시적
...
| TaskAttempt → Worker/Agent | Attempt는 실제 `worker_id`를 항상 기록하고, Agent를 사용할 때 `agent_id`와 execution snapshot을 함께 기록한다. |
...
`Agent.max_concurrent_attempts = 1`이며, Worker의 `max_agent_processes`가 전역 실행 상한이다.
...
기본 정책은 **ephemeral**이다. TaskAttempt가 terminal이면 Agent process는 종료하고 Agent는
...
`warm_idle_ttl` 안에서 process를 유지한다. WarmIdle process에는 TaskAttempt가 없고, terminal 때
모든 credential delivery grant와 attach grant를 먼저 회수한다. process memory는 재사용 최적화일
뿐 durable context의 정본이 아니다.

WarmIdle 재사용은 새 Attempt의 runtime/image digest, isolation class, workspace identity, Tool/Skill
...
WarmIdle, (3) 가장 오래 idle인 WarmIdle, (4) 필요 시 새 Task를 Pending으로 남긴다. Running
Attempt를 WarmIdle보다 우선해 강제 종료하지 않으며, 다른 Project의 context를 재사용하지 않는다.
...
| Tool/Skill/runtime/isolation execution snapshot | Attempt 감사 레코드로 유지 |
...
flowchart LR
    ...
    Request --> Snapshot["TaskAttempt snapshot\nid + revision + hash"]
```

- Project deny 또는 capability 부족은 Agent template, Task 요청, 사용자 입력으로 다시 허용할 수 없다.
- 필수 Tool/Skill은 Project grant와 Agent binding 모두에서 허용되어야 하며, 누락하면 Attempt를 시작하지 않는다.
- 선택 Tool/Skill은 Task가 명시 요청한 허용 항목만 snapshot에 넣는다.
- catalog·grant·binding을 수정해도 이미 실행 중인 Attempt의 snapshot은 변하지 않는다.
...
stateDiagram-v2
    [*] --> Ready: "Agent created"
    Ready --> Starting: "attempt assigned"
    Starting --> Running: "Worker start ACK"
    Running --> WarmIdle: "attempt terminal; warm lease granted"
    Running --> Hibernated: "attempt terminal; default"
    WarmIdle --> Running: "next compatible attempt"
...
Project가 `Draining`으로 전이하면 새 Task, Attempt, Agent activation, warm lease 연장을
허용하지 않는다. 실행 중 Attempt는 완료 또는 명시 cancel deadline까지 관찰한다. 모든 Agent
...
1. Agent process 종료 뒤 durable context로 같은 Agent가 새 Attempt를 재개하는 E2E 시험
