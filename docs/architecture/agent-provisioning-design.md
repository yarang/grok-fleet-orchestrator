...
1. ~~**다중 grok 프로세스 로그 수집 부재**~~ — **2026-08-14 신설 `#50`
   항목으로 해소 방향 확정**: Phase 4에서 host당 여러 grok 프로세스가
   뜨는데, 각 프로세스의 stdout/stderr를 어디로 보낼지 설계가 없었습니다
   (현재 단일 프로세스도 `fleet-worker`가 별도로 리다이렉트하지 않고
   상속 — 다중 프로세스가 되면 뒤섞임). 사용자 요청으로 등록된
   [`agent-terminal-access-design.md`](agent-terminal-access-design.md)(`#50`)가
   `GrokRunner`의 grok spawn을 tmux 세션으로 감싸는 것으로 이 문제를
   함께 해결하도록 설계했습니다 — Phase 4 구현 시 `#50` 설계를 함께
   반영해야 합니다(별도 로그 인프라를 따로 만들지 않음).
...
3. **기존 단일 워커 배포와의 업그레이드 경로**: 기본값들(`max_agents = 1`,
   ...
   **단, `#50`(에이전트 터미널 모니터링·CLI 직접 접속, 2026-08-14 신설)이
   Phase 4에 얹이면서 `tmux`가 새로운 필수 호스트 의존성이 됩니다** —
   Phase 4 배포 전 전체 호스트 인벤토리에서 tmux 설치 여부를 먼저
   확인/일괄 재프로비저닝하도록 안내해야 합니다(상세는
