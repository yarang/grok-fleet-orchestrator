...
> **지위: Derived implementation reference.** 이 파일은 이전 `overview.md`의
> 코드 대조와 구현 이력을 보존한 호환성 참조다. 현재 설계 규칙은
> [아키텍처 정본 지도](canonical-map.md)에서 해당 주제의 정본을 확인한다. 이 문서는
> 코드 구조·구현 단계·과거 제한의 상세 근거를 보존하며, 정본과 충돌할 때 정본이 우선한다.

> **읽기 순서:** 빠른 시스템 지도는 [overview.md](overview.md)를 먼저 읽는다. 이 문서의
> Phase 표기, 정정 이력, 미구현 Self-Healing 절은 현재 운영 규칙이 아니며 각 정본·제안 문서의
> 근거로만 사용한다.

이 문서는 Grok Fleet Orchestrator의 **현재 구현 컴포넌트 지도와 구현 근거**를 설명한다.
배포 가이드는 [`deployment/README.md`](../deployment/README.md), 외부 계약은
[`contracts/README.md`](../contracts/README.md)를 참조한다. 상태 전이·정책·외부 계약의 결정은
아래 정본으로 이동했으며, 이 문서는 그것들을 다시 결정하지 않는다.

## 설계 탐색과 책임 경계

```mermaid
flowchart TD
    Overview["Overview\nimplementation map"] --> Project["Project model & governance"]
    Overview --> Task["Task management"]
    Overview --> Life["Lifecycle contract"]
    Overview --> Provision["Agent provisioning"]
    Provision --> Harness["Harness"]
    Provision --> Runtime["Runtime"]
    Provision --> Terminal["Terminal"]
    Overview --> Platform["Control plane / Worker platform"]
```

| 질문 | 정본 |
|---|---|
| Project 정책·격리·host/worker 배정은 무엇인가? | [`project-feature-design.md`](project-feature-design.md) |
| Task 제출·의존성·취소·결과는 어떻게 관리하는가? | [`task-management-design.md`](task-management-design.md) |
| Project·Task·Attempt·Agent가 함께 전이할 때의 규칙은 무엇인가? | [`project-task-agent-lifecycle.md`](project-task-agent-lifecycle.md) |
| Agent 생성·명령·ack·회수는 어떻게 하는가? | [`agents/provisioning.md`](agents/provisioning.md) |
| prompt/Skill/tool, vendor runtime, terminal의 세부 계약은 무엇인가? | [`agents/README.md`](agents/README.md) |
| 보안·가용성·TaskAttempt 일관성은 무엇인가? | [`../security/control-plane-security-model.md`](../security/control-plane-security-model.md), [`control-plane-availability.md`](control-plane-availability.md), [`task-execution-consistency.md`](task-execution-consistency.md) |

후속 정리에서는 아래 긴 구현 단락에서 이미 별도 정본이 소유한 정책 서술을 제거하고,
코드 경로·구현 상태·제약 근거만 남긴다.
...
### 4. PostgreSQL LISTEN/NOTIFY 동기화와 단일 Active 운영

> **운영 결정 (2026-08-16)**: Fleet의 지원 운영 모델은 Primary Orchestrator
> 하나만 실행하는 Single Active와 평상시 중지된 Cold Standby다. PostgreSQL을
> 공유하더라도 ACP 연결과 session map은 인스턴스 로컬이므로 Active-Active dispatch를
> 지원한다고 간주하지 않는다. 정본은
> [`control-plane-availability.md`](control-plane-availability.md)다.

`MultiAdminSync`는 테스트, 점검, 전환 구간에서 인스턴스 상태를 따라잡기 위한
방어 계층이다. 이를 Active-Active 지원 근거로 사용하지 않는다. Cold Standby 승격은
기존 Primary fencing, 새 control epoch 획득, Worker 재연결과 reconciliation을 거친다.
...
현재 설치·구성·운영 절차는 [`deployment/README.md`](../deployment/README.md)를 참조하세요.
