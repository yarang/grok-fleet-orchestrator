...
없으므로 여기서 값어치를 하는 검사는 경로 조작 방어가 아니라 **symlink 저항**이다 —
`workspace_root/<agent_id>`가 이미 링크로 존재하면 `create_dir_all`은 조용히 성공하고 그 뒤의 모든
쓰기가 링크 대상으로 간다. 반대로 **root 자신이 링크인 것은 위반이 아니다**: 양변을 정규화하면
자식은 root가 가리키는 실제 경로 아래에 놓이고, workspace를 다른 볼륨에 두는 것은 정상 구성이다.
한쪽만 정규화하면 그 정상 구성이 위반으로 보고된다. 이 검사는 워커가 자기 파일시스템
