...
`container_required` Attempt는 image digest로 고정된 rootless·비특권 container에서만 시작한다.
...
| filesystem | read-only rootfs, Attempt의 Agent worktree만 read-write bind mount | 승인된 build output/artifact mount만 별도 경로로 허용 |
...
| cache | Project/Agent 간 공유 write cache 금지 | content-addressed read-only cache 또는 Attempt 전용 cache |
...
- Worker는 Agent별 worktree를 만들고 해당 Attempt에는 그 경로만 mount한다. Project root, 다른
  Agent worktree, Worker 설정 디렉터리, credential backend는 mount하지 않는다.
- worktree의 Git remote와 branch/ref는 TaskAttempt snapshot으로 고정한다. untrusted Task가 remote,
  hooks, global Git config, credential helper를 바꾸는 것을 금지한다.
- Git push credential은 Security Manager의 Attempt-bound credential으로만 제공하며, user global
...
- helper는 root-owned executable과 root-owned allow-list를 사용하고, `attempt_id`, Project scope,
...
- helper가 없거나 policy·fencing 검증에 실패하면 Attempt를 약한 격리로 fallback하지 않고 거절한다.
