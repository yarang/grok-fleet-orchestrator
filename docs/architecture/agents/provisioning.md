...
사이의 한 주기 — 은 여기서 닫지 않는다. 닫는 것은 `#67` 로드맵의 게이트 ③(ACK 유실
`OutcomeUnknown`)이며, 그 게이트를 막고 있는 판독기 둘 중 process inventory 쪽이다. 원문자는
로드맵 자신의 목록을 세는 표기라서 위의 「구현 게이트」 1–6과 번호가 대응하지 않는다.
...
**이 규칙은 펜싱이 실제로 일어난다는 것에 기댄다.** 펜싱은 Worker의 heartbeat 루프 안에서만
일어나므로, 루프가 사라지면 유예를 아무리 넘겨도 프로세스는 멈추지 않는다. 그 루프는
`runner.rs`에서 `tokio::spawn`으로 띄우고 종료 경로에서만 join했기 때문에, 죽은 루프는 운영자가
워커를 내릴 때까지 **아무 신호도 내지 않았다.**

**2026-09-03: 그 경로를 닫았다.** `await_shutdown`이 OS 신호와 루프의 `JoinHandle`을 함께 기다려,
신호보다 먼저 끝난 루프를 결함으로 분류하고 워커를 fail-closed로 내린다(`shutdown_all()`이 Agent를
정리하고 `main`이 `ExitCode::FAILURE`를 반환해 감시자가 재기동한다). panic과 조용한 조기 반환이
여기에 걸린다.

**남은 구멍은 좁아졌지만 없어지지 않았다.** 루프가 `await`에서 영구히 막히거나(데드락), 런타임이
블로킹 호출에 굶거나, 프로세스가 `SIGSTOP`으로 멈추면 `JoinHandle`이 끝나지 않아 그 감시도 깨어나지
않는다. 그것까지 잡으려면 진행 카운터를 **OS 스레드**가 감시하고 `.fleet-agent.json`의 pid를 직접
죽여야 한다 — 그 경우에는 유예를 넘겨 기다려도 안전이 회복되지 않으므로, 판단은 다시 사람의 몫이다.

**2026-09-06: 그 워치독을 만들었다** (`crates/fleet-worker/src/watchdog.rs`, 로드맵 `#67` 게이트 ⑥). heartbeat 루프가 매
바퀴의 **시작**에 `ProgressBeacon`을 올리고, 전용 OS 스레드가 그것을 지켜본다.
`worker.watchdog_stall_after_secs`(기본 600초, `0`이면 끔) 동안 비콘이 한 번도 움직이지 않으면
`.fleet-agent.json`의 pid를 **동기적으로** 죽이고 종료 코드 `70`으로 프로세스를 끝낸다. 그러므로
위 문단의 운영 규칙(`last_seen` + `agent_fence_after_secs` 뒤에 삭제)이 이 부류에서도 성립한다 —
다만 기다려야 하는 것은 `agent_fence_after_secs`가 아니라 `watchdog_stall_after_secs`다.

세 가지가 이 설계를 강제했다.

1. **감시자는 OS 스레드여야 한다.** `tokio::spawn`으로 띄우면 굶은 런타임이 감시자도 굶긴다 —
   잡아야 할 바로 그 경우에 잡지 못한다.
2. **죽이는 것은 pid여야 한다.** `fence_all()`은 `async`인데, 지금 확정한 사실이 "그 런타임이
   진행하지 않는다"이므로 그것을 부르는 것은 정의상 걸린다. 그리고 `std::process::exit`은
   소멸자를 돌리지 않으므로 `kill_on_drop`이 자식을 거두지 않는다 — 죽이지 않고 나가면 Agent는
   그대로 남고, 그것을 회수하는 것은 감독자가 재기동해 줄 때뿐이다. 감독 없는 실행의 상한이
   감독자의 존재에 기대면 그것은 상한이 아니다.
3. **비콘은 heartbeat *성공*이 아니라 한 바퀴의 *시작*을 센다.** 성공을 세면 단절된 Worker가 곧
   죽는데, 그것은 self-fencing이 훨씬 부드럽게 다루는 경우다(프로세스만 멈추고 Worker는 살아
   재연결을 기다린다). 워치독이 끼어들면 그 복구 경로가 사라진다.

**프로세스 전체의 정지는 굶주림이 아니다.** `SIGSTOP`·디버거 attach·VM 정지는 워치독 스레드까지
함께 멈추므로 비콘이 멈춰 있는 것이 정상이다. 두 경우는 **워치독 자신의 잠을 재서** 갈린다 —
굶주림이면 워치독은 제때 깨어나고, 프로세스 정지면 자기 `sleep`도 함께 크게 늘어난다. 판정이
애매하면 죽이지 않는 쪽으로 접는다. 그래서 `SIGSTOP`은 위 목록에서 빠진다 — 워치독이 덮는 것은
데드락과 런타임 굶주림이고, 디버거를 붙이는 자리에서는 `0`으로 끄는 것이 옳다.

**검증 한계.** 판정 로직은 잠을 주입해 결정론적으로 시험하고(`watchdog.rs`의 7건), pid 종료와
pid 재사용 방어는 진짜 자식으로 시험하며(`agent_process.rs`의 4건), "진짜 루프가 비콘을 올린다"와
"정상 종료가 워치독을 해제한다"는 진짜 루프로 시험한다(`registration.rs`의 2건). 시험하지 **않은**
것은 그 셋을 잇는 마지막 한 걸음이다 — 실제로 굶은 tokio 런타임이 종료 코드 `70`을 내는 것.
런타임을 의도적으로 굶히려면 그러기 위한 코드를 프로덕션에 넣어야 하고, 그 비용이 이 한 걸음의
값보다 크다. 근거는 기전(위 1~3)이지 재현이 아니다.
...
## 명령 감사 추적 (`#67` · 이 문서 「구현 게이트」 4)

위 「구현 게이트」의 4번은 "명령·ACK·actor·generation의 감사 추적"이다. `#67`의 원문자
게이트 목록은 ①②③ 셋뿐이므로 이것을 "게이트 ④"로 부르지 않는다 — 그렇게 부르면 원문자가
두 목록을 세게 된다. 감사 로그가 **누가 무엇을 명령했는가**
