...
| 순서 | 문서 | 담당 책임 |
|---|---|---|
| 1 | [실행 격리](execution-isolation.md) | 위험도에 따른 host/container 결정과 cleanup 경계 |
| 2 | [프로비저닝](provisioning.md) | 생성·명령·ACK·회수·재조정 상태 전이 |
| 3 | [런타임 어댑터](runtime-adapters.md) | 허용된 runtime의 실행·능력 선언·입출력 경계 |
| 4 | [하네스 구성](harness-composition.md) | prompt·Skill·tool의 immutable 실행 snapshot |
| 5 | [도구 카탈로그](tool-catalog.md) | 도구 정의·권한·비밀값 전달 금지 |
| 6 | [컨텍스트와 메모리](context-and-memory.md) | thread context와 장기 메모리의 분리 |
| 7 | [터미널 접근](terminal-access.md) | capture와 interactive attach의 별도 보안 게이트 |
