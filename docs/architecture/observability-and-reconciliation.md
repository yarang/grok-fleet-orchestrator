...
| 6. audit 상관관계 필드로 경로 재구성 | 차단 | `lease_generation`·`fencing_token`·`control_epoch`와 effect 경로가 필드로 존재하지 않는다. `crates/fleet-core/src/audit.rs`는 actor·outcome 계열만 갖는다. **2026-09-06 재판정 — 위 사유는 두 군데가 부정확하고, 진짜 차단 지점은 더 앞이다.** (1) `control_epoch`는 **실재한다**: 코드 48곳, 마이그레이션 셋(026 task dispatch·031 agent desired state·035 agent command). 없는 것은 `lease_generation`·`fencing_token` 둘뿐이다(각 0건). (2) effect 경로도 같은 날 절반이 생겼다(`FleetEvent::TaskToolCall`, 게이트 ④ 행 참고). **그럼에도 게이트는 차단이며, 필드를 더하는 것으로는 열리지 않는다.** 실측: 감사 기록을 내는 곳은 35군데인데 전부 `fleet-api`(8)·`fleet-dashboard`(25)·`fleet-core`(2)이고 **`fleet-scheduler`는 0건이다.** 즉 dispatch·재배정·펜싱·breaker 전이 같은 **제어면 결정은 감사에 아무것도 남기지 않는다.** 그리고 감사를 내는 그 계층은 lease를 보지 못한다 — `fleet-api` 전체에 `lease` 언급이 0건이라 epoch를 실을 수단 자체가 없다. 지금 `AuditEvent`에 `control_epoch`를 더하면 35개 emitter 전부에서 **항상 NULL인 컬럼**이 된다(게이트 ④가 피한 것과 같은 함정). 그러므로 이 게이트의 선행은 필드가 아니라 **제어면 결정의 감사 경로**이고, 그것은 `FleetState`가 이미 쥔 `control_fence()`를 감사와 잇는 일이다. **2026-09-06(같은 날, 두 번째 증분): 그 선행을 만들었다.** `FleetState::audit_control`이 생기면서 `fleet-scheduler`가 처음으로 감사 기록을 낸다. 남기는 제어면 결정은 셋이며 **전부 거절·유기 계열이다**: `control.dispatch_refused`(리스가 없어 스스로 물러섬), `control.write_fenced`(리스가 있다고 믿고 쓰러 갔다가 저장소가 세대 불일치로 막음), `control.outcome_abandoned`(리스를 잃어 워커 결과의 제어 처리를 건너뜀 — 그 Task의 최종 상태를 아무도 확정하지 않은 채 창이 닫혔다는 뜻이라 새 소유자의 재조정 대상이다). **성공한 dispatch는 남기지 않는다** — 이미 `FleetEvent::TaskDispatched`가 있고 `tasks.dispatch_control_epoch`가 세대를 들고 있어 중복이며, 이 표가 요구하는 증거는 거절 쪽이다. 마이그레이션 038이 `audit_log.control_epoch`(nullable, 부분 인덱스)를 더한다. **`None`이 두 가지 정상을 뜻한다**: 운영자 행위(세대 개념이 없다)와 리스를 갖지 못한 채 내린 거절(`Fenced`/`Stopped`에는 epoch가 없다) — 후자에서는 "세대가 없었다"가 곧 증거이므로 `detail.lease_status`가 그것을 나른다. 행위자는 `cluster_id`가 아니라 **`instance_id`**다(`LeaseObserver::instance_id`를 새로 노출); 이 표가 요구하는 것이 "둘 이상의 owner 관측"인데 같은 cluster의 두 인스턴스는 cluster_id로 구분되지 않는다. **게이트는 여전히 차단이다** — `lease_generation`·`fencing_token`은 아직 없고, 여기서 닫힌 것은 "제어면 결정이 감사에 아무것도 남기지 않는다"는 선행 하나다. 시험 6건(scheduler 4·store 왕복 2), 변이 5건 전부 죽는다 |
