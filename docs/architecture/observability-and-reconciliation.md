...
게이트별 현황이다. **닫힌 것은 1·2·5 셋, 부분은 3 하나이며, 남은 셋은 4·6·7이다**
(2026-09-16에 2가 차단에서 부분으로 옮겨졌고 2026-09-17에 닫혔다 — 아래 표와 그 아래 절들 참고). 다만 그
...
### 순서는 전제조건이라야 순서다 (2026-09-17) — 게이트 2 닫힘

앞의 두 절이 수단을 만들었고(인벤토리 조회, 양방향 회수), 남은 것은 게이트 문언의 한
단어였다: 복구가 신규 dispatch보다 **먼저**.

#### 배경 루프로는 순서가 만들어지지 않는다

`Reconciler`는 첫 tick을 한 주기(기본 30초) 뒤에 돈다. 그 사이의 dispatch는 아무것도 묻지
않은 채 나간다. 주기를 줄여도 창은 좁아질 뿐 닫히지 않고, **닫혔는지 확인할 방법도 없다** —
루프와 dispatch가 경합하는 한 "먼저"는 관측이지 보장이 아니다.

그래서 순서를 **dispatch의 전제조건**으로 옮겼다. `Dispatcher::ensure_worker_recovered`를
지나지 않은 워커로는 이 제어 세대의 어떤 일도 나가지 않는다. 구조적으로 어길 수 없다.

#### 전역 배리어가 아니라 워커별이다

"failover 뒤"라는 말은 전역 단계처럼 들리지만, **"모든 워커가 재연결했다"는 시점은 존재하지
않는다.** 워커는 자기가 join할 때만 transport에 등록되고(오케스트레이터가 저장소를 보고
먼저 다이얼하지 않는다), 영영 돌아오지 않는 워커도 있다. 전역 배리어를 두면 그런 워커
하나가 fleet 전체의 dispatch를 영구히 막는다.

물어볼 수 있는 시점은 **그 워커에게 실제로 일을 주려는 순간**뿐이고, 그때가 곧 "먼저"가
성립해야 하는 유일한 자리다. 세대 기록(`recovered_workers`)이 인메모리인 것도 같은 이유다 —
프로세스가 죽으면 비워지고, 그것이 정확히 이 복구가 다시 필요해지는 조건이다. lease epoch이
바뀌면 집합을 통째로 버린다: 새 제어 기관은 이전 기관이 물어본 것을 물려받지 않는다.

#### 통과 조건은 "물어봤다"이지 "답을 받았다"가 아니다

| 인벤토리 | dispatch | 이유 |
| --- | --- | --- |
| `Reported` | 허용 (어긋난 Task 정리 후) | 권위 있는 답을 받았다 |
| `Undeclared` | 허용 | 저쪽이 말해 줄 수 없다고 **말했다** |
| `Refused` | 허용 | 저쪽 구현이 선언과 어긋나지만 연결은 살아 있다 |
| 조회 실패 | **거절** | 묻지 못했다 — 어차피 보낼 수도 없다 |

`Undeclared`를 막으면 **오늘의 거의 모든 배포에서 dispatch가 영구히 막힌다.** 실제 Agent
다수가 `session/list`를 광고하지 않기 때문이다. 이 게이트가 요구하는 것은 "복구했다"가 아니라
"복구를 먼저 시도했다"이며, 물어볼 수 없는 상대에게 물어본 뒤 통과시키는 것이 그 요구의
정직한 이행이다. 시험 하나가 이 줄을 부정 단정으로 고정한다.

#### 브레이커 뒤, CAS 앞

위치가 두 방향으로 묶여 있다.

- **브레이커 뒤**여야 못 닿는 워커가 회로를 열 수 있다. 앞에 두면 이 경로가 transport 실패보다
  먼저 반환해 브레이커 기록을 건너뛰고, selector가 그 워커를 영영 계속 고른다. 그래서 복구
  실패도 transport 실패와 **같은 자리에** 브레이커 실패를 남긴다. 회로가 열린 워커는 어차피
  여기 오지 않으므로 복구할 필요도 없다 — 그 워커의 어긋난 Task는 주기 스윕이 본다.
- **CAS 앞**이어야 상태만 옮겨 두고 보내지 못한 실행이 생기지 않는다.

#### 순서를 사후에 재구성하는 기록

`control.worker_recovered`가 워커당 세대당 한 줄을 남긴다. **이 줄이 없는 워커로는 이 세대에
dispatch가 나가지 않았다**는 뜻이고, 그것이 게이트가 요구하는 순서의 감사 근거다. 줄의
`inventory` 필드는 무엇을 근거로 통과시켰는지 말하며, `undeclared`인 줄이 대부분일 것이다 —
그 사실 자체가 값어치가 있다: 그 fleet에서는 이 게이트가 실질적으로 비어 있다는 뜻이기 때문이다.

#### 무엇이 닫히지 않았는가

게이트 2의 문언은 닫혔다. 닫히지 **않은** 것은 인접한 사실 하나다: 오늘 이 capability를
광고하는 Agent가 실제로 있는지는 여전히 관측되지 않았다. 다만 그것이 이제 런타임이 읽어
감사에 남기는 값이므로 배포 한 번이면 답이 나온다 — 문서가 미지로 안고 있을 값이 아니다.

### 확인은 알 수 있었는데 시계가 없었다 (2026-09-17) — 게이트 3의 ACK 유실

이 칸의 남은 절반은 둘이었다: ACK 유실과 grant expiry. 앞쪽이 닫혔다.

#### 값은 있었고 판독자가 없었다

`031`이 `command_generation`과 `last_acked_generation`을 만들었고,
`Agent::command_delivered()`·`start_pending()`이 그 위에 술어까지 올려 두었다. 그런데 **그
술어들의 호출부는 MCP 응답을 조립하는 두 줄뿐이었다** — grep이 그렇게 말한다. 운영자가
`fleet_list_agents`를 직접 호출해 그 필드를 눈으로 보지 않는 한, Worker가 명령을 영영
집어가지 않아도 아무 신호가 나지 않았다.

**이번 주 세 번째로 같은 모양이다.** `#70` 게이트 2의 차단 사유가 "실측되지 않았다"가 아니라
"읽지 않았다"였고, 고아 세션 회수에서는 `041`의 인덱스에 질의가 없었으며, 여기서는 술어에
판독자가 없었다. 셋 다 "없다"가 아니라 **"있는데 쓰이지 않는다"**였다.

#### 막고 있던 것은 시계였다

"확인됐는가"는 두 정수의 비교로 알 수 있었지만 **"얼마나 오래 확인되지 않았는가"는 잴 값이
없었다.** 그리고 `updated_at`으로는 잴 수 없다 — 그 컬럼은 관측 보고(`#67` 4c-B)를 포함해
행을 건드리는 모든 쓰기에서 움직인다. 명령을 집어가지 않는 Worker라도 heartbeat은 계속 올 수
있으므로, `updated_at`을 기준으로 재면 미확인 시간이 영원히 0에 가깝게 유지된다. **정확히
탐지하려는 상황에서 탐지가 되지 않는 지표다.**

`042`가 `agents.command_issued_at`을 만들고, 세대가 **실제로 오르는 그 자리에서만** 함께
찍는다(세 UPDATE 중 둘은 조건부 CASE이므로 시각도 같은 CASE에 넣었다 — 조건이 갈리면 값을
바꾸지 않은 호출이 시계를 되감는다).

#### 재발행하지 않는다

게이트 문언의 "**자동 중복 실행 없이**"가 요구하는 자리가 정확히 여기다. 확인되지 않은
명령을 자동으로 다시 보내면, Worker가 첫 명령을 늦게 집어갔을 때 같은 Agent가 두 번 뜬다.
확인이 없다는 것은 "도달하지 않았다"가 아니라 **"모른다"**이고, 모르는 상태에서 행동을
반복하는 것이 정확히 중복 실행의 경로다.

그래서 이 스윕이 하는 일은 하나뿐이다: `agent.command_unacked`를 남긴다. Agent·세대 쌍마다
한 번이며, 매 tick 남기면 같은 사실이 감사를 채워 다른 기록을 덮는다. 시험 하나가 세대가
오르지 않는지를 단정한다 — 세대가 오르면 그것이 곧 재발행이다.

#### 042 이전 행은 보고하지 않는다

`command_issued_at`이 `None`인 행은 잴 근거가 없다. **`None`을 "오래됐다"로 읽는 구현은
마이그레이션 직후에 모든 미확인 Agent를 한꺼번에 쏟아낸다.** 부정 단정 시험이 그것을
고정한다.

마이그레이션은 미확인 명령을 가진 기존 행만 `updated_at`으로 채운다. `041`이 "기본값을 주면
없던 사실을 만든다"며 NULL을 고수한 것과 모순되지 않는다 — 저기서 채울 뻔한 값은 **추측**
이었지만 이 `updated_at`은 증명 가능한 **상한**이다(명령이 그 행을 마지막으로 손댄 시각보다
나중에 나갔을 수는 없다). 상한으로 재면 판정이 늦어질 뿐 거짓 양성이 생기지 않는다.

#### grant expiry가 남은 이유는 성격이 다르다

같은 칸의 나머지 절반은 판독자가 없어서가 아니라 **증거가 없어서** 막혀 있다. 정본
[실행 일관성](tasks/execution-consistency.md)의 완료 조건 4가 요구하는 것은 "credential
delivery grant와 Agent execution lease의 release/정리 **증거**"이고, 그 증거의 자리가 effect
ledger다. 즉 게이트 4의 의도적 보류와 같은 선행에 걸려 있으며, 그 보류가 풀리기 전에는 이쪽도
열리지 않는다.
...
**2026-09-23 — 문헌 대조로 보류에 조건 하나가 붙었고, 미결 하나가 추가됐다.**
전문은 [effect ledger 설계의 문헌 대조](../reviews/effect-ledger-research-2026-09-23.md)다.

- **보류는 유지된다. 단 "첫 비가역 effect를 도입하는 바로 그 변경과 동시에"가 하한이다.**
  Azure Compensating Transaction 패턴은 원장을 인프라의 하드 전제조건으로 규정하지만 그
  전제는 증명·보상 대상 effect가 존재할 때 발동하므로, 지금의 보류는 문헌과 모순되지 않는다.
  기록이 외부화보다 **늦으면** 그 구간은 영구 미증명 잔여가 되므로 "나중에 소급해서"는
  선택지가 아니다. 정본 [실행 일관성](tasks/execution-consistency.md#원장을-언제-만들-것인가--2026-09-14-판정)의
  「재개 조건」이 적은 **먼저**는 이 하한보다 강하므로 그대로 둔다.
- **미결 추가 — `Cancelled`와 `Compensated`를 가른다.** 보상은 되돌리기가 아니라 새로운
  전진형 부작용이므로(1987 Sagas 원문·현행 Azure 지침), 보상으로 도달한 종점은 "아무 일도
  없었음"이 아니다. `CancelUnconfirmed`의 출구인 `Cancelled`는 **`Started` effect 없음이
  증명된 경우에만** 주어야 하고, 보상 성공은 별도 terminal이어야 한다. 원장 없이는 그
  증명을 할 수 없으므로 이 분리는 원장을 도입하는 변경이 함께 들고 온다.
- **이 원장은 예방 장치가 될 수 없다.** 문헌의 예방 레버는 거의 전부 제어면이 effect를
  발행·게이팅한다는 전제 위에 있는데 이 오케스트레이터는 사후 관측만 한다. 기대할 수 있는
  것은 탐지·증적·에스컬레이션까지다.
