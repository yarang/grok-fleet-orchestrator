...
`CancelUnconfirmed`)가 저장소에 **없어서** 시험을 작성할 수조차 없고, 게이트 2와 3은 **같은
하나**를 기다린다(워커가 명령과 무관하게 들고 있는 것을 여는 process inventory). 게이트 6은
그 둘과 또 달라서, 필드가 아니라 **제어면 결정의 감사 경로**가 선행인데 그 선행은
2026-09-06에 들어왔다(아래 행 참고) — 남은 것은 `lease_generation`·`fencing_token`이다.
