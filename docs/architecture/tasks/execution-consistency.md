...
### 원장을 언제 만들 것인가 — 2026-09-14 판정

**지금은 만들지 않는다.** 이 fleet은 아직 외부 메시지 발송·DB 마이그레이션 같은 비가역 외부
부작용을 내는 워크로드를 돌리지 않는다(2026-09-14 확인). 위 「도구 부작용 분류」의 네 등급 중
`Irreversible`과 `Compensatable`에 해당하는 실제 사례가 없으므로, 원장이 증명할 대상 자체가
아직 존재하지 않는다.

그 상태에서 여덟 상태 테이블을 만들면 **정의되지 않은 개념 셋을 발명하게 된다** — 외부
idempotency key 공식의 네 요소 중 `Project ID`만 실재하고, `Task ID`는 redrive에서 바뀌며
(위 「설계 미결」), `안정된 effect scope`는 정의가 없고, `제출 시 정책 revision`은 개념 자체가
저장소·코드 어디에도 없다. 셋 다 "무엇이 같은 부작용인가"라는 질문의 답이고, 그 답은 **어떤
외부 provider를 쓰는지가 정해져야** 나온다.

**단, 이 부재는 강제된 것이 아니다.** 도구 수준 allow-list가 없고(`allow_list`/`sandbox` 등
grep 0건) `ToolInvocationKind`는 `Execute` 하나로 `ls`와 외부 API 호출을 똑같이 표현한다. 즉
오늘도 Agent가 `bash`로 비가역 호출을 **할 수는 있으며**, 하지 않고 있을 뿐이다. 등급을 매길
주체가 없다는 사실이 원장보다 앞선 공백이고, 그것을 닫는 것은 [실행 격리](../agents/execution-isolation.md)
(로드맵 `#64`)의 범위다.

**재개 조건**: 비가역·보상불가 부작용을 내는 워크로드를 도입하는 순간, 원장은 그 워크로드보다
**먼저** 있어야 한다. 부작용이 먼저 나가고 원장이 나중에 붙으면 그 사이의 실행은 영원히 증명
불가 구간으로 남는다. 그때 필요한 결정은 순서대로 (1) 부작용 등급을 누가 매기는가,
(2) redrive 앵커, (3) effect scope의 정의, (4) HMAC 키 소유·회전이다.

#### 문헌 대조 (2026-09-23) — 보류는 유지, 세 가지가 보태진다

위 판정을 문헌과 대조했다. 전문과 인용은
[effect ledger 설계의 문헌 대조](../../reviews/effect-ledger-research-2026-09-23.md)가 갖는다.

1. **보류 자체는 문헌과 모순되지 않는다.** Azure Compensating Transaction 패턴이 원장을
   "있으면 좋은 것"이 아니라 인프라의 **하드 전제조건**으로 규정하지만, 그 전제는 *증명·보상
   대상 effect가 존재할 때* 발동한다. 위 「재개 조건」의 **먼저**는 문헌이 요구하는 하한
   (첫 비가역 effect를 도입하는 변경과 **동시에**)보다 강하므로 그대로 둔다. 다만 사전
   도입과 소급 도입의 실무 비용 비교 증거는 **어떤 소스에도 없었다** — 위 문장은 합의된
   원칙이지 측정된 결론이 아니다.
2. **우리 원장은 예방 장치가 될 수 없다.** 문헌의 예방 레버는 거의 전부 *제어면이 effect를
   발행·게이팅한다*는 전제 위에 서 있는데, 이 오케스트레이터는 도구 호출을 사후 알림으로
   **관측만** 한다. 그러므로 원장을 만들 때 기대할 수 있는 것은 탐지·증적·에스컬레이션까지이고,
   "막는 것"은 [실행 격리](../agents/execution-isolation.md)(`#64`)의 allow-list 쪽에 남는다.
   이 구분을 잃으면 원장을 만들어 놓고 막지 못하는 것을 막았다고 읽게 된다.
3. **위 (4) HMAC 키 파생을 제어면이 한다는 설계에는 문헌 근거가 없다.** 이 가설을 세우려던
   네 갈래(Atomix·IETF 초안·Stripe·Helland)가 적대적 검증에서 전부 기각됐고, 확인된 것은
   오히려 **호출부 생성 + 수신자 소유 dedup**이 업계 표준이라는 사실이다. 기각은 "반대가
   참"이 아니라 "근거 미확보"를 뜻하므로 설계를 뒤집지는 않되, 재개 시 이 결정은 **인용이
   아니라 우리 제약(무재시도·관측 전용)으로부터의 자체 논증으로** 정당화해야 한다.

#### 미결 — `Cancelled`와 `Compensated`를 가른다 (2026-09-23 추가)

위 해소 규칙은 `CancelUnconfirmed`의 출구를 `Cancelled` 하나로 두고 있는데, 그 이름 하나가
성격이 다른 두 결말을 담고 있다.

1987 Sagas 원문과 현행 Azure 지침이 같은 것을 적는다 — 보상은 되돌리기가 아니라 **새로운
전진형 부작용**이고(*"to compensate for the letter, send a second letter explaining the
problem"*), 보상 후의 상태는 *"does not necessarily return the database to the state that
existed when the execution began"*이다. 즉 보상으로 도달한 종점은 "아무 일도 없었음"이
아니라 "흔적이 둘 남았음"이다.

따라서 원장을 만들 때 terminal은 둘로 갈라야 한다.

| 상태 | 부여 조건 |
| --- | --- |
| `Cancelled` | `Started` effect가 **없음이 증명됨**. 외부에 나간 것이 없다 |
| `Compensated` | effect가 나갔고 보상 행위가 성공했다. 외부에는 원 행위와 보상 행위가 **둘 다** 남아 있다 |

이 분리는 원장 없이는 판정할 수 없으므로(증명의 출처가 원장이다) 지금 `TaskStatus`에
넣지 않는다. **원장을 도입하는 변경이 이 두 terminal을 함께 도입한다.**
