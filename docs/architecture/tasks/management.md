...
Task Management는 Project 안에서 수행할 **검증 가능한 한 건의 작업**을 만들고, 우선순위·의존성·취소·삭제·결과·감사를 관리한다. Agent는 Task의 지속 소유자가 아니라 실행 가능한 후보이며, 실제 실행 한 번은 Task 자신이 표현한다 — 별도 `TaskAttempt` 엔티티는 없다([흡수 판정](../project-task-agent-lifecycle.md#attempt-흡수-판정)).
...
| Task dispatch 요청과 감사 | 실행 CAS·부작용 fencing — `execution-consistency.md` |
...
flowchart LR
    ...
    Queue --> Dispatch["dispatch 요청"]
    Dispatch --> Agent["optional Agent context\nselected Worker"]
...
목표 모델은 다음 두 가지를 구분한다. 실행 시도는 **별도 엔티티가 아니라** Task 자신이며, 재시도가 없으므로 Task당 실행은 최대 하나다.

| 개념 | 의미 | 상태 |
|---|---|---|
| Task | 사용자가 요청한 작업 의도와 완료 조건, 그리고 그것의 실행 1회 | 현재 일부 구현, Project 귀속은 미구현 |
| Execution snapshot | Project policy, isolation, harness revision, input hash | 설계만 존재 |

Task 상태가 곧 실행 상태이므로 투영 계층이 필요하지 않다. 실행 식별자와 CAS 술어의 배치는 [실행 일관성 정본](execution-consistency.md)이 소유한다.
...
5. 의존 Task가 있으면 Task는 생성되지만, 모든 선행 Task가 성공하기 전에는 dispatch하지 않는다. 실패·취소된 선행 Task를 자동 성공으로 간주하지 않는다.
...
stateDiagram-v2
    ...
    Pending --> Dispatched: dispatch accepted
...
- `Dispatched`는 Task가 특정 실행 주체에 수락되었음을 뜻하며, 단순 네트워크 송신 성공만으로 전이하지 않는다.
- terminal Task는 재개하지 않는다. 같은 의도를 다시 실행하려면 **새 Task**를 만든다 — retry 요청이라는 경로는 없다.
- cancel은 요청과 확정을 구분한다. 부작용이 이미 발생했을 수 있으므로 `Cancelled`가 rollback 성공을 의미하지 않는다.
- 실행 결과가 `PartiallyApplied`이면 Task 상태는 `Failed`로 두되, failure disposition과
  effect ledger를 반드시 함께 반환한다. 단순 output만으로 종결하지 않는다.

정확한 CAS 술어, ack 유실 분류와 외부 부작용의 보상 규칙은 [`execution-consistency.md`](execution-consistency.md)가 정본이다.
...
- 실행 중인 Task는 실제 `worker_id`를 기록한다. Agent context를 사용할 때만 `agent_id`를 함께
...
- Project drain은 새 Task 생성을 막고, 실행 중 Task에는 cancel deadline을 적용할 수 있다. 자세한 교차 전이는 lifecycle 계약을 따른다.
...
모든 제출, dedupe 결과, dispatch 결정, 취소 요청/확정, terminal 결과, 삭제에는 actor, request id, Task id, Project id, policy revision, dispatch control epoch를 감사 이벤트로 남긴다. 외부 effect는 별도 ledger에 provider receipt·idempotency key·보상 결과를 남기며, secret 원문은 어느 기록에도 남기지 않는다.
...
4. dispatch CAS·ack 계약은 실행 일관성 정본의 게이트를 만족한 뒤 도입한다. 별도 `TaskAttempt` 테이블은 만들지 않는다.
