...
- WebSocket 재연결 로직 (현재 AcpTransport는 첫 연결 실패 시 에러; Phase 8.2에서 지수 백오프 추가)
...
- WebSocket 재연결 미구현 — orchestrator→worker ACP 연결이 끊기면 task 실패 (Phase 8.2)
- 단일 세션 per worker — 동시 task 처리 불가 (Phase 8.4)
- mTLS 미지원 — Cloudflare Tunnel에 위임 (Phase 8.5)
- 시스템 메트릭의 `active_tasks`는 항상 0 — Phase 8.4에서 동시성 도입 시 실제 카운트
