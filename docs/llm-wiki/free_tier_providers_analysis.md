...
### 1.4 Groq 프롬프트 축소 실험 (2026-08-11) — 읽기 전용 작업이면 무카드로도 가능

`grok --help`/`grok inspect`로 토큰 절감 수단을 조사하고 실제 Groq 429 에러 메시지의
"Requested: N" 수치로 직접 측정했다. `grok inspect` 결과 `~/.claude/skills/`(Claude Code
호환 스캔)에서 **74개 스킬**(대부분 MoAI 스위트)이 자동 로드되고 있었던 것이 주요 원인 중
하나였다.

| 구성 | 요청 토큰 | 6K TPM (8b-instant) | 8K TPM (gpt-oss-20b) | 12K TPM (70b-versatile) |
|---|---|---|---|---|
| 기본값 (아무 옵션 없음) | ~19,000–22,000 | ❌ | ❌ | ❌ |
| `--agent explore`(읽기 전용) 단독 | 12,470 | ❌ | ❌ | ❌ |
| `GROK_CLAUDE_SKILLS_ENABLED=false` 단독 (기본 에이전트, 편집 가능) | 14,275 | ❌ | ❌ | ❌ |
| `--agent explore` + 스킬 비활성화 | **7,784** | ❌ | ✅ | ✅ |
| 기본 에이전트(편집 가능) + 스킬 비활성화 + `--disable-web-search --no-subagents --no-plan` | 10,208–14,309 | ❌ | ❌ | ❌ |

**결론**: 트리밍을 최대로 적용해도 **편집 가능한(파일 쓰기 도구를 가진) 에이전트는 어떤
무료 모델의 TPM 한도에도 들어가지 않는다.** 읽기 전용 `explore` 에이전트로 제한할 때만
(7,784토큰) 8K/12K TPM 모델에서 실제로 동작한다 — 아래 명령으로 실측 확인함:

```bash
GROK_CLAUDE_SKILLS_ENABLED=false GROK_CURSOR_SKILLS_ENABLED=false \
  grok -m groq-free-oss20b --agent explore -p "..."
```

**실질적 결론**: Groq 무카드 무료 티어는 **코드 탐색/질의응답(읽기 전용) 용도로만** 카드 등록
없이 즉시 쓸 수 있다. 실제 코드 편집이 필요한 에이전틱 작업에는 여전히 Developer 티어(카드
등록, 무과금, 10배 한도)가 필요하거나, TPM 제한이 없는 OpenRouter를 쓰는 것이 낫다.
...
- [ ] **Groq Developer 티어(무료, 카드 등록만, 과금 없음)**: §1.4 실측 결과 카드 등록 없이도
      `--agent explore`(읽기 전용, 7,784토큰) + `GROK_CLAUDE_SKILLS_ENABLED=false`로 8K/12K
      TPM 모델은 실사용 가능함을 확인. **편집 가능한 에이전틱 작업**에는 여전히 카드 등록이
      사실상 필수(트리밍 최대 적용해도 10,208~14,309토큰, 모든 무료 TPM 초과) — 읽기 전용
      용도로만 쓸지, 편집까지 원해 카드 등록할지 결정 필요
