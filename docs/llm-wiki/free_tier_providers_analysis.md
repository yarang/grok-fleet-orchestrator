...
> `llama-prompt-guard-2-*` 등은 원본에 없던 항목. 코딩 에이전트 워크로드에는
> `llama-3.1-8b-instant`(일 14,400회로 압도적으로 넉넉함)와 `llama-3.3-70b-versatile`(품질 우선,
> 일 1,000회)가 실질적 후보다.

**추가로 발견한 중요 옵션**: Groq은 신용카드를 등록해도 **과금되지 않는** "Developer 티어"를
제공한다 — 카드 등록만으로 **한도가 10배 증가**하고 유료 사용 시 토큰당 25% 할인까지 받는다.
무료 운영이 목표라면 이 티어도 카드 등록 자체가 부담이 아니라면 고려할 만하다(§6 체크리스트 참고).
...
- [ ] **공급자 활성화**: Groq만 / OpenRouter만 / 둘 다 / 보류
- [ ] **API 키 발급**: 계정 생성은 본인 확인/이메일이 필요해 제가 대신 만들 수 없습니다 —
      [console.groq.com](https://console.groq.com)과 [openrouter.ai](https://openrouter.ai)에서
      직접 가입 후 키를 전달해주셔야 합니다 (전달 시 `docs/credentials/` 절차대로 저장하겠습니다).
- [ ] **Groq 화이트리스트 모델**: `llama-3.1-8b-instant`(일 14,400회, 코딩 에이전트 실사용에
      가장 현실적) / `llama-3.3-70b-versatile`(품질 우선, 일 1,000회) / 둘 다 / 기타
- [ ] **OpenRouter `:free` 모델**: 매번 바뀌는 라인업이라 특정 모델을 지금 고정하기보다,
      연동 시점에 [openrouter.ai/models?max_price=0](https://openrouter.ai/models) 확인 후
      선택 — 지금 결정 불필요, 연동 단계에서 재확인
- [ ] **Groq Developer 티어(무료, 카드 등록만)**: 카드 등록해서 10배 한도 받을지, 순수
      무카드로 남을지
- [ ] **쿼터 관리 수준**: §2.2 방안 A(최소 대응, 지금 시작) vs 방안 B(능동적 추적, 코드
      필요) — A로 시작 후 필요 시 B로 승격을 권장
- [ ] **호스트 배치**: 기존 워커(arm1/ec1/ec2)에 `[model.groq-free]` 등을 추가 항목으로만
      넣을지, 전용 "무료 티어 실험용" 워커를 새로 둘지
