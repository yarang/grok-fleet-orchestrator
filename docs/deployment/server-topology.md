...
*   **단일 Egress IP 통제**: 모든 분산 워커의 모델 API 호출을 단일 게이트웨이(**liteLLM**)로 통합하여 IP 차단을 방지하고 사용량 제한을 격리합니다.
*   **하이브리드 설치 모델**: 메인 컨트롤러는 오버헤드가 적은 **네이티브(systemd)** 서비스로 운영하고, 관리가 편리해야 하는 DB와 API 프록시는 **컨테이너(Docker)**로 격리하여 운영 오버헤드를 줄입니다.
...
1. 사용자가 AI 클라이언트에서 작업을 요청하면 Cloudflare Tunnel을 통해 메인 오케스트레이터 서버의 `Nginx`로 유입됩니다.
2. `Nginx`가 `fleet serve` 네이티브 포트(`8081` 또는 `8082`)로 중계합니다.
...
1. 워커 노드의 `grok agent`가 빌드 및 기획 도중 LLM 완성(Completion) 호출이 필요할 시, 자체적으로 외부 인터넷으로 직접 나가지 않고 **메인 서버의 `liteLLM` 게이트웨이 포트(4000)**로 API 요청을 전달합니다.
2. `liteLLM`은 사전에 설정된 여러 공급업체(OpenAI, Anthropic, Gemini 등)의 API 키를 자동으로 주입하고 적절한 타겟 모델로 스위칭합니다.
...
services:
  ...
  litellm:
    ...
    environment:
      - DATABASE_URL=postgresql://fleet:CHANGE_ME_DB_PASSWORD@postgres:5432/litellm
