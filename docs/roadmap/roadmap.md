...
48. ⏳ **프로젝트(Project) 기능 도입** (P2, 신규 기능) — **설계 완료, 구현 대기**.
    ...
    **핵심 설계 결정** (AskUserQuestion으로 사용자 확인, 2026-08-14): 프로젝트↔
    host/agent 소속은 **다대다(M:N)**(하나의 host/worker가 여러 프로젝트에 동시
    배치 가능 — 조인 테이블 `project_workers`/`project_hosts`), `project_id`가
    지정된 태스크의 디스패치는 **소프트 힌트**(배치된 agent가 없거나 전부 불가하면
    에러 없이 전체 워커 풀로 폴백 — `server_hint`의 엄격한 `HintedNotFound`/
    `HintedUnavailable`과 의도적으로 다름).

    전체 설계(ER 다이어그램, `WorkerSelector` 파이프라인에 삽입되는 5.5단계 소프트
    선호 필터, `Store` 트레이트 확장, RBAC 권한 4종, REST/MCP API 표면, 4단계
    구현 계획, 열린 질문)는
...
49. ⏳ **에이전트(Agent) 동적 프로비저닝 · 메모리 · 스레드 요약** (P2, 신규 기능) —
    **설계 완료, 구현 대기**. `#48` 등록 직후 사용자가 요구사항을 크게 확장:
    (1) host당 여러 에이전트 동시 운영, (2) custom 프롬프트로 에이전트 구분,
    (3) 프로젝트가 필요할 때 에이전트를 직접 만들어서 호출, (4) host 여유
    있을 때만 생성, (5) 에이전트가 여러 세션에 걸친 맥락 유지, (6) 그 맥락을
    프로젝트별 메모리로 관리, (7) 프로젝트에 속하지 않는 태스크 스레드는
    스레드별 요약으로 관리, (8) 필요 시 결과물을 디렉토리로 관리.

    기술적 표면이 `#48`(스키마/RBAC/소프트 디스패치 필터)과 크게 달라(프로세스
    수명주기 관리, 오케스트레이터→워커 제어 채널, 메모리 저장소) AskUserQuestion
    확인 후 별도 항목(`#49`)으로 분리.

    **핵심 설계 결정** (AskUserQuestion, 2026-08-14): Agent를 Worker와 분리된
    **신규 엔티티**로 도입(Worker는 저수준 접속/용량 개념 유지, Agent가
    custom_prompt·메모리·프로젝트 소속을 담당하고 Running 상태일 때 정확히
    하나의 Worker에 연결). 프로비저닝은 **진짜 동적**(오케스트레이터가 실행 중
    원격으로 grok 프로세스 시작/종료) — 단, `fleet-worker`가 인바운드 연결을
    받지 않는다는 기존 설계 원칙(`#42`에서 확인)을 지키기 위해 **새 인바운드
    채널을 만들지 않고 기존 heartbeat 폴링에 커맨드를 얹는 방식**을 채택. 메모리는
    **구조화된 텍스트/JSON 누적 + 프롬프트 주입**(임베딩/벡터DB 불필요, 기존
    `build_threaded_prompt` 패턴의 자연스러운 확장).

    전체 설계(ER 다이어그램, heartbeat 커맨드 큐 프로토콜 시퀀스 다이어그램,
    custom_prompt/메모리 주입 플로우 다이어그램, `agents`/`agent_commands`/
    `agent_memory`/`thread_summaries` 스키마, RBAC 4종, 4단계 구현 계획, 5개
    열린 질문)는
    [`docs/architecture/agent-provisioning-design.md`](../architecture/agent-provisioning-design.md)에
    정리했습니다.

    ⚠️ **알려진 리스크**: Phase 3(동적 프로비저닝 — `fleet-worker`를 단일
    프로세스 관리에서 다중 프로세스 관리로 재작성)이 이번 확장에서 가장 위험도가
    높은 부분입니다. 설계 문서는 순수 유닛테스트만으로는 신뢰성을 담보하기
    어려워 실기기 대상 수동 검증을 병행하도록 권고하고 있습니다 — 구현 착수
    시 이 순서(Phase 1 정적 등록 디딤돌 → Phase 2 메모리/요약 → Phase 3 진짜
    프로비저닝 → Phase 4 API/UI)를 반드시 지킬 것.

    **다음 단계**: 설계 문서 §11의 Phase 1(스키마 + 정적 등록)부터 순차 구현
