...
| #73 | HTTP capability 행렬 기본 deny 전환 | P0 · 설계 확정·구현 대기 | [Authorization](../security/authorization-and-audit.md) | `authorize_http_endpoint`가 `required_capability`의 `None`을 통과가 아니라 거절로 처리하고, capability 불필요 route(`/health` 등)만 명시 allow-list로 둔다. 현재 누락 route인 `GET /v1/workers/{id}`(secret을 담은 `endpoint` 노출)와 `POST /v1/hosts/register`(기존 Host의 `ssh_host`/`ssh_user`/`status`/`worker_id` 덮어쓰기)가 거절되는 테스트, 그리고 router의 모든 route가 행렬에 등록됐음을 강제하는 커버리지 테스트. **범위 확장(2026-08-22)**: `crates/fleet-dashboard`에는 중앙 capability 행렬이 아예 없고 핸들러에 `PermissionKind` 검사가 29곳 산재해 있다 — 같은 결함 계열이 Dashboard 표면에서 이미 재현 가능하며, `#86`~`#92`의 관리 화면이 대부분 이 표면에 놓인다. `/v1`과 동일한 중앙 행렬+커버리지 테스트를 Dashboard에도 적용한다 |
...
| #79 | 원격 실행 실패 관측 가능화 | P0 · 구현 대기 | 이 행 | `crates/fleet-provisioner/src/ssh.rs`의 `exec`/`exec_streaming`이 원격 exit code를 에러로 승격하고, `install_fleet_worker.rs`·`start_services.rs`·`push_credentials.rs`·`install_cloudflared.rs`의 `let _ =`와 `|| true`를 결과 검사로 교체한다. `start_services.rs`의 죽은 `wait_timeout_secs`를 실제 heartbeat 도달 폴링으로 구현하고, 실패 리포트의 빈 `steps` 벡터에 스텝 이력을 채운다. 게이트: 원격 실패가 스텝 실패로 전파되는 테스트 |
...
| #88 | Issue 엔티티와 상태 머신 | P2 · 설계 확정·구현 대기 | [Issue 추적](../architecture/issues.md) | 선행 `#48`, `#73`, `#76`. `Open/Triaged/Resolved/Closed(+reason)`이며 `InProgress` 부재. `tasks`에 `issue_id` 컬럼을 두지 않고 join 테이블로 연관. **교착 없음 3종**: 열린 Issue가 있어도 Task가 dead-letter까지 도달, 비터미널 Task가 있어도 Issue close 성공, Attempt 전이 코드 경로가 issue 테이블을 참조하지 않음을 강제하는 구조 시험. MemStore/PgStore 공유 행동 테스트 |
...
| #90 | dead-letter → Issue 집계 자동 생성 | P2 · 설계 확정·구현 대기 | [Issue 추적](../architecture/issues.md), [관측성·재조정](../architecture/observability-and-reconciliation.md) | 선행 `#89`, `#70`. Task당 1건이 아니라 원인별 집계. `CredentialMissing`은 model당 1건, `WorkerUnavailable`은 hysteresis 후, 일반 `WorkerError`는 OFF, `Cancelled`는 어떤 정책에서도 미생성. reconciler 재시작 후 재sweep이 중복 생성하지 않음(멱등). 자동 생성이 Task 상태를 바꾸지 않음 |
