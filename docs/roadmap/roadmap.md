...
| #83 | 인벤토리 모드 완결성 | P1 · 구현 대기 | 이 행 | `runtime.rs`의 `fleet_worker_bin: None` 하드코딩을 제거하고 `InventoryDefaults`/`InventoryWorker`에 필드를 추가한다. `grok_secret`은 `#82` 구현 결과 미지정 시 원격 `fleet-worker join`이 무작위로 생성하므로(오케스트레이터가 아니라 대상 호스트 쪽) 인벤토리에서 필수값으로 요구할 필요가 없다 — 선택 필드로 유지할지 완전히 제거할지 결정한다. `api_token` 필수 검증 추가(`#82`부터 `JoinWorker`가 `token:issue`로 이를 소비하므로 누락 시 사전에 막는 편이 낫다). 선언만 있고 참조 0건인 `retry_failed`는 구현하거나 제거한다. 게이트: 커밋된 25노드 인벤토리로 dry-run이 완주 |
