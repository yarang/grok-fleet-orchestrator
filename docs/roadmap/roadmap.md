...
| #72 | Admin API bearer token DB 기반 rotate/revoke | P1 · 설계 확정·구현 대기 | 이 행 (설계 요약: 현재 `FLEET_API_TOKENS`는 부팅 시 1회 로드되는 정적 env var(JSON `ApiTokenCredential[]`)라 회전에 파일 편집+서비스 재기동이 필요하고, 값이 세션 로그 등에 노출되면 재발급 전까지 무효화할 방법이 없다. `#60`의 Worker operational credential과 동일 패턴(digest 저장, 1회 원문 응답, rotate로 즉시 이전 값 무효화)을 admin bearer 토큰에 적용한다: 신규 `admin_api_tokens` 테이블(principal_id, token_digest UNIQUE, capabilities, created_at, rotated_at, revoked_at, rotation_generation), `POST /v1/admin/tokens`(생성)·`POST /v1/admin/tokens/:principal_id/rotate`·`DELETE /v1/admin/tokens/:principal_id`(회수)·`GET /v1/admin/tokens`(메타데이터만) API, 새 capability(기존 `token:*`은 bootstrap token 전용이라 재사용 금지 — `#66`에서 겪은 capability 이름 충돌 재발 방지). `auth_middleware`는 기존 `valid_tokens`(env) 목록과 DB 목록을 모두 검사해 하위 호환을 유지하되, 최초 기동 시 `FLEET_API_TOKENS`에 있는 항목을 DB로 1회 자동 가져오는(digest 계산 후 upsert) migration/startup 단계를 추가해 무중단 전환을 가능하게 한다. self-lockout 방지를 위해 rotate 응답에만 원문을 1회 반환한다.) | env 전용 배포에서 DB로 자동 전환 회귀, rotate 뒤 이전 토큰 즉시 거부, revoke 뒤 거부, self-rotate(호출자 자신의 토큰 회전) 시 새 원문이 응답에만 있고 로그에는 없음을 확인하는 redaction 테스트, `FLEET_GMAIL_APP_PASS`류 제3자 발급 시크릿은 이 메커니즘 대상이 아님을 문서에 명시 |
