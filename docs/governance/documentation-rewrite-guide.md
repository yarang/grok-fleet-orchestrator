...
진입점만 색인하며, 새 문서를 무분별하게 평면 나열하지 않는다.
...
Git 이력은 삭제된 본문을 복구할 수 있는 기록이다. 현재 구현에 영향을 주지 않는 오래된 문서를
정본 도메인에 보존하지 않는다.
...
```mermaid
flowchart LR
    A["도메인 진입점과 책임 확인"] --> B["현재 계약·근거·절차 분류"]
    B --> C["중복을 정본 링크로 교체"]
    C --> D["폐기 파일의 링크 정리·삭제"]
    D --> E["reviews에 비교 부기"]
    E --> F["index · log · lint"]
    F --> G["범위가 분명한 Git commit"]
```
...
- [ ] `docs/index.md`와 `docs/log.md`를 갱신하고 링크·형식 검사를 통과했다.
