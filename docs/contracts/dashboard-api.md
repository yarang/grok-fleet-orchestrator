...
`POST /api/agents/{id}/place`의 실패 코드는 2026-09-01(`#67` 게이트 ②)에 바뀌었다.

| 상황 | 코드 |
| --- | --- |
| 대상 Worker가 프로세스 상한에 도달 | 409 |
| 다른 Worker가 `running`으로 보고한 Agent를 옮기려 함 | 409 (**신규**) |
| 이 인스턴스가 더 이상 제어 기관이 아님 | 503 |
| Agent 또는 Worker 부재 | 404 |
| ~~회수된(`stopped`) Agent 배정~~ | ~~400~~ → **성공한다** |

두 줄이 계약 변경이다. 마지막 줄은 **거절하던 요청이 이제 성공한다**는 뜻이므로, 400을 기대하던
호출자는 조용히 다른 결과를 받는다. 이유는 [제어평면 권위와 장애 전환](../architecture/control-plane-authority-and-failover.md)의
`Agent self-fencing` 칸에 있다 — 관측 술어가 생기면 살아 있는 Agent를 옮기는 유일한 안전한
순서가 "회수 → 관측 소멸 → 이동 → 재기동"이 되고, 400 가드는 그 두 번째 걸음을 막아 이동
자체를 없앤다. 새 409는 그 반대 방향이다: 전에는 통과하던 재배정(다른 Worker가 실제로 돌리고
있는 Agent)이 이제 거절된다. 같은 Worker로의 재배정은 거절하지 않는다 — 프로세스가 움직이지
않으므로 중복이 생길 여지가 없다.
