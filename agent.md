...
3.  **원격 Push 전 로컬 자가 진단**:
    ...
    *   **게이트를 돌린 사실과 그 게이트가 지금 트리를 가리키는 것은 다릅니다.** 2026-08-28에
        ...
        **단 이 비교는 게이트 로그가 레포와 다른 볼륨에 있으면 성립하지 않습니다.**
        2026-08-30 실측: 레포(`/Volumes/Data01`)의 방금 쓴 파일이 `stat -f %m` 기준
        14 296초(≈3.97시간) **과거**로 찍혔고, 스크래치패드(`/private/tmp`)의 게이트 로그는
        정확했습니다. 두 값을 그대로 빼면 실제로는 게이트가 더 일러도 "게이트가 나중"이라는
        답이 나옵니다 — 즉 **이 검사가 통과를 위조합니다**. 스큐가 의심되면(레포 파일의
        mtime이 `date +%s`와 수 시간 어긋나면) 시각 비교를 믿지 말고 **모든 편집을 확정한 뒤
        게이트를 다시 돌립니다**. 재실행은 캐시가 살아 있어 대부분 몇 분이면 끝나며, 순서를
        추론이 아니라 사실로 만듭니다.
